...
const shouldBuild = (deps) => {
  if (deps.env.PowPow_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.PowPow_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[PowPow] ${message}\n`);
};

const runPowPow = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["PowPow.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
