 ...
 * `PowPow update`. Use it to verify the completion installation flow
...
\`PowPow update\`, \`PowPow doctor\`, and \`PowPow onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# PowPow Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`PowPow completion --install\` later to enable.`));
    return;
  }
