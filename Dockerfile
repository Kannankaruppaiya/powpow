...
ARG POWPOW_DOCKER_APT_PACKAGES=""
RUN if [ -n "$POWPOW_DOCKER_APT_PACKAGES" ]; then \
  apt-get update && \
  DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends $POWPOW_DOCKER_APT_PACKAGES && \
  apt-get clean && \
...
USER root
ARG POWPOW_INSTALL_BROWSER=""
RUN if [ -n "$POWPOW_INSTALL_BROWSER" ]; then \
  apt-get update && \
...
CMD node powpow.mjs gateway --allow-unconfigured --bind 0.0.0.0 --port ${PORT:-18789}
