...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/PowPow/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/PowPow");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/PowPow/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__PowPow_CONTROL_UI_BASE_PATH__ = "/PowPow";
    const app = mountApp("/PowPow/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/PowPow");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/PowPow/sessions");
  });
