...
export async function handleWhatsAppStart(host: PowPowApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: PowPowApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: PowPowApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: PowPowApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: PowPowApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: PowPowApp): string {
...
function resolveGatewayHttpAuthHeader(host: PowPowApp): string | null {
...
function buildGatewayHttpHeaders(host: PowPowApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: PowPowApp) {
...
export function handleNostrProfileToggleAdvanced(host: PowPowApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: PowPowApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: PowPowApp) {
  const state = host.nostrProfileFormState;
