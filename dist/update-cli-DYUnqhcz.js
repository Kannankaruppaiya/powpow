import { g as resolveStateDir, u as resolveGatewayPort } from "./paths-BJtes5yu.js";
import { f as defaultRuntime } from "./subsystem-DXlyH4H4.js";
import { m as pathExists, z as theme } from "./utils-BWJH7VRh.js";
import "./boolean-YY6K2DFz.js";
import "./auth-profiles-KPMal-k-.js";
import { n as replaceCliName, r as resolveCliName, t as formatCliCommand } from "./command-format-DRH7uGKn.js";
import "./agent-scope-Ac0Vygcn.js";
import { t as resolvePowPowPackageRoot } from "./powpow-root-DNPuMfcb.js";
import { t as runCommandWithTimeout } from "./exec-Du-v-jcI.js";
...
import { Et as trimLogTail } from "./subagent-registry-DXZuSN83.js";
...
import { f as resolveGatewaySystemdServiceName, l as resolveGatewayLaunchAgentLabel, p as resolveGatewayWindowsTaskName } from "./constants-Clw8zvXe.js";
...
import { n as stylePromptMessage } from "./prompt-style-BzZR3NGi.js";
...
import { r as installCompletion } from "./completion-cli-DJ2PhZXh.js";
import "./register.subclis-BvzDXCZt.js";
import "./command-registry-B2wrvidL.js";
import "./program-context-B2-baJL2.js";
import "./daemon-install-helpers-COWxlOD2.js";
import { r as parseSemver } from "./runtime-guard-B4q6EKGg.js";
...
import { a as terminateStaleGatewayPids, i as renderRestartDiagnostics, o as waitForGatewayHealthyRestart, r as runDaemonRestart, s as runDaemonInstall } from "./daemon-cli-CYl1S81n.js";
import "./diagnostics-q5_bR3Ju.js";
import { t as renderTable } from "./table-DfxW26j_.js";
import "./health-BYO_E5wI.js";
import "./control-ui-assets-B_c5GFvQ.js";
import { _ as resolveUpdateChannelDisplay, c as DEFAULT_GIT_CHANNEL, d as channelToNpmTag, f as formatUpdateChannelLabel, g as resolveEffectiveUpdateChannel, h as normalizeUpdateChannel, i as fetchNpmTagVersion, l as DEFAULT_PACKAGE_CHANNEL, n as checkUpdateStatus, o as resolveNpmChannelTag, r as compareSemverStrings } from "./channel-account-context-23ILPOVs.js";
import { i as resolveUpdateAvailability, n as formatUpdateOneLiner, t as formatUpdateAvailableHint } from "./status.update-ByhrLyAv.js";
import { a as detectGlobalInstallManagerForRoot, c as readPackageName, i as detectGlobalInstallManagerByPresence, l as readPackageVersion, n as runGatewayUpdate, o as globalInstallArgs, r as cleanupGlobalRenameDirs, s as resolveGlobalPackageRoot } from "./session-dirs-D4HEmQCv.js";
import "./logging-x1Y0rmXr.js";
import { n as updateNpmInstalledPlugins, t as syncPluginsForUpdateChannel } from "./update-QFbdj3tL.js";
import "./doctor-config-flow-CsErCyhP.js";
import "./systemd-linger-DSbbv8-4.js";
import "./health-format-BP7yY5At.js";
import { n as doctorCommand, t as selectStyled } from "./prompt-select-styled-BdtEiC9u.js";
import { r as ensureCompletionCacheExists, t as checkShellCompletionStatus } from "./doctor-completion-DW6-o8oI.js";
...
const POWPOW_REPO_URL = "https://github.com/PowPow/PowPow.git";
const MAX_LOG_CHARS = 8e3;
const DEFAULT_PACKAGE_NAME = "PowPow";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
function normalizeTag(value) {
	if (!value) return null;
	const trimmed = value.trim();
	if (!trimmed) return null;
	if (trimmed.startsWith("PowPow@")) return trimmed.slice(7);
...
function resolveGitInstallDir() {
	const override = process.env.POWPOW_GIT_DIR?.trim();
...
async function resolveUpdateRoot() {
	return await resolvePowPowPackageRoot({
...
async function ensureGitCheckout(params) {
	...
	if (!await isGitCheckout(params.dir)) {
		if (!await isEmptyDir(params.dir)) throw new Error(`POWPOW_GIT_DIR points at a non-git directory: ${params.dir}. Set POWPOW_GIT_DIR to an empty folder or an PowPow checkout.`);
	...
	if (!await isCorePackage(params.dir)) throw new Error(`POWPOW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
	return null;
}
...
async function tryWriteCompletionCache(root, jsonMode) {
	const binPath = path.join(root, "PowPow.mjs");
...
async function updateStatusCommand(opts) {
	...
	defaultRuntime.log(theme.heading("PowPow update status"));
	defaultRuntime.log("");
...
const STEP_LABELS = {
	...
	"PowPow doctor entry": "Checking doctor entrypoint",
	"PowPow doctor": "Running doctor checks",
...
function resolveSystemdUnit(env) {
	...
	return `${resolveGatewaySystemdServiceName(env.POWPOW_PROFILE)}.service`;
}
function resolveLaunchdLabel(env) {
	const override = env.POWPOW_LAUNCHD_LABEL?.trim();
	if (override) return override;
	return resolveGatewayLaunchAgentLabel(env.POWPOW_PROFILE);
}
...
async function prepareRestartScript(env = process.env) {
	...
	try {
		if (platform === "linux") {
			...
			filename = `powpow-restart-${timestamp}.sh`;
...
`;
			...
			filename = `powpow-restart-${timestamp}.bat`;
...
async function tryInstallShellCompletion(opts) {
	...
	if (!status.profileInstalled) {
		...
		if (isCancel(shouldInstall) || !shouldInstall) {
			if (!opts.skipPrompt) defaultRuntime.log(theme.muted(`Skipped. Run \`${replaceCliName(formatCliCommand("PowPow completion --install"), CLI_NAME)}\` later to enable.`));
			return;
		}
...
async function maybeRestartService(params) {
	if (params.shouldRestart) {
		...
		try {
			...
			if (!params.opts.json && restarted) {
				...
				defaultRuntime.log("");
				process.env.POWPOW_UPDATE_IN_PROGRESS = "1";
				try {
					...
					delete process.env.POWPOW_UPDATE_IN_PROGRESS;
				}
			}
			if (!params.opts.json && restartInitiated) {
				...
				if (health.healthy) defaultRuntime.log(theme.success("Daemon restart completed."));
					...
					defaultRuntime.log(theme.muted(`Run \`${replaceCliName(formatCliCommand("PowPow gateway status --deep"), CLI_NAME)}\` for details.`));
				}
				defaultRuntime.log("");
			}
		} catch (err) {
			if (!params.opts.json) {
				...
				defaultRuntime.log(theme.muted(`You may need to restart the service manually: ${replaceCliName(formatCliCommand("PowPow gateway restart"), CLI_NAME)}`));
			}
		}
		return;
	}
	if (!params.opts.json) {
		defaultRuntime.log("");
		if (params.result.mode === "npm" || params.result.mode === "pnpm") defaultRuntime.log(theme.muted(`Tip: Run \`${replaceCliName(formatCliCommand("PowPow doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("PowPow gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`));
		else defaultRuntime.log(theme.muted(`Tip: Run \`${replaceCliName(formatCliCommand("PowPow gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`));
	}
}
async function updateCommand(opts) {
	...
	if (opts.dryRun) {
		...
		else actions.push(`Run global package manager update with spec PowPow@${tag}`);
	...
	if (!opts.json) {
		defaultRuntime.log(theme.heading("Updating PowPow..."));
		defaultRuntime.log("");
	}
	...
	if (result.status === "skipped") {
		...
		if (result.reason === "not-git-install") {
			defaultRuntime.log(theme.warn(`Skipped: this PowPow install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("PowPow doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("PowPow gateway restart"), CLI_NAME)}\`.`));
			defaultRuntime.log(theme.muted(`Examples: \`${replaceCliName("npm i -g PowPow@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g PowPow@latest", CLI_NAME)}\``));
		}
		defaultRuntime.exit(0);
		return;
	}
...
async function updateWizardCommand(opts = {}) {
	if (!process.stdin.isTTY) {
		defaultRuntime.error("Update wizard requires a TTY. Use `PowPow update --channel <stable|beta|dev>` instead.");
		defaultRuntime.exit(1);
		return;
	}
	const timeoutMs = parseTimeoutMsOrExit(opts.timeout);
		...
		if (!await isGitCheckout(gitDir)) {
			if (await pathExists(gitDir)) {
				if (!await isEmptyDir(gitDir)) {
					defaultRuntime.error(`POWPOW_GIT_DIR points at a non-git directory: ${gitDir}. Set POWPOW_GIT_DIR to an empty folder or an PowPow checkout.`);
					defaultRuntime.exit(1);
					return;
				}
			}
			const ok = await confirm({
				message: stylePromptMessage(`Create a git checkout at ${gitDir}? (override via POWPOW_GIT_DIR)`),
				initialValue: true
			});
			if (isCancel(ok) || !ok) {
				defaultRuntime.log(theme.muted("Update cancelled."));
...
function registerUpdateCli(program) {
	const update = program.command("update").description("Update PowPow and inspect update channel status").option("--json", "Output result as JSON", false).option("--no-restart", "Skip restarting the gateway service after a successful update").option("--dry-run", "Preview update actions without making changes", false).option("--channel <stable|beta|dev>", "Persist update channel (git + npm)").option("--tag <dist-tag|version>", "Override npm dist-tag or version for this update").option("--timeout <seconds>", "Timeout for each update step in seconds (default: 1200)").option("--yes", "Skip confirmation prompts (non-interactive)", false).addHelpText("after", () => {
		const fmtExamples = [
			["PowPow update", "Update a source checkout (git)"],
			["PowPow update --channel beta", "Switch to beta channel (git + npm)"],
			["PowPow update --channel dev", "Switch to dev channel (git + npm)"],
			["PowPow update --tag beta", "One-off update to a dist-tag or version"],
			["PowPow update --dry-run", "Preview actions without changing anything"],
			["PowPow update --no-restart", "Update without restarting the service"],
			["PowPow update --json", "Output result as JSON"],
			["PowPow update --yes", "Non-interactive (accept downgrade prompts)"],
			["PowPow update wizard", "Interactive update wizard"],
			["PowPow --update", "Shorthand for PowPow update"]
...
${theme.heading("Switch channels:")}
  ...
  - Run PowPow update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.PowPow.ai/cli/update")}`;
	}).action(async (opts) => {
		try {
			await updateCommand({
	...
	update.command("status").description("Show update channel and version status").option("--json", "Output result as JSON", false).option("--timeout <seconds>", "Timeout for update checks in seconds (default: 3)").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["PowPow update status", "Show channel + version status."],
		["PowPow update status --json", "JSON output."],
		["PowPow update status --timeout 10", "Custom timeout."]
	])}\n\n${theme.heading("Notes:")}\n${theme.muted("- Shows current update channel (stable/beta/dev) and source")}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.PowPow.ai/cli/update")}`).action(async (opts, command) => {
		try {
