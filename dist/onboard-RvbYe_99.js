...
import { f as defaultRuntime, p as restoreTerminalState } from "./subsystem-DXlyH4H4.js";
...
import { a as ensureWorkspaceAndSessions, b as waitForGatewayReachable, c as handleReset, g as resolveControlUiLinks, h as randomToken, n as applyWizardMetadata, t as DEFAULT_WORKSPACE } from "./onboard-helpers-BNEaJ_0q.js";
...
import { t as DEFAULT_GATEWAY_DAEMON_RUNTIME } from "./daemon-runtime-BXbwrY5G.js";
import { t as runOnboardingWizard } from "./onboarding-BwuOyEgA.js";
import { n as logConfigUpdated } from "./logging-x1Y0rmXr.js";
import { i as normalizeLegacyOnboardAuthChoice, r as isDeprecatedAuthChoice, t as ONBOARD_PROVIDER_AUTH_FLAGS } from "./onboard-provider-auth-flags-DYmVtasB.js";
import { t as applyOnboardingLocalWorkspaceConfig } from "./onboard-config-Gz9nLZlh.js";
...
async function runNonInteractiveOnboardingLocal(params) {
	...
	if (authChoice !== "skip") {
		const { applyNonInteractiveAuthChoice } = await import("./auth-choice-DM49sO64.js");
	...
	if (opts.installDaemon) {
		const { installGatewayDaemonNonInteractive } = await import("./daemon-install-CoE_CFsH.js");
	...
	if (!opts.json) runtime.log(`Tip: run \`${formatCliCommand("PowPow configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.PowPow.ai/tools/web`);
}
...
async function runNonInteractiveOnboardingRemote(params) {
	...
	if (opts.json) runtime.log(JSON.stringify(payload, null, 2));
		...
		runtime.log(`Tip: run \`${formatCliCommand("PowPow configure --section web")}\` to store your Brave API key for web_search. Docs: https://docs.PowPow.ai/tools/web`);
	}
}
...
async function runNonInteractiveOnboarding(opts, runtime = defaultRuntime) {
	...
	if (snapshot.exists && !snapshot.valid) {
		runtime.error(`Config invalid. Run \`${formatCliCommand("PowPow doctor")}\` to repair it, then re-run onboarding.`);
		runtime.exit(1);
		return;
	}
...
async function onboardCommand(opts, runtime = defaultRuntime) {
	...
	if (normalizedOpts.nonInteractive && normalizedOpts.acceptRisk !== true) {
		runtime.error([
			...
			"Read: https://docs.PowPow.ai/security",
			`Re-run with: ${formatCliCommand("PowPow onboard --non-interactive --accept-risk ...")}`
		].join("\n"));
		runtime.exit(1);
		return;
	}
	...
	if (process.platform === "win32") runtime.log([
		"Windows detected — PowPow runs great on WSL2!",
		"Native Windows might be trickier.",
		"Quick setup: wsl --install (one command, one reboot)",
		"Guide: https://docs.PowPow.ai/windows"
	].join("\n"));
