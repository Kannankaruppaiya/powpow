...
import { a as isCarrierGradeNatIpv4Address, u as isRfc1918Ipv4Address } from "./ip-oUNIYnzv.js";
...
function resolveAuth(cfg, env) {
	...
	const token = env.POWPOW_GATEWAY_TOKEN?.trim() || env.powpow_GATEWAY_TOKEN?.trim() || cfg.gateway?.auth?.token?.trim();
	const password = env.POWPOW_GATEWAY_PASSWORD?.trim() || env.powpow_GATEWAY_PASSWORD?.trim() || cfg.gateway?.auth?.password?.trim();
	if (mode === "password") {
	...
	program.command("qr").description("Generate an iOS pairing QR code and setup code").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/qr", "docs.PowPow.ai/cli/qr")}\n`).option("--remote", "Use gateway.remote.url and gateway.remote token/password (ignores device-pair publicUrl)", false).option("--url <url>", "Override gateway URL used in the setup payload").option("--public-url <url>", "Override gateway public URL used in the setup payload").option("--token <token>", "Override gateway token for setup payload").option("--password <password>", "Override gateway password for setup payload").option("--setup-code-only", "Print only the setup code", false).option("--no-ascii", "Skip ASCII QR rendering").option("--json", "Output JSON", false).action(async (opts) => {
		try {
			...
			const lines = [
				...
				"Scan this with the PowPow iOS app (Onboarding -> Scan QR).",
				""
			];
			if (opts.ascii !== false) {
			...
			lines.push(`${theme.muted("Setup code:")} ${setupCode}`, `${theme.muted("Gateway:")} ${resolved.payload.url}`, `${theme.muted("Auth:")} ${resolved.authLabel}`, `${theme.muted("Source:")} ${resolved.urlSource}`, "", "Approve after scan with:", `  ${theme.command("PowPow devices list")}`, `  ${theme.command("PowPow devices approve <requestId>")}`);
