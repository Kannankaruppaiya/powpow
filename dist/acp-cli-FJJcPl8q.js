...
import { v as resolveUserPath, x as shortenHomePath, z as theme } from "./utils-BWJH7VRh.js";
...
import { t as buildGatewayConnectionDetails } from "./call-Dy4jPY6_.js";
...
import { r as isKnownCoreToolId } from "./tool-catalog-7y19qjm_.js";
import { t as formatDocsLink } from "./links-C00eySj4.js";
import { t as ensurePowPowCliOnPath } from "./path-env-DDMTwZSn.js";
import { t as DANGEROUS_ACP_TOOLS } from "./dangerous-tools-yEBF5xhU.js";
...
async function createAcpClient(opts = {}) {
	...
	const serverCommand = opts.serverCommand ?? (entryPath ? process.execPath : "PowPow");
...
function printHelp() {
	console.log(`Usage: PowPow acp [options]
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
	...
	if (argv.includes("--token") || argv.includes("--gateway-token")) console.error("Warning: --token can be exposed via process listings. Prefer --token-file or POWPOW_GATEWAY_TOKEN.");
	if (argv.includes("--password") || argv.includes("--gateway-password")) console.error("Warning: --password can be exposed via process listings. Prefer --password-file or POWPOW_GATEWAY_PASSWORD.");
...
function registerAcpCli(program) {
	...
	acp.command("client").description("Run an interactive ACP client against the local ACP bridge").option("--cwd <dir>", "Working directory for the ACP session").option("--server <command>", "ACP server command (default: PowPow)").option("--server-args <args...>", "Extra arguments for the ACP server").option("--server-verbose", "Enable verbose logging on the ACP server", false).option("-v, --verbose", "Verbose client logging", false).action(async (opts, command) => {
