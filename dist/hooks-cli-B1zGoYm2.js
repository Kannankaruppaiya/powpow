...
import { t as CONFIG_DIR, v as resolveUserPath, x as shortenHomePath, z as theme } from "./utils-BWJH7VRh.js";
...
import { a as MANIFEST_KEY, n as isPathInside, r as isPathInsideWithRealpath } from "./scan-paths-CYzbUHyr.js";
import "./skills-D7zMUUWl.js";
import "./manifest-registry-C9O9KUb3.js";
import { i as loadConfig, l as writeConfigFile } from "./config-B91e8u6L.js";
...
import { l as promptYesNo } from "./tailscale-Mny9GO9e.js";
...
import { c as resolveArchiveKind, i as unscopedPackageName, o as fileExists, s as readJsonFile, t as resolveSafeInstallDir } from "./install-safe-path-DoxY3Jl3.js";
import { a as resolveInstallModeOptions, c as withExtractedArchiveRoot, i as installPackageDir, l as resolveArchiveSourcePath, n as finalizeNpmSpecArchiveInstall, o as resolveTimedInstallModeOptions, r as installFromNpmSpecArchiveWithInstaller, s as resolveExistingInstallPath, t as validateRegistryNpmSpec } from "./npm-registry-spec-LTflfgy4.js";
import { t as renderTable } from "./table-DfxW26j_.js";
import { a as parseFrontmatter, t as loadWorkspaceHookEntries } from "./workspace-Bl_aACEx.js";
import { t as buildWorkspaceHookStatus } from "./hooks-status-BjfDzbkF.js";
import { n as resolvePinnedNpmInstallRecordForCli, r as buildPluginStatusReport, t as buildNpmInstallRecordFields } from "./npm-resolution-Cw09qyEr.js";
...
async function ensurePowPowHooks(manifest) {
	const hooks = manifest[MANIFEST_KEY]?.hooks;
	if (!Array.isArray(hooks)) throw new Error("package.json missing PowPow.hooks");
	const list = hooks.map((e) => typeof e === "string" ? e.trim() : "").filter(Boolean);
	if (list.length === 0) throw new Error("package.json PowPow.hooks is empty");
	return list;
}
...
async function installHookPackageFromDir(params) {
	...
	let hookEntries;
	try {
		hookEntries = await ensurePowPowHooks(manifest);
	} catch (err) {
		return {
			ok: false,
			error: String(err)
	...
	for (const entry of hookEntries) {
		...
		if (!isPathInside(params.packageDir, hookDir)) return {
			ok: false,
			error: `PowPow.hooks entry escapes package directory: ${entry}`
		};
		await validateHookDir(hookDir);
		if (!isPathInsideWithRealpath(params.packageDir, hookDir, { requireRealpath: true })) return {
			ok: false,
			error: `PowPow.hooks entry resolves outside package directory: ${entry}`
		};
...
async function installHooksFromNpmSpec(params) {
	...
	return finalizeNpmSpecArchiveInstall(await installFromNpmSpecArchiveWithInstaller({
		tempDirPrefix: "powpow-hook-pack-",
		spec,
		timeoutMs,
...
function formatHooksList(report, opts) {
	...
	if (hooks.length === 0) return opts.eligible ? `No eligible hooks found. Run \`${formatCliCommand("PowPow hooks list")}\` to see all hooks.` : "No hooks found.";
...
function formatHookInfo(report, hookName, opts) {
	...
	if (!hook) {
		...
		return `Hook "${hookName}" not found. Run \`${formatCliCommand("PowPow hooks list")}\` to see available hooks.`;
	}
