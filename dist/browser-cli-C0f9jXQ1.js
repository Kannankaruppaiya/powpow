...
import { D as danger, O as info, x as shortenHomePath, z as theme } from "./utils-BWJH7VRh.js";
...
import { i as resolveExistingPathsWithinRoot, r as DEFAULT_UPLOAD_DIR } from "./paths-B2BIhK5k.js";
...
import { t as copyToClipboard } from "./clipboard-BureGS5q.js";
...
function registerBrowserFilesAndDownloadsCommands(browser, parentOpts) {
	...
	browser.command("upload").description("Arm file upload for the next file chooser").argument("<paths...>", "File paths to upload (must be within PowPow temp uploads dir, e.g. /tmp/PowPow/uploads/file.pdf)").option("--ref <ref>", "Ref id from snapshot to click after arming").option("--input-ref <ref>", "Ref id for <input type=file> to set directly").option("--element <selector>", "CSS selector for <input type=file>").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the next file chooser (default: 120000)", (v) => Number(v)).action(async (paths, opts, cmd) => {
		const { parent, profile } = resolveBrowserActionContext(cmd, parentOpts);
		try {
	...
	browser.command("waitfordownload").description("Wait for the next download (and save it)").argument("[path]", "Save path within PowPow temp downloads dir (default: /tmp/PowPow/downloads/...; fallback: os.tmpdir()/PowPow/downloads/...)").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the next download (default: 120000)", (v) => Number(v)).action(async (outPath, opts, cmd) => {
		await runDownloadCommand(cmd, opts, {
			path: "/wait/download",
			body: { path: outPath?.trim() || void 0 }
		});
	});
	browser.command("download").description("Click a ref and save the resulting download").argument("<ref>", "Ref id from snapshot to click").argument("<path>", "Save path within PowPow temp downloads dir (e.g. report.pdf or /tmp/PowPow/downloads/report.pdf)").option("--target-id <id>", "CDP target id (or unique prefix)").option("--timeout-ms <ms>", "How long to wait for the download to start (default: 120000)", (v) => Number(v)).action(async (ref, outPath, opts, cmd) => {
		await runDownloadCommand(cmd, opts, {
			path: "/download",
			body: {
				ref,
...
function registerBrowserDebugCommands(browser, parentOpts) {
	...
	trace.command("stop").description("Stop trace recording and write a .zip").option("--out <path>", "Output path within PowPow temp dir (e.g. trace.zip or /tmp/PowPow/trace.zip)").option("--target-id <id>", "CDP target id (or unique prefix)").action(async (opts, cmd) => {
		const parent = parentOpts(cmd);
		const profile = parent?.browserProfile;
		await runBrowserDebug(async () => {
			const result = await callBrowserRequest(parent, {
...
async function installChromeExtension(opts) {
	...
	if (!hasManifest(src)) throw new Error("Bundled Chrome extension is missing. Reinstall PowPow and try again.");
...
function registerBrowserExtensionCommands(browser, parentOpts) {
	...
	ext.command("install").description("Install the Chrome extension to a stable local path").action(async (_opts, cmd) => {
		...
		defaultRuntime.error(info([
			...
			`- Pin “PowPow Browser Relay”, then click it on the tab (badge shows ON)`,
			"",
			`${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.PowPow.ai/tools/chrome-extension")}`
		].join("\n")));
	});
	ext.command("path").description("Print the path to the installed Chrome extension (load unpacked)").action(async (_opts, cmd) => {
		...
		if (!hasManifest(dir)) {
			defaultRuntime.error(danger([`Chrome extension is not installed. Run: "${formatCliCommand("PowPow browser extension install")}"`, `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.PowPow.ai/tools/chrome-extension")}`].join("\n")));
			defaultRuntime.exit(1);
		}
		if (parent?.json) {
...
async function runBrowserToggle(parent, params) {
	...
	const name = status.profile ?? "PowPow";
...
function registerBrowserManageCommands(browser, parentOpts) {
	...
	browser.command("create-profile").description("Create a new browser profile").requiredOption("--name <name>", "Profile name (lowercase, numbers, hyphens)").option("--color <hex>", "Profile color (hex format, e.g. #0066CC)").option("--cdp-url <url>", "CDP URL for remote Chrome (http/https)").option("--driver <driver>", "Profile driver (PowPow|extension). Default: PowPow").action(async (opts, cmd) => {
		const parent = parentOpts(cmd);
		await runBrowserCommand$1(async () => {
			const result = await callBrowserRequest(parent, {
				method: "POST",
...
function registerBrowserCli(program) {
	const browser = program.command("browser").description("Manage PowPow's dedicated browser (Chrome/Chromium)").option("--browser-profile <name>", "Browser profile name (default from config)").option("--json", "Output machine-readable JSON", false).addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([...browserCoreExamples, ...browserActionExamples].map((cmd) => [cmd, ""]), true)}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/browser", "docs.PowPow.ai/cli/browser")}\n`).action(() => {
		browser.outputHelp();
		defaultRuntime.error(danger(`Missing subcommand. Try: "${formatCliCommand("PowPow browser status")}"`));
		defaultRuntime.exit(1);
	});
