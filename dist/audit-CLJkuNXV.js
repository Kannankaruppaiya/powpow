import { g as resolveStateDir, m as resolveOAuthDir, o as resolveConfigPath } from "./paths-BJtes5yu.js";
import { t as formatCliCommand } from "./command-format-DRH7uGKn.js";
import { M as resolveAgentModelFallbackValues, N as resolveAgentModelPrimaryValue, d as resolveDefaultAgentId } from "./agent-scope-Ac0Vygcn.js";
import { s as normalizeAgentId } from "./session-key-DXmKFT2p.js";
import { n as runExec } from "./exec-Du-v-jcI.js";
import { a as MANIFEST_KEY, n as isPathInside, t as extensionUsesSkippedScannerPath } from "./scan-paths-CYzbUHyr.js";
import { i as loadWorkspaceSkillEntries } from "./skills-D7zMUUWl.js";
import { u as normalizePluginsConfig } from "./manifest-registry-C9O9KUb3.js";
import { C as isDangerousNetworkMode, Dt as MAX_INCLUDE_DEPTH, Et as INCLUDE_KEY, J as normalizeTrustedSafeBinDirs, r as createConfigIO, w as normalizeNetworkMode } from "./config-B91e8u6L.js";
import { l as normalizeStringEntries } from "./dock-DaMhmHr_.js";
import { h as GATEWAY_CLIENT_NAMES, m as GATEWAY_CLIENT_MODES } from "./message-channel-DYK7E7vt.js";
import { n as listChannelPlugins } from "./plugins-CPaMeorQ.js";
import { t as GatewayClient } from "./client-FngVpCQH.js";
import { c as READ_SCOPE, t as buildGatewayConnectionDetails } from "./call-Dy4jPY6_.js";
import { t as resolveGatewayCredentialsFromConfig } from "./credentials-DAkLWBAC.js";
import { C as resolveToolProfilePolicy, f as resolveSandboxConfigForAgent, k as SANDBOX_BROWSER_SECURITY_HASH_EPOCH, l as execDockerRaw, m as resolveSandboxToolPolicyForAgent, u as getBlockedBindReason } from "./sandbox-Vr4z2IvO.js";
import { a as resolveGatewayAuth } from "./auth-BQr6HjtW.js";
import { a as resolveProfile, i as resolveBrowserConfig, s as resolveBrowserControlAuth } from "./server-context-B-VoITNb.js";
import { n as formatErrorMessage } from "./errors-BpyuHard.js";
import { r as resolveMergedSafeBinProfileFixtures, t as listInterpreterLikeSafeBins } from "./exec-safe-bin-runtime-policy-C5YP9uai.js";
import { a as resolveNativeSkillsEnabled, i as resolveNativeCommandsEnabled } from "./commands-BVUHpNuP.js";
import { i as readChannelAllowFromStore } from "./pairing-store-h5-Hnjki.js";
import { t as listAgentWorkspaceDirs } from "./workspace-dirs-CwvkZfTb.js";
import { n as isToolAllowedByPolicies, o as pickSandboxToolPolicy, s as resolveDmAllowState } from "./pi-tools.policy-BVP5EZDM.js";
import { n as isDangerousNameMatchingEnabled } from "./dangerous-name-matching-GmKpOlxa.js";
...
import { t as scanDirectoryWithSummary } from "./skill-scanner-Cs4mlgip.js";
import { c as normalizeTelegramAllowFromEntry, s as isNumericTelegramUserId, t as isDiscordMutableAllowEntry } from "./mutable-allowlist-detectors-CbikekZq.js";
import { t as inferParamBFromIdOrName } from "./model-param-b-Fhp5PLdR.js";
...
async function collectChannelSecurityFindings(params) {
	...
	const warnDmPolicy = async (input) => {
		...
		if (dmScope === "main" && isMultiUserDm) findings.push({
			...
			remediation: "Run: " + formatCliCommand("PowPow config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate DM sessions per sender."
		});
	};
		...
		for (const accountId of orderedAccountIds) {
			...
			if (plugin.id === "discord") {
				...
				addDiscordNameBasedEntries({
					...
					source: "~/.PowPow/credentials/discord-allowFrom.json"
				});
...
function collectSyncedFolderFindings(params) {
	...
	if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) findings.push({
		...
		remediation: `Keep POWPOW_STATE_DIR on a local-only volume and re-run "${formatCliCommand("PowPow security audit --fix")}".`
	});
	return findings;
}
function collectSecretsInConfigFindings(cfg) {
	...
	if (password && !looksLikeEnvRef(password)) findings.push({
		...
		remediation: "Prefer POWPOW_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk."
	});
...
function collectHooksHardeningFindings(cfg, env = process.env) {
	...
	const PowPowGatewayToken = typeof env.POWPOW_GATEWAY_TOKEN === "string" && env.POWPOW_GATEWAY_TOKEN.trim() ? env.POWPOW_GATEWAY_TOKEN.trim() : null;
	const gatewayToken = gatewayAuth.mode === "token" && typeof gatewayAuth.token === "string" && gatewayAuth.token.trim() ? gatewayAuth.token.trim() : PowPowGatewayToken ? PowPowGatewayToken : null;
...
function collectGatewayHttpSessionKeyOverrideFindings(cfg) {
	...
	findings.push({
		...
		detail: `${enabledEndpoints.join(", ")} accept x-powpow-session-key for per-request session routing. Treat API credential holders as trusted principals.`
	});
	return findings;
}
...
function collectSandboxDangerousConfigFindings(cfg) {
	...
	if (browserExposurePaths.length > 0) findings.push({
		...
		remediation: "Set sandbox.browser.network to a dedicated bridge network (recommended default: powpow-sandbox-browser), or set sandbox.browser.cdpSourceRange (for example 172.21.0.1/32) to restrict container-edge CDP ingress."
	});
	return findings;
}
...
function collectLikelyMultiUserSetupFindings(cfg) {
	...
	findings.push({
		...
		detail: "Heuristic signals indicate this gateway may be reachable by multiple users:\n" + signals.map((signal) => `- ${signal}`).join("\n") + `\n${impactLine}\n${riskyContextsDetail}\nPowPow's default security model is personal-assistant (one trusted operator boundary), not hostile multi-tenant isolation on one shared gateway.`,
...
async function readSandboxBrowserHashLabels(params) {
	try {
		const result = await params.execDockerRawFn([
			"inspect",
			"-f",
			"{{ index .Config.Labels \"PowPow.configHash\" }}	{{ index .Config.Labels \"PowPow.browserConfigEpoch\" }}",
...
async function collectSandboxBrowserHashLabelFindings(params) {
	...
	if (missingHash.length > 0) findings.push({
		...
		remediation: `${formatCliCommand("PowPow sandbox recreate --browser --all")} (add --force to skip prompt).`
	});
	if (staleEpoch.length > 0) findings.push({
		...
		detail: `Containers: ${staleEpoch.join(", ")}. Expected PowPow.browserConfigEpoch=${SANDBOX_BROWSER_SECURITY_HASH_EPOCH}.`,
		remediation: `${formatCliCommand("PowPow sandbox recreate --browser --all")} (add --force to skip prompt).`
	});
	if (nonLoopbackPublished.length > 0) findings.push({
		...
		remediation: `${formatCliCommand("PowPow sandbox recreate --browser --all")} (add --force to skip prompt), then verify published ports are bound to 127.0.0.1.`
	});
	return findings;
}
async function collectPluginsTrustFindings(params) {
	...
	if (pluginDirs.length > 0) {
		...
		if (enabledExtensionPluginIds.length > 0) {
			...
			for (const context of contexts) {
				...
				const broadPolicy = isToolAllowedByPolicies("__POWPOW_plugin_probe__", resolveToolPolicies({
					cfg: params.cfg,
					agentTools: context.tools,
					sandboxMode,
	...
	if (npmPluginInstalls.length > 0) {
		...
		if (pluginVersionDrift.length > 0) findings.push({
			...
			remediation: "Run `PowPow plugins update --all` (or reinstall affected plugins) to refresh install metadata."
		});
	}
	...
	if (npmHookInstalls.length > 0) {
		...
		if (hookVersionDrift.length > 0) findings.push({
			...
			remediation: "Run `PowPow hooks update --all` (or reinstall affected hooks) to refresh install metadata."
		});
	}
	return findings;
}
...
async function collectPluginsCodeSafetyFindings(params) {
	...
	const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
		...
		onReadError: (err) => {
			findings.push({
				...
				remediation: "Check file permissions and plugin layout, then rerun `PowPow security audit --deep`."
			});
		}
	});
	for (const pluginName of pluginDirs) {
		...
		if (escapedEntries.length > 0) findings.push({
			...
			remediation: "Update the plugin manifest so all PowPow.extensions entries stay inside the plugin directory."
		});
		...
		if (summary.critical > 0) {
			...
			findings.push({
				...
				remediation: "Review the plugin source code carefully before use. If untrusted, remove the plugin from your PowPow extensions state directory."
			});
...
async function collectInstalledSkillsCodeSafetyFindings(params) {
	...
	for (const workspaceDir of workspaceDirs) {
		...
		for (const entry of entries) {
			if (entry.skill.source === "powpow-bundled") continue;
			...
			const summary = await scanDirectoryWithSummary(skillDir).catch((err) => {
				findings.push({
					...
					remediation: "Check file permissions and skill layout, then rerun `PowPow security audit --deep`."
				});
				return null;
			});
			if (!summary) continue;
...
async function collectFilesystemFindings(params) {
	...
	if (stateDirPerms.ok) {
		...
		if (stateDirPerms.worldWritable) findings.push({
			...
			detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your PowPow state.`,
			remediation: formatPermissionRemediation({
			...
			detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your PowPow state.`,
			remediation: formatPermissionRemediation({
				targetPath: params.stateDir,
				perms: stateDirPerms,
				isDir: true,
...
function collectBrowserControlFindings(cfg, env) {
	...
	try {
		...
		findings.push({
			...
			detail: String(err),
			remediation: `Fix browser.cdpUrl in ${resolveConfigPath()} and re-run "${formatCliCommand("PowPow security audit --deep")}".`
		});
		return findings;
	}
...
async function runSecurityAudit(opts) {
	...
	if (deep?.gateway?.attempted && !deep.gateway.ok) findings.push({
		...
		remediation: `Run "${formatCliCommand("PowPow status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("PowPow security audit --deep")}".`
	});
...
//#endregion
export { collectIncludePathsRecursive as a, resolveNodeCommandAllowlist as c, formatIcaclsResetCommand as i, probeGateway as l, collectEnabledInsecureOrDangerousFlags as n, resolveGatewayProbeAuth as o, createIcaclsResetCommand as r, isNodeCommandAllowed as s, runSecurityAudit as t };