...
import { n as resolveBrewPathDirs } from "./brew-Ber9DJ6h.js";
...
function candidateBinDirs(opts) {
	...
	try {
		...
		if (isExecutable(path.join(execDir, "PowPow"))) prepend.push(execDir);
	} catch {}
	if (opts.allowProjectLocalBin === true || isTruthyEnvValue(process.env.POWPOW_ALLOW_PROJECT_LOCAL_BIN)) {
		const localBinDir = path.join(cwd, "node_modules", ".bin");
		if (isExecutable(path.join(localBinDir, "PowPow"))) append.push(localBinDir);
	}
...
*/
function ensurePowPowCliOnPath(opts = {}) {
	if (isTruthyEnvValue(process.env.POWPOW_PATH_BOOTSTRAPPED)) return;
...
//#endregion
export { ensurePowPowCliOnPath as t };