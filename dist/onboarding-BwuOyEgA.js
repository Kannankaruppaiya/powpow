import { n as DEFAULT_GATEWAY_PORT, u as resolveGatewayPort } from "./paths-BJtes5yu.js";
...
async function requireRiskAcknowledgement(params) {
	...
	await params.prompter.note([
		...
		"",
		"PowPow is a hobby project and still in beta. Expect sharp edges.",
		"By default, PowPow is a personal agent: one trusted operator boundary.",
		"This bot can read files and run actions if tools are enabled.",
		"A bad prompt can trick it into doing unsafe things.",
		"",
		"PowPow is not a hostile multi-tenant boundary by default.",
		"If multiple users can message one tool-enabled agent, they share that delegated tool authority.",
		"",
		"If you’re not comfortable with security hardening and access control, don’t run PowPow.",
		...
		"PowPow security audit --fix",
		"",
		"Must read: https://docs.PowPow.ai/gateway/security"
	].join("\n"), "Security");
...
async function runOnboardingWizard(opts, runtime = defaultRuntime, prompter) {
	const onboardHelpers = await import("./onboard-helpers-BNEaJ_0q.js").then((n) => n.d);
	onboardHelpers.printWizardHeader(runtime);
	await prompter.intro("PowPow onboarding");
	...
	if (snapshot.exists && !snapshot.valid) {
		...
		await prompter.outro(`Config invalid. Run \`${formatCliCommand("PowPow doctor")}\` to repair it, then re-run onboarding.`);
		runtime.exit(1);
		return;
	}
	const quickstartHint = `Configure details later via ${formatCliCommand("PowPow configure")}.`;
	...
	if (mode === "remote") {
		const { promptRemoteGatewayConfig } = await import("./onboard-remote-DTIOnGgc.js").then((n) => n.t);
		const { logConfigUpdated } = await import("./logging-x1Y0rmXr.js").then((n) => n.r);
	...
	const { applyOnboardingLocalWorkspaceConfig } = await import("./onboard-config-Gz9nLZlh.js").then((n) => n.n);
	...
	const { promptAuthChoiceGrouped } = await import("./auth-choice-prompt-BkMDAVXQ.js").then((n) => n.t);
	const { promptCustomApiConfig } = await import("./onboard-custom-Da9woe9U.js").then((n) => n.r);
	const { applyAuthChoice, resolvePreferredProviderForAuthChoice, warnIfModelConfigLooksOff } = await import("./auth-choice-DR41Z0CW.js").then((n) => n.t);
	const { applyPrimaryModel, promptDefaultModel } = await import("./model-picker-BEfZoi__.js").then((n) => n.i);
	...
	const { configureGatewayForOnboarding } = await import("./onboarding.gateway-config-Crrg7w0B.js");
	...
	if (opts.skipChannels ?? opts.skipProviders) await prompter.note("Skipping channel setup.", "Channels");
		...
		const { listChannelPlugins } = await import("./plugins-CPaMeorQ.js").then((n) => n.i);
		const { setupChannels } = await import("./onboard-channels-02EDsJ--.js").then((n) => n.n);
	...
	if (opts.skipSkills) await prompter.note("Skipping skills setup.", "Skills");
		...
		const { setupSkills } = await import("./onboard-skills-DxXXGb1O.js").then((n) => n.t);
		nextConfig = await setupSkills(nextConfig, workspaceDir, runtime, prompter);
	}
	const { setupInternalHooks } = await import("./onboard-hooks-BDGxaK3B.js");
	...
	await writeConfigFile(nextConfig);
	const { finalizeOnboardingWizard } = await import("./onboarding.finalize-DN2JN7n4.js");
