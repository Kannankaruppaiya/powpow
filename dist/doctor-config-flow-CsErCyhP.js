import { f as resolveLegacyStateDirs, g as resolveStateDir, m as resolveOAuthDir, p as resolveNewStateDir, t as CONFIG_PATH } from "./paths-BJtes5yu.js";
import { g as resolveHomeDir, u as isRecord } from "./utils-BWJH7VRh.js";
import { t as formatCliCommand } from "./command-format-DRH7uGKn.js";
import { d as resolveDefaultAgentId } from "./agent-scope-Ac0Vygcn.js";
import { m as DEFAULT_ACCOUNT_ID, n as DEFAULT_MAIN_KEY, r as buildAgentMainSessionKey, s as normalizeAgentId } from "./session-key-DXmKFT2p.js";
import { Ct as resolveSlackNativeStreaming, G as getTrustedSafeBinDirs, J as normalizeTrustedSafeBinDirs, K as isTrustedSafeBinPath, St as resolveDiscordPreviewStreamMode, Tt as resolveTelegramPreviewStreamMode, d as PowPowSchema, it as resolveCommandResolutionFromArgv, n as migrateLegacyConfig, o as readConfigFileSnapshot, wt as resolveSlackStreamingMode } from "./config-B91e8u6L.js";
import { f as parseToolsBySenderTypedKey } from "./dock-DaMhmHr_.js";
import { Z as canonicalizeMainSessionAlias, p as saveSessionStore } from "./sessions-DGKLiJDU.js";
import { d as resolveTelegramAccount, l as listTelegramAccountIds } from "./plugins-CPaMeorQ.js";
...
async function autoMigrateLegacyStateDir(params) {
	...
	if ((params.env ?? process.env).POWPOW_STATE_DIR?.trim()) return {
		migrated: false,
		skipped: true,
		changes: [],
		warnings: []
	...
	try {
		...
		try {
			...
			try {
				...
				warnings.push(`Rollback failed; set POWPOW_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`);
...
function stripUnknownConfigKeys(config) {
	const parsed = PowPowSchema.safeParse(config);
...
async function maybeMigrateLegacyConfig() {
	...
	const targetDir = path.join(home, ".PowPow");
	const targetPath = path.join(targetDir, "PowPow.json");
	try {
		await fs$1.access(targetPath);
		return changes;
	} catch {}
	const legacyCandidates = [
		path.join(home, ".powpow", "powpow.json"),
...
async function loadAndMaybeMigrateDoctorConfig(params) {
	...
	if (snapshot.legacyIssues.length > 0) {
		...
		} else fixHints.push(`Run "${formatCliCommand("PowPow doctor --fix")}" to apply legacy migrations.`);
	}
	...
	if (normalized.changes.length > 0) {
		...
		else fixHints.push(`Run "${formatCliCommand("PowPow doctor --fix")}" to apply these changes.`);
	}
	...
	if (shouldRepair) {
		...
		if (hits.length > 0) note([`- Telegram allowFrom contains ${hits.length} non-numeric entries (e.g. ${hits[0]?.entry ?? "@"}); Telegram authorization requires numeric sender IDs.`, `- Run "${formatCliCommand("PowPow doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`].join("\n"), "Doctor warnings");
		const discordHits = scanDiscordNumericIdEntries(candidate);
		if (discordHits.length > 0) note([`- Discord allowlists contain ${discordHits.length} numeric entries (e.g. ${discordHits[0]?.path}=${discordHits[0]?.entry}).`, `- Discord IDs must be strings; run "${formatCliCommand("PowPow doctor --fix")}" to convert numeric IDs to quoted strings.`].join("\n"), "Doctor warnings");
		const allowFromScan = maybeRepairOpenPolicyAllowFrom(candidate);
		if (allowFromScan.changes.length > 0) note([...allowFromScan.changes, `- Run "${formatCliCommand("PowPow doctor --fix")}" to add missing allowFrom wildcards.`].join("\n"), "Doctor warnings");
		...
		if (toolsBySenderHits.length > 0) {
			...
			note([
				...
				`- Run "${formatCliCommand("PowPow doctor --fix")}" to migrate legacy keys to typed id: entries.`
			].join("\n"), "Doctor warnings");
		}
		...
		if (safeBinCoverage.length > 0) {
			...
			lines.push(`- Run "${formatCliCommand("PowPow doctor --fix")}" to scaffold missing custom safeBinProfiles entries.`);
			note(lines.join("\n"), "Doctor warnings");
		}
	...
	if (unknown.removed.length > 0) {
		...
		if (shouldRepair) {
			...
			fixHints.push("Run \"PowPow doctor --fix\" to remove these keys.");
		}
	}
