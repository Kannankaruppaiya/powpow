...
import { d as listPairingChannels, f as notifyPairingApproved, n as approveChannelPairingCode, r as listChannelPairingRequests } from "./pairing-store-BSzGxPBw.js";
import { t as formatDocsLink } from "./links-DloDafB-.js";
import { t as resolvePairingIdLabel } from "./pairing-labels-Bil8jyxp.js";
...
function registerPairingCli(program) {
	...
	pairing.command("approve").description("Approve a pairing code and allow that sender").option("--channel <channel>", `Channel (${channels.join(", ")})`).option("--account <accountId>", "Account id (for multi-account channels)").argument("<codeOrChannel>", "Pairing code (or channel when using 2 args)").argument("[code]", "Pairing code (when channel is passed as the 1st arg)").option("--notify", "Notify the requester on the same channel", false).action(async (codeOrChannel, code, opts) => {
		...
		if (!channelRaw || !resolvedCode) throw new Error(`Usage: ${formatCliCommand("PowPow pairing approve <channel> <code>")} (or: ${formatCliCommand("PowPow pairing approve --channel <channel> <code>")})`);
		if (opts.channel && code != null) throw new Error(`Too many arguments. Use: ${formatCliCommand("PowPow pairing approve --channel <channel> <code>")}`);
		const channel = parseChannel(channelRaw, channels);
		const accountId = String(opts.account ?? "").trim();
