...
import { i as loadConfig, r as createConfigIO } from "./config-B91e8u6L.js";
...
import "./dangerous-tools-yEBF5xhU.js";
import "./skill-scanner-Cs4mlgip.js";
import { a as collectIncludePathsRecursive, i as formatIcaclsResetCommand, r as createIcaclsResetCommand, t as runSecurityAudit } from "./audit-CLJkuNXV.js";
...
function registerSecurityCli(program) {
	program.command("security").description("Audit local config and state for common security foot-guns").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["PowPow security audit", "Run a local security audit."],
		["PowPow security audit --deep", "Include best-effort live Gateway probe checks."],
		["PowPow security audit --fix", "Apply safe remediations and file-permission fixes."],
		["PowPow security audit --json", "Output machine-readable JSON."]
	])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.PowPow.ai/cli/security")}\n`).command("audit").description("Audit config + local state for common security foot-guns").option("--deep", "Attempt live Gateway probe (best-effort)", false).option("--fix", "Apply safe fixes (tighten defaults + chmod state/config)", false).option("--json", "Print JSON", false).action(async (opts) => {
		...
		const lines = [];
		lines.push(heading("PowPow security audit"));
		lines.push(muted(`Summary: ${formatSummary(report.summary)}`));
		lines.push(muted(`Run deeper: ${formatCliCommand("PowPow security audit --deep")}`));
		if (opts.fix) {
			lines.push(muted(`Fix: ${formatCliCommand("PowPow security audit --fix")}`));
