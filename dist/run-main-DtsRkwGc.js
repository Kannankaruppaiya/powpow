...
import { f as defaultRuntime, r as enableConsoleCapture } from "./subsystem-DXlyH4H4.js";
import "./utils-BWJH7VRh.js";
import "./boolean-YY6K2DFz.js";
import { a as normalizeEnv, c as getCommandPath, d as getPrimaryCommand, f as getVerboseFlag, l as getFlagValue, m as hasHelpOrVersion, o as normalizeWindowsArgv, p as hasFlag, r as isTruthyEnvValue, u as getPositiveIntFlagValue } from "./entry.js";
...
import { Mt as loadDotEnv } from "./config-B91e8u6L.js";
...
import { r as formatUncaughtError } from "./errors-BpyuHard.js";
...
import { u as installUnhandledRejectionHandler } from "./runner-DY7HbTfW.js";
...
import { t as ensurePluginRegistryLoaded } from "./plugin-registry-BoiHUSmO.js";
import { t as assertSupportedRuntime } from "./runtime-guard-B4q6EKGg.js";
import { t as emitCliBanner } from "./banner-BlKZU8n7.js";
import "./doctor-config-flow-CsErCyhP.js";
import { n as ensureConfigReady } from "./config-guard-vapQ8HVU.js";
...
const routeHealth = {
	...
	run: async (argv) => {
		...
		const { healthCommand } = await import("./health-BYO_E5wI.js").then((n) => n.i);
		await healthCommand({
			json,
			timeoutMs,
			verbose
...
const routeStatus = {
	...
	run: async (argv) => {
		...
		const timeoutMs = getPositiveIntFlagValue(argv, "--timeout");
		if (timeoutMs === null) return false;
		const { statusCommand } = await import("./status-DB4bqdY_.js").then((n) => n.t);
		await statusCommand({
			json,
			deep,
			all,
...
const routeSessions = {
	...
	run: async (argv) => {
		...
		const { sessionsCommand } = await import("./sessions-WXo7REh0.js").then((n) => n.n);
...
const routeAgentsList = {
	...
	run: async (argv) => {
		...
		const { agentsListCommand } = await import("./agents-D7EXzCv5.js").then((n) => n.t);
...
const routeMemoryStatus = {
	...
	run: async (argv) => {
		...
		const { runMemoryStatus } = await import("./memory-cli-BiY99pg8.js").then((n) => n.t);
...
const routes = [
	...
	routeMemoryStatus,
	{
		...
		run: async (argv) => {
			...
			const json = hasFlag(argv, "--json");
			const { runConfigGet } = await import("./config-cli-BX4nM1P5.js");
			await runConfigGet({
				path: pathArg,
				json
			});
	...
	},
	{
		...
		run: async (argv) => {
			const pathArg = getCommandPositionals(argv)[2];
			if (!pathArg) return false;
			const { runConfigUnset } = await import("./config-cli-BX4nM1P5.js");
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsListCommand } = await import("./models-fqcihpsv.js").then((n) => n.t);
	...
	},
	{
		...
		run: async (argv) => {
			...
			const { modelsStatusCommand } = await import("./models-fqcihpsv.js").then((n) => n.t);
...
async function tryRouteCli(argv) {
	if (isTruthyEnvValue(process.env.POWPOW_DISABLE_ROUTE_FIRST)) return false;
...
async function runCli(argv = process$1.argv) {
	...
	normalizeEnv();
	if (shouldEnsureCliPath(normalizedArgv)) ensurePowPowCliOnPath();
	assertSupportedRuntime();
	if (await tryRouteCli(normalizedArgv)) return;
	enableConsoleCapture();
	const { buildProgram } = await import("./program-rJD0Gxgm.js");
	const program = buildProgram();
	installUnhandledRejectionHandler();
	process$1.on("uncaughtException", (error) => {
		console.error("[powpow] Uncaught exception:", formatUncaughtError(error));
		process$1.exit(1);
	});
	const parseArgv = rewriteUpdateFlagArgv(normalizedArgv);
	const primary = getPrimaryCommand(parseArgv);
	if (primary) {
		const { getProgramContext } = await import("./program-context-B2-baJL2.js").then((n) => n.n);
		const ctx = getProgramContext(program);
		if (ctx) {
			const { registerCoreCliByName } = await import("./command-registry-B2wrvidL.js").then((n) => n.t);
			await registerCoreCliByName(program, ctx, primary, parseArgv);
		}
		const { registerSubCliByName } = await import("./register.subclis-BvzDXCZt.js").then((n) => n.a);
	...
	if (!shouldSkipPluginCommandRegistration({
		...
		const { registerPluginCliCommands } = await import("./cli-BHf-bsBk.js");
