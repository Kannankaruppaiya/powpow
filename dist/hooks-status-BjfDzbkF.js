...
import { n as isConfigPathTruthy, r as resolveHookConfig, t as loadWorkspaceHookEntries } from "./workspace-Bl_aACEx.js";
...
function normalizeInstallOptions(entry) {
	...
	return install.map((spec, index) => {
		...
		if (!label) if (spec.kind === "bundled") label = "Bundled with PowPow";
...
function buildHookStatus(entry, config, eligibility) {
	...
	const managedByPlugin = entry.hook.source === "powpow-plugin";
