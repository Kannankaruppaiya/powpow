import { t as CONFIG_DIR, y as resolveUserPath } from "./utils-C9Yd70Dr.js";
import { i as isPathInside, n as MANIFEST_KEY, r as extensionUsesSkippedScannerPath } from "./legacy-names-SFMgq_oD.js";
import { o as loadPluginManifest } from "./manifest-registry-jY2uQzFh.js";
import { c as resolveArchiveKind, i as unscopedPackageName, n as safeDirName, o as fileExists, s as readJsonFile, t as resolveSafeInstallDir } from "./install-safe-path-DE9lNTM4.js";
...
async function ensurePowPowExtensions(manifest) {
	const extensions = manifest[MANIFEST_KEY]?.extensions;
	if (!Array.isArray(extensions)) throw new Error("package.json missing PowPow.extensions");
	const list = extensions.map((e) => typeof e === "string" ? e.trim() : "").filter(Boolean);
	if (list.length === 0) throw new Error("package.json PowPow.extensions is empty");
	return list;
}
...
async function installPluginFromPackageDir(params) {
	...
	let extensions;
	try {
		extensions = await ensurePowPowExtensions(manifest);
	} catch (err) {
		return {
			ok: false,
			error: String(err)
	...
	try {
		...
		} else if (scanSummary.warn > 0) logger.warn?.(`Plugin "${pluginId}" has ${scanSummary.warn} suspicious code pattern(s). Run "PowPow security audit --deep" for details.`);
	} catch (err) {
		logger.warn?.(`Plugin "${pluginId}" code safety scan failed (${String(err)}). Installation continues; run "PowPow security audit --deep" after install.`);
	}
