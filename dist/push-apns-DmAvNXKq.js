...
import { s as shouldLogSubsystemToConsole, t as createSubsystemLogger } from "./subsystem-DXlyH4H4.js";
import { k as isVerbose } from "./utils-BWJH7VRh.js";
import { w as parseAgentSessionKey } from "./session-key-DXmKFT2p.js";
import { p as sniffMimeFromBase64 } from "./subagent-registry-DXZuSN83.js";
import { c as readJsonFile, l as writeJsonAtomic, s as createAsyncLock } from "./pairing-token-DdQlEK2o.js";
import { n as redactSensitiveText, t as getDefaultRedactPatterns } from "./redact-CAjBvpi8.js";
import { o as estimateBase64DecodedBytes } from "./tool-images-B1e7Ffhh.js";
...
async function resolveApnsAuthConfigFromEnv(env = process.env) {
	const teamId = normalizeNonEmptyString(env.POWPOW_APNS_TEAM_ID);
	const keyId = normalizeNonEmptyString(env.POWPOW_APNS_KEY_ID);
	if (!teamId || !keyId) return {
		ok: false,
		error: "APNs auth missing: set POWPOW_APNS_TEAM_ID and POWPOW_APNS_KEY_ID"
	};
	const inlineKeyRaw = normalizeNonEmptyString(env.POWPOW_APNS_PRIVATE_KEY_P8) ?? normalizeNonEmptyString(env.POWPOW_APNS_PRIVATE_KEY);
	...
	const keyPath = normalizeNonEmptyString(env.POWPOW_APNS_PRIVATE_KEY_PATH);
	if (!keyPath) return {
		ok: false,
		error: "APNs private key missing: set POWPOW_APNS_PRIVATE_KEY_P8 or POWPOW_APNS_PRIVATE_KEY_PATH"
	};
	try {
		return {
			ok: true,
		...
		return {
			ok: false,
			error: `failed reading POWPOW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${err instanceof Error ? err.message : String(err)}`
		};
	}
}
...
function createPowPowPushMetadata(params) {
	return {
		kind: params.kind,
...
async function sendApnsAlert(params) {
	const payload = {
		...
		PowPow: createPowPowPushMetadata({
			kind: "push.test",
			nodeId: params.nodeId
		})
	};
