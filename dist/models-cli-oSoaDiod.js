import { t as CONFIG_PATH } from "./paths-Qu8BKDZT.js";
import { B as theme, R as colorize, S as shortenHomePath, z as isRich$1 } from "./utils-C9Yd70Dr.js";
import "./thinking-EAliFiVK.js";
import { Ct as formatUsageWindowSummary, Zt as describeFailoverError, a as maskApiKey, c as formatRemainingShort, f as runEmbeddedPiAgent, o as DEFAULT_OAUTH_WARN_MS, s as buildAuthHealthSummary, wt as resolveUsageProviderId, xt as loadProviderUsageSummary } from "./reply-DXKoJPvW.js";
import { A as resolveDefaultAgentWorkspaceDir, M as resolveAgentModelFallbackValues, N as resolveAgentModelPrimaryValue, P as toAgentModelListLike, a as resolveAgentDir, c as resolveAgentModelFallbacksOverride, d as resolveDefaultAgentId, s as resolveAgentExplicitModelPrimary, u as resolveAgentWorkspaceDir } from "./agent-scope-Coqt3COl.js";
import { f as defaultRuntime } from "./subsystem-A8uKC1In.js";
import "./exec-7lv95ExR.js";
import { Gt as resolveAuthProfileDisplayLabel, J as getCustomProviderApiKey, Mt as upsertAuthProfile, Pt as ensureAuthProfileStore, Q as resolveEnvApiKey, Qt as DEFAULT_PROVIDER, Rt as resolveAuthStorePathForDisplay, X as resolveApiKeyForProvider, Z as resolveAwsSdkEnvVarName, Zt as DEFAULT_MODEL, _ as resolveModelRefFromString, at as shouldEnableShellEnvFallback, bt as resolveProfileUnusableUntilForDisplay, c as modelKey, d as parseModelRef, et as getShellEnvAppliedKeys, h as resolveDefaultModelForAgent, ht as resolveAuthProfileOrder, i as findNormalizedProviderValue, jt as setAuthProfileOrder, kt as listProfilesForProvider, m as resolveConfiguredModelRef, r as buildModelAliasIndex, u as normalizeProviderId, zt as resolvePowPowAgentDir } from "./model-selection-DAT49TVG.js";
...
import { T as parseDurationMs, i as loadConfig } from "./config-ClR1jZcN.js";
...
import { t as ensurePowPowModelsJson } from "./models-config-Ds7_Vwbp.js";
import { n as discoverModels, t as discoverAuthStorage } from "./pi-model-discovery-DOnkTbEw.js";
...
import { a as resolveSessionTranscriptPath, c as resolveSessionTranscriptsDirForAgent } from "./paths-Da74Udq3.js";
...
import { r as resolveForwardCompatModel } from "./model-BF_Y_g2C.js";
import { t as ensurePiAuthJsonFromAuthProfiles } from "./model-B3KcaDtP.js";
...
import { n as runCommandWithRuntime, t as resolveOptionFromCommand } from "./cli-utils-1c5fmOOR.js";
import "./help-format-FvjEYf8n.js";
import { r as withProgressTotals } from "./progress-B2zqIvBy.js";
...
function resolveRequestedLoginProviderOrThrow(providers, rawProvider) {
	...
	throw new Error(`Unknown provider "${requested}". Loaded providers: ${availableText}. Verify plugins via \`${formatCliCommand("PowPow plugins list --json")}\`.`);
}
...
async function modelsAuthLoginCommand(opts, runtime) {
	...
	if (providers.length === 0) throw new Error(`No provider plugins found. Install one via \`${formatCliCommand("PowPow plugins install")}\`.`);
...
async function loadModelRegistry(cfg) {
	await ensurePowPowModelsJson(cfg);
	const agentDir = resolvePowPowAgentDir();
...
async function modelsListCommand(opts, runtime) {
	ensureFlagCompatibility(opts);
	const { loadConfig } = await import("./config-ClR1jZcN.js").then((n) => n.t);
	const { ensureAuthProfileStore } = await import("./model-selection-DAT49TVG.js").then((n) => n.mt);
	const cfg = loadConfig();
	...
	if (opts.all) {
		...
		if (!model) {
			const { resolveModel } = await import("./model-B3KcaDtP.js");
...
async function modelsStatusCommand(opts, runtime) {
	...
	const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolvePowPowAgentDir();
	...
	if (missingProvidersInUse.length > 0) {
		...
		for (const provider of missingProvidersInUse) {
			const hint = provider === "anthropic" ? `Run \`claude setup-token\`, then \`${formatCliCommand("PowPow models auth setup-token")}\` or \`${formatCliCommand("PowPow configure")}\`.` : `Run \`${formatCliCommand("PowPow configure")}\` or set an API key env var.`;
...
function registerModelsCli(program) {
	const models = program.command("models").description("Model discovery, scanning, and configuration").option("--status-json", "Output JSON (alias for `models status --json`)", false).option("--status-plain", "Plain output (alias for `models status --plain`)", false).option("--agent <id>", "Agent id to inspect (overrides POWPOW_AGENT_DIR/PI_CODING_AGENT_DIR)").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/models", "docs.PowPow.ai/cli/models")}\n`);
	...
	models.command("status").description("Show configured model state").option("--json", "Output JSON", false).option("--plain", "Plain output", false).option("--check", "Exit non-zero if auth is expiring/expired (1=expired/missing, 2=expiring)", false).option("--probe", "Probe configured provider auth (live)", false).option("--probe-provider <name>", "Only probe a single provider").option("--probe-profile <id>", "Only probe specific auth profile ids (repeat or comma-separated)", (value, previous) => {
		...
		next.push(value);
		return next;
	}).option("--probe-timeout <ms>", "Per-probe timeout in ms").option("--probe-concurrency <n>", "Concurrent probes").option("--probe-max-tokens <n>", "Probe max tokens (best-effort)").option("--agent <id>", "Agent id to inspect (overrides POWPOW_AGENT_DIR/PI_CODING_AGENT_DIR)").action(async (opts, command) => {
