...
import { F as resolveAgentModelPrimaryValue, P as resolveAgentModelFallbackValues } from "./agent-scope-DxSdR-i1.js";
import { M as isRecord, V as shortenHomeInString } from "./subsystem-J2JHz_PH.js";
import { C as getApiKeyForModel, T as requireApiKey, X as resolvePowPowAgentDir, _ as normalizeProviders, b as resolveImplicitProviders, et as normalizeSecretInput, v as resolveImplicitBedrockProvider, y as resolveImplicitCopilotProvider } from "./model-selection-BIieOTxJ.js";
import { n as discoverModels, t as discoverAuthStorage } from "./pi-model-discovery-DQnj7CO7.js";
import { P as sanitizeUserFacingText } from "./pi-embedded-helpers-BtfCBOnO.js";
import { n as loadConfig } from "./config-ctWck2-F.js";
import { n as redactToolDetail } from "./redact-DsIiPP8n.js";
...
async function ensurePowPowModelsJson(config, agentDirOverride) {
	const cfg = config ?? loadConfig();
	const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolvePowPowAgentDir();
...
function summarizeKnownExec(words) {
	...
	if (bin === "PowPow") {
		...
		return sub ? `run PowPow ${sub}` : "run PowPow";
	}
...
function resolveToolDisplay(params) {
	...
	const fallbackVerb = key === "web_search" ? "search" : key === "web_fetch" ? "fetch" : key.replace(/_/g, " ").replace(/\./g, " ");
	const verb = normalizeVerb(actionSpec?.label ?? action ?? fallbackVerb);
	let detail;
	if (key === "exec") detail = resolveExecDetail(params.args);
	if (!detail && key === "read") detail = resolveReadDetail(params.args);
	if (!detail && (key === "write" || key === "edit" || key === "attach")) detail = resolveWriteDetail(key, params.args);
	if (!detail && key === "web_search") detail = resolveWebSearchDetail(params.args);
	if (!detail && key === "web_fetch") detail = resolveWebFetchDetail(params.args);
...
function formatToolDetail(display) {
	const detailRaw = display.detail ? redactToolDetail(display.detail) : void 0;
	if (!detailRaw) return;
	if (detailRaw.includes(" · ")) {
		const compact = detailRaw.split(" · ").map((part) => part.trim()).filter((part) => part.length > 0).join(", ");
		return compact ? `with ${compact}` : void 0;
	}
	return detailRaw;
}
...
async function describeImageWithModel(params) {
	await ensurePowPowModelsJson(params.cfg, params.agentDir);
...
//#endregion
export { extractTextFromChatContent as C, isInsideCode as S, stripMinimaxToolCallXml as _, decodeDataUrl as a, stripReasoningTagsFromText as b, extractAssistantText as c, extractThinkingFromTaggedText as d, formatReasoningMessage as f, stripDowngradedToolCallText as g, promoteThinkingTagsToBlocks as h, coerceImageModelConfig as i, extractAssistantThinking as l, isAssistantMessage as m, image_exports as n, resolveProviderVisionModelFromConfig as o, inferToolMetaFromArgs as p, coerceImageAssistantText as r, minimaxUnderstandImage as s, describeImageWithModel as t, extractThinkingFromTaggedStream as u, stripThinkingTagsFromText as v, ensurePowPowModelsJson as w, findCodeRegions as x, resolveToolDisplay as y };