import { d as resolveIsNixMode, g as resolveStateDir, m as resolveOAuthDir, t as CONFIG_PATH, u as resolveGatewayPort, y as resolveRequiredHomeDir } from "./paths-BJtes5yu.js";
import { f as defaultRuntime } from "./subsystem-DXlyH4H4.js";
import { H as getResolvedLoggerSettings, S as sleep, v as resolveUserPath, x as shortenHomePath } from "./utils-BWJH7VRh.js";
import { r as isTruthyEnvValue } from "./entry.js";
import { C as updateAuthProfileStoreWithLock, G as resolveHooksGmailModel, Jt as DEFAULT_MODEL, P as getModelRefStatus, Qt as CODEX_CLI_PROFILE_ID, Tt as resolveApiKeyForProvider, U as resolveConfiguredModelRef, Yt as DEFAULT_PROVIDER, Zt as CLAUDE_CLI_PROFILE_ID, l as resolveApiKeyForProfile, m as repairOAuthProfileIdMismatch, s as resolveProfileUnusableUntilForDisplay, x as ensureAuthProfileStore } from "./auth-profiles-KPMal-k-.js";
import { t as formatCliCommand } from "./command-format-DRH7uGKn.js";
import { a as resolveAgentDir, d as resolveDefaultAgentId, g as DEFAULT_AGENTS_FILENAME, u as resolveAgentWorkspaceDir } from "./agent-scope-Ac0Vygcn.js";
import { t as resolvePowPowPackageRoot } from "./powpow-root-DNPuMfcb.js";
import { n as runExec, t as runCommandWithTimeout } from "./exec-Du-v-jcI.js";
import { t as buildWorkspaceSkillStatus } from "./skills-status-TqUhMg2L.js";
import { l as writeConfigFile, o as readConfigFileSnapshot } from "./config-B91e8u6L.js";
import { B as cleanStaleLockFiles, at as isPrimarySessionTranscriptFileName, c as loadSessionStore, et as resolveMainSessionKey, it as formatSessionArchiveTimestamp } from "./sessions-DGKLiJDU.js";
import { n as listChannelPlugins } from "./plugins-CPaMeorQ.js";
import { $t as formatRemainingShort, Qt as buildAuthHealthSummary, Zt as DEFAULT_OAUTH_WARN_MS, _ as loadPowPowPlugins } from "./subagent-registry-DXZuSN83.js";
import { c as resolveSessionTranscriptsDirForAgent, i as resolveSessionFilePathOptions, l as resolveStorePath, r as resolveSessionFilePath } from "./paths-BB4B9o0x.js";
import { n as callGateway, t as buildGatewayConnectionDetails } from "./call-Dy4jPY6_.js";
import { r as isLoopbackHost, u as resolveGatewayBindHost } from "./net-CliLiAjH.js";
import { D as DEFAULT_SANDBOX_COMMON_IMAGE, E as DEFAULT_SANDBOX_BROWSER_IMAGE, O as DEFAULT_SANDBOX_IMAGE, p as resolveSandboxScope } from "./sandbox-Vr4z2IvO.js";
import { a as resolveGatewayAuth } from "./auth-BQr6HjtW.js";
import { a as formatPortDiagnostics, n as inspectPortUsage } from "./ports-61DuVTUr.js";
import { i as resolveMemoryBackendConfig } from "./memory-cli-BiY99pg8.js";
...
import { f as resolveGatewaySystemdServiceName, h as resolveNodeLaunchAgentLabel, l as resolveGatewayLaunchAgentLabel, p as resolveGatewayWindowsTaskName } from "./constants-Clw8zvXe.js";
import { r as isWSLEnv, t as isWSL } from "./wsl-CvQfS6aU.js";
import { s as resolveDmAllowState } from "./pi-tools.policy-BVP5EZDM.js";
import { h as randomToken, n as applyWizardMetadata, p as printWizardHeader, s as guardCancel } from "./onboard-helpers-BNEaJ_0q.js";
import { n as stylePromptMessage, r as stylePromptTitle, t as stylePromptHint } from "./prompt-style-BzZR3NGi.js";
import { t as note$1 } from "./note-0Tgggy8C.js";
import { t as collectChannelStatusIssues } from "./channels-status-issues-B_hfqcmH.js";
import { d as resolveSystemNodeInfo, l as renderSystemNodeWarning, n as gatewayInstallErrorHint, t as buildGatewayInstallPlan } from "./daemon-install-helpers-COWxlOD2.js";
import { n as GATEWAY_DAEMON_RUNTIME_OPTIONS, t as DEFAULT_GATEWAY_DAEMON_RUNTIME } from "./daemon-runtime-BXbwrY5G.js";
import { l as uninstallLegacySystemdUnits, r as isSystemdUserServiceAvailable } from "./systemd-BBp3JvkT.js";
import { a as launchAgentPlistExists, i as isLaunchAgentLoaded, o as repairLaunchAgentBootstrap, r as isLaunchAgentListed, s as resolveGatewayLogPaths, t as resolveGatewayService } from "./service-DkDZ7shP.js";
import { i as auditGatewayServiceConfig, n as renderSystemdUnavailableHints, o as needsNodeRuntimeMigration, r as SERVICE_AUDIT_CODES, s as formatRuntimeStatus, t as isSystemdUnavailableDetail } from "./systemd-hints-Cfyn9qjl.js";
import { t as readLastGatewayErrorLine } from "./diagnostics-q5_bR3Ju.js";
import { n as renderGatewayServiceCleanupHints, t as findExtraGatewayServices } from "./inspect-XZ81In3N.js";
import { r as healthCommand } from "./health-BYO_E5wI.js";
import { n as resolveControlUiDistIndexHealth, r as resolveControlUiDistIndexPathForRoot } from "./control-ui-assets-B_c5GFvQ.js";
import { t as resolveDefaultChannelAccountContext } from "./channel-account-context-23ILPOVs.js";
import { n as runGatewayUpdate, t as resolveAgentSessionDirs } from "./session-dirs-D4HEmQCv.js";
import { n as logConfigUpdated } from "./logging-x1Y0rmXr.js";
import { n as detectLegacyStateMigrations, r as runLegacyStateMigrations, t as loadAndMaybeMigrateDoctorConfig } from "./doctor-config-flow-CsErCyhP.js";
import { t as ensureSystemdUserLingerInteractive } from "./systemd-linger-DSbbv8-4.js";
import { t as formatHealthCheckFailure } from "./health-format-BP7yY5At.js";
import { n as doctorShellCompletion } from "./doctor-completion-DW6-o8oI.js";
...
async function maybeRemoveDeprecatedCliAuthProfiles(cfg, prompter) {
	...
	if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) lines.push(`- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("PowPow models auth setup-token")}`);
	if (deprecated.has(CODEX_CLI_PROFILE_ID)) lines.push(`- ${CODEX_CLI_PROFILE_ID} (OpenAI Codex): use OAuth → ${formatCliCommand("PowPow models auth login --provider openai-codex")}`);
	note$1(lines.join("\n"), "Auth profiles");
...
function formatAuthIssueHint(issue) {
	if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) return `Deprecated profile. Use ${formatCliCommand("PowPow models auth setup-token")} or ${formatCliCommand("PowPow configure")}.`;
	if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) return `Deprecated profile. Use ${formatCliCommand("PowPow models auth login --provider openai-codex")} or ${formatCliCommand("PowPow configure")}.`;
	return `Re-auth via \`${formatCliCommand("PowPow configure")}\` or \`${formatCliCommand("PowPow onboard")}\`.`;
}
...
function buildGatewayRuntimeHints(runtime, options = {}) {
	...
	if (runtime.cachedLabel && platform === "darwin") {
		const label = resolveGatewayLaunchAgentLabel(env.POWPOW_PROFILE);
		hints.push(`LaunchAgent label cached but plist missing. Clear with: launchctl bootout gui/$UID/${label}`);
		hints.push(`Then reinstall: ${formatCliCommand("PowPow gateway install", env)}`);
	}
	if (runtime.missingUnit) {
		hints.push(`Service not installed. Run: ${formatCliCommand("PowPow gateway install", env)}`);
		if (fileLog) hints.push(`File logs: ${fileLog}`);
		return hints;
	}
	if (runtime.status === "stopped") {
		...
		if (platform === "darwin") {
			...
			const unit = resolveGatewaySystemdServiceName(env.POWPOW_PROFILE);
			hints.push(`Logs: journalctl --user -u ${unit}.service -n 200 --no-pager`);
		} else if (platform === "win32") {
			const task = resolveGatewayWindowsTaskName(env.POWPOW_PROFILE);
...
async function maybeRepairGatewayDaemon(params) {
		...
		if (params.cfg.gateway?.mode !== "remote") {
			if (await params.prompter.confirmSkipInNonInteractive({
				...
				const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
					env: process.env,
					port,
					token: params.cfg.gateway?.auth?.token ?? process.env.POWPOW_GATEWAY_TOKEN,
	...
	if (process.platform === "darwin") {
		const label = resolveGatewayLaunchAgentLabel(process.env.POWPOW_PROFILE);
		note$1(`LaunchAgent loaded; stopping requires "${formatCliCommand("PowPow gateway stop")}" or launchctl bootout gui/$UID/${label}.`, "Gateway");
	}
	if (serviceRuntime?.status === "running") {
		if (await params.prompter.confirmSkipInNonInteractive({
...
function resolveGatewayAuthToken(cfg, env) {
	...
	return (env.POWPOW_GATEWAY_TOKEN ?? env.powpow_GATEWAY_TOKEN)?.trim() || void 0;
}
...
async function maybeScanExtraGatewayServices(options, runtime, prompter) {
	...
	if (legacyServices.length > 0) {
		if (await prompter.confirmSkipInNonInteractive({
			message: "Remove legacy gateway services (powpow/moltbot) now?",
			initialValue: true
		})) {
			const removed = [];
			...
			if (removed.length > 0) runtime.log("Legacy gateway services removed. Installing PowPow gateway next.");
		}
	}
...
* Runs as part of `PowPow doctor` — config-only, no network calls.
*/
	...
	if (resolved.provider !== "auto") {
		if (resolved.provider === "local") {
			...
			note$1([
				...
				`- Switch to a remote provider: ${formatCliCommand("PowPow config set agents.defaults.memorySearch.provider openai")}`,
				"",
				`Verify: ${formatCliCommand("PowPow memory status --deep")}`
			].join("\n"), "Memory search");
			return;
		}
		...
		note$1([
			...
			`- Configure credentials: ${formatCliCommand("PowPow configure --section model")}`,
			`- To disable: ${formatCliCommand("PowPow config set agents.defaults.memorySearch.enabled false")}`,
			"",
			`Verify: ${formatCliCommand("PowPow memory status --deep")}`
		].join("\n"), "Memory search");
		return;
...
async function noteMacLaunchAgentOverrides() {
	...
	const markerPath = [path.join(home, ".PowPow", "disable-launchagent")].find((candidate) => fs.existsSync(candidate));
...
async function noteMacLaunchctlGatewayEnvOverrides(cfg, deps) {
	...
	const deprecatedLaunchctlEntries = [["powpow_GATEWAY_TOKEN", await getenv("powpow_GATEWAY_TOKEN")], ["powpow_GATEWAY_PASSWORD", await getenv("powpow_GATEWAY_PASSWORD")]].filter((entry) => Boolean(entry[1]?.trim()));
	if (deprecatedLaunchctlEntries.length > 0) {
		const lines = ["- Deprecated launchctl environment variables detected (ignored).", ...deprecatedLaunchctlEntries.map(([key]) => `- \`${key}\` is set; use \`POWPOW_${key.slice(key.indexOf("_") + 1)}\` instead.`)];
		(deps?.noteFn ?? note$1)(lines.join("\n"), "Gateway (macOS)");
	}
	const tokenEntries = [["POWPOW_GATEWAY_TOKEN", await getenv("POWPOW_GATEWAY_TOKEN")]];
	const passwordEntries = [["POWPOW_GATEWAY_PASSWORD", await getenv("POWPOW_GATEWAY_PASSWORD")]];
	...
	const lines = [
		...
		envPassword ? `- \`${envPasswordKey ?? "POWPOW_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.` : void 0,
...
function noteDeprecatedLegacyEnvVars(env = process.env, deps) {
	const entries = Object.entries(env).filter(([key, value]) => key.startsWith("powpow_") && value?.trim()).map(([key]) => key);
	if (entries.length === 0) return;
	const lines = [
		"- Deprecated legacy environment variables detected (ignored).",
		"- Use POWPOW_* equivalents instead:",
		...entries.map((key) => {
			return `  ${key} -> POWPOW_${key.slice(key.indexOf("_") + 1)}`;
		})
	];
	(deps?.noteFn ?? note$1)(lines.join("\n"), "Environment");
}
...
async function maybeRepairSandboxImages(cfg, runtime, prompter) {
	...
	if (!await isDockerAvailable()) {
		note$1([
			...
			"- Disable sandbox mode: PowPow config set agents.defaults.sandbox.mode off"
		].join("\n"), "Sandbox");
		return cfg;
	}
	let next = cfg;
...
async function noteSecurityWarnings(cfg) {
	const warnings = [];
	const auditHint = `- Run: ${formatCliCommand("PowPow security audit --deep")}`;
	if (cfg.approvals?.exec?.enabled === false) warnings.push("- Note: approvals.exec.enabled=false disables approval forwarding only.", "  Host exec gating still comes from ~/.PowPow/exec-approvals.json.", `  Check local policy with: ${formatCliCommand("PowPow approvals get --gateway")}`);
	...
	const saferRemoteAccessLines = [
		...
		"  Docs: https://docs.PowPow.ai/gateway/remote"
	];
	if (isExposed) if (!hasSharedSecret) {
		const authFixLines = resolvedAuth.mode === "password" ? [`  Fix: ${formatCliCommand("PowPow configure")} to set a password`, `  Or switch to token: ${formatCliCommand("PowPow config set gateway.auth.mode token")}`] : [`  Fix: ${formatCliCommand("PowPow doctor --fix")} to generate a token`, `  Or set token directly: ${formatCliCommand("PowPow config set gateway.auth.mode token")}`];
		warnings.push(`- CRITICAL: Gateway bound to ${bindDescriptor} without authentication.`, `  Anyone on your network (or internet if port-forwarded) can fully control your agent.`, `  Fix: ${formatCliCommand("PowPow config set gateway.bind loopback")}`, ...saferRemoteAccessLines, ...authFixLines);
	...
	const warnDmPolicy = async (params) => {
		...
		if (dmScope === "main" && isMultiUserDm) warnings.push(`- ${params.label} DMs: multiple senders share the main session; run: ` + formatCliCommand("PowPow config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate sessions.");
	};
...
async function noteSessionLockHealth(params) {
	...
	if (staleCount > 0 && !shouldRepair) {
		...
		lines.push("- Run \"PowPow doctor --fix\" to remove stale lock files automatically.");
	}
...
function findOtherStateDirs(stateDir) {
	...
	for (const root of roots) {
		...
		for (const entry of entries) {
			...
			const candidates = [".PowPow"].map((dir) => path.resolve(root, entry.name, dir));
...
function shouldRequireOAuthDir(cfg, env) {
	if (env.POWPOW_OAUTH_DIR?.trim()) return true;
...
async function noteStateIntegrity(cfg, prompter, configPath) {
	...
	const defaultStateDir = path.join(homedir(), ".PowPow");
	...
	if (entries.length > 0) {
		...
		if (missing.length > 0) warnings.push([
			...
			`  Verify sessions in store: ${formatCliCommand(`PowPow sessions --store "${absoluteStorePath}"`)}`,
			`  Preview cleanup impact: ${formatCliCommand(`PowPow sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`
		].join("\n"));
...
function noteWorkspaceBackupTip(workspaceDir) {
	...
	note$1([
		...
		"- Keep ~/.PowPow out of git; it contains credentials and session history.",
...
async function detectPowPowGitCheckout(root) {
...
async function maybeOfferUpdateBeforeDoctor(params) {
	if (!(!isTruthyEnvValue(process.env.POWPOW_UPDATE_IN_PROGRESS) && params.options.nonInteractive !== true && params.options.yes !== true && params.options.repair !== true && Boolean(process.stdin.isTTY)) || !params.root) return { updated: false };
	const git = await detectPowPowGitCheckout(params.root);
	if (git === "git") {
		if (!await params.confirm({
			message: "Update PowPow from git before running doctor?",
			initialValue: true
	...
	if (git === "not-git") note$1(["This install is not a git checkout.", `Run \`${formatCliCommand("PowPow update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`].join("\n"), "Update");
	return { updated: false };
}
...
const MEMORY_SYSTEM_PROMPT = [
	...
	"https://github.com/PowPow/PowPow/commit/9ffea23f31ca1df5183b25668f8f814bee0fb34e",
	"https://github.com/PowPow/PowPow/commit/7d1fee70e76f2f634f1b41fca927ee663914183a"
].join("\n");
...
async function doctorCommand(runtime = defaultRuntime, options = {}) {
	...
	if (!cfg.gateway?.mode) {
		const lines = [
			...
			`Fix: run ${formatCliCommand("PowPow configure")} and set Gateway mode (local/remote).`,
			`Or set directly: ${formatCliCommand("PowPow config set gateway.mode local")}`
		];
		if (!fs.existsSync(configPath)) lines.push(`Missing config: run ${formatCliCommand("PowPow setup")} first.`);
		note$1(lines.join("\n"), "Gateway");
	}
	...
	} else if (!prompter.shouldRepair) runtime.log(`Run "${formatCliCommand("PowPow doctor --fix")}" to apply changes.`);
