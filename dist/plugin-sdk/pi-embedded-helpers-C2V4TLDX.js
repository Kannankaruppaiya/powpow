import { l as resolveAgentIdFromSessionKey, s as normalizeAgentId } from "./session-key-C7qWg3SH.js";
import { i as resolveGatewayPort, t as STATE_DIR } from "./paths-Biw6RtsH.js";
import { H as resolveUserPath, Y as truncateUtf16Safe, c as CHANNEL_IDS, i as defaultRuntime, t as createSubsystemLogger } from "./subsystem-BBPkQl8F.js";
import { n as loadConfig, o as writeConfigFile, t as createConfigIO, z as getBlockedNetworkModeReason } from "./config-Beao1DYH.js";
import { i as parseBooleanValue, t as formatCliCommand } from "./command-format-KJDDAM2E.js";
import { D as runExec, _ as DEFAULT_IDENTITY_FILENAME, b as DEFAULT_USER_FILENAME, d as resolveSessionAgentId, g as DEFAULT_HEARTBEAT_FILENAME, h as DEFAULT_BOOTSTRAP_FILENAME, m as DEFAULT_AGENT_WORKSPACE_DIR, p as DEFAULT_AGENTS_FILENAME, r as resolveAgentConfig, v as DEFAULT_SOUL_FILENAME, x as ensureAgentWorkspace, y as DEFAULT_TOOLS_FILENAME } from "./agent-scope-D2mtbkzb.js";
import { n as formatErrorMessage, t as extractErrorCode } from "./errors-BjJdLum7.js";
import { i as getImageMetadata, n as buildImageResizeSideGrid, s as resizeToJpeg, t as IMAGE_REDUCE_QUALITY_STEPS } from "./image-ops-B6WDOaEz.js";
import { t as SsrFBlockedError } from "./ssrf-D7nGqg1W.js";
import { t as sanitizeContentBlocksImages } from "./tool-images-C-3xgF0t.js";
import { _ as safeEqualSecret, a as syncSkillsToWorkspace, f as PATH_ALIAS_POLICIES, g as sanitizeEnvVars, l as resolveSandboxInputPath, p as assertNoPathAliasEscape, u as resolveSandboxPath } from "./skills-BYiSApWK.js";
import { A as DEFAULT_AI_SNAPSHOT_EFFICIENT_DEPTH, C as stopChromeExtensionRelayServer, D as resolveExistingPathsWithinRoot, E as DEFAULT_UPLOAD_DIR, F as DEFAULT_POWPOW_BROWSER_COLOR, I as DEFAULT_POWPOW_BROWSER_ENABLED, L as DEFAULT_POWPOW_BROWSER_PROFILE_NAME, M as DEFAULT_AI_SNAPSHOT_MAX_CHARS, N as DEFAULT_BROWSER_DEFAULT_PROFILE_NAME, P as DEFAULT_BROWSER_EVALUATE_ENABLED, R as isLoopbackHost, S as ensureChromeExtensionRelayServer, T as DEFAULT_TRACE_DIR, _ as appendCdpPath, a as resolvePowPowUserDataDir, c as captureScreenshot, d as normalizeCdpWsUrl, f as snapshotAria, g as withBrowserNavigationPolicy, h as assertBrowserNavigationResultAllowed, i as launchPowPowChrome, j as DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS, k as resolveWritablePathWithinRoot, l as createTargetViaCdp, m as assertBrowserNavigationAllowed, n as isChromeCdpReady, o as stopPowPowChrome, p as InvalidBrowserNavigationUrlError, r as isChromeReachable, s as resolveBrowserExecutableForPlatform, v as fetchJson, w as DEFAULT_DOWNLOAD_DIR, y as fetchOk } from "./chrome-BCWo81Ce.js";
import { A as acquireSessionWriteLock, H as resolveAgentMainSessionKey, V as canonicalizeMainSessionAlias } from "./sessions-XJKwRHd-.js";
import { i as saveMediaBuffer, t as ensureMediaDir } from "./store-CAyXMlC1.js";
import { o as normalizeThinkLevel } from "./thinking-BFsWfunr.js";
import { t as generateSecureToken } from "./secure-random-C2TggqdC.js";
...
function readGatewayTokenEnv(env, includeLegacyEnv) {
	const primary = trimToUndefined(env.POWPOW_GATEWAY_TOKEN);
	if (primary) return primary;
	if (!includeLegacyEnv) return;
	return trimToUndefined(env.powpow_GATEWAY_TOKEN);
}
function readGatewayPasswordEnv(env, includeLegacyEnv) {
	const primary = trimToUndefined(env.POWPOW_GATEWAY_PASSWORD);
	if (primary) return primary;
	if (!includeLegacyEnv) return;
	return trimToUndefined(env.powpow_GATEWAY_PASSWORD);
}
...
const DEFAULT_SANDBOX_IMAGE = "powpow-sandbox:bookworm-slim";
const DEFAULT_SANDBOX_CONTAINER_PREFIX = "powpow-sbx-";
...
const DEFAULT_SANDBOX_BROWSER_IMAGE = "powpow-sandbox-browser:bookworm-slim";
const SANDBOX_BROWSER_SECURITY_HASH_EPOCH = "2026-02-21-novnc-auth-default";
const DEFAULT_SANDBOX_BROWSER_PREFIX = "powpow-sbx-browser-";
const DEFAULT_SANDBOX_BROWSER_NETWORK = "powpow-sandbox-browser";
...
function buildCoreToolGroupMap() {
	...
	const PowPowTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInPowPowGroup).map((tool) => tool.id);
	return {
		"group:PowPow": PowPowTools,
...
async function loadPwAiModule(mode) {
	try {
		return await import("./pw-ai-DsqL12fy.js");
	} catch (err) {
			...
			run: async ({ cdpUrl, tab, pw }) => {
				...
				switch (kind) {
					...
					case "wait": {
						...
						if (fn && !evaluateEnabled) return jsonError(res, 403, ["wait --fn is disabled by config (browser.evaluateEnabled=false).", "Docs: /gateway/configuration#browser-powpow-managed-browser"].join("\n"));
					...
					case "evaluate": {
						if (!evaluateEnabled) return jsonError(res, 403, ["act:evaluate is disabled by config (browser.evaluateEnabled=false).", "Docs: /gateway/configuration#browser-powpow-managed-browser"].join("\n"));
...
function normalizeHexColor(raw) {
	...
	if (!value) return DEFAULT_POWPOW_BROWSER_COLOR;
	const normalized = value.startsWith("#") ? value : `#${value}`;
	if (!/^#[0-9a-fA-F]{6}$/.test(normalized)) return DEFAULT_POWPOW_BROWSER_COLOR;
...
/**
* Ensure the default "PowPow" profile exists in the profiles map.
...
function ensureDefaultProfile(profiles, defaultColor, legacyCdpPort, derivedDefaultCdpPort) {
	...
	if (!result[DEFAULT_POWPOW_BROWSER_PROFILE_NAME]) result[DEFAULT_POWPOW_BROWSER_PROFILE_NAME] = {
...
*
* Note: this is an PowPow browser profile (routing config), not a Chrome user profile.
...
function resolveBrowserConfig(cfg, rootConfig) {
	const enabled = cfg?.enabled ?? DEFAULT_POWPOW_BROWSER_ENABLED;
	...
	const defaultProfile = defaultProfileFromConfig ?? (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME] ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME : DEFAULT_POWPOW_BROWSER_PROFILE_NAME);
...
function resolveProfile(resolved, profileName) {
	...
	let cdpUrl = "";
	const driver = profile.driver === "extension" ? "extension" : "PowPow";
	if (rawProfileUrl) {
...
function createBrowserProfilesService(ctx) {
	...
	const deleteProfile = async (nameRaw) => {
		...
		if (resolveProfile(state.resolved, name)?.cdpIsLoopback) {
			...
			const userDataDir = resolvePowPowUserDataDir(name);
...
function createProfileContext(opts, profile) {
	...
	const ensureBrowserAvailable = async () => {
		...
		if (isExtension) {
			...
			throw new Error(`Chrome extension relay is running, but no tab is connected. Click the PowPow Chrome extension icon on a tab to attach it (profile "${profile.name}").`);
		}
		if (!httpReachable) {
			...
			attachRunning(await launchPowPowChrome(current.resolved, profile));
			return;
		}
		if (await isReachable()) return;
		if (!profileState.running) throw new Error(`Port ${profile.cdpPort} is in use for profile "${profile.name}" but not by PowPow. Run action=reset-profile profile=${profile.name} to kill the process.`);
		...
		await stopPowPowChrome(profileState.running);
	...
	const ensureTabAvailable = async (targetId) => {
		...
		if ((await listTabs()).length === 0) {
			if (profile.driver === "extension") throw new Error(`tab not found (no attached Chrome tabs for profile "${profile.name}"). Click the PowPow Browser Relay toolbar icon on the tab you want to control (badge ON).`);
	...
	const resetProfile = async () => {
		...
		const userDataDir = resolvePowPowUserDataDir(profile.name);
		const profileState = getProfileState();
		if (await isHttpReachable(300) && !profileState.running) try {
			await (await import("./pw-ai-DsqL12fy.js")).closePlaywrightBrowserConnection();
		} catch {}
...
function isAuthorizedBrowserRequest(req, auth) {
	...
	if (auth.password) {
		const passwordHeader = firstHeaderValue(req.headers["x-powpow-password"]).trim();
...
function formatBindBlockedError(params) {
	...
	if (params.reason.kind === "reserved_target") return /* @__PURE__ */ new Error(`Sandbox security: bind mount "${params.bind}" targets reserved container path "${params.reason.reservedPath}" (resolved target: "${params.reason.targetPath}"). This can shadow PowPow sandbox mounts. Use a dangerous override only when you fully trust this runtime.`);
...
function buildSandboxCreateArgs(params) {
	...
	args.push("--label", "PowPow.sandbox=1");
	args.push("--label", `PowPow.sessionKey=${params.scopeKey}`);
	args.push("--label", `PowPow.createdAtMs=${createdAtMs}`);
	if (params.configHash) args.push("--label", `PowPow.configHash=${params.configHash}`);
...
async function readContainerConfigHash(containerName) {
	return await readDockerContainerLabel(containerName, "PowPow.configHash");
}
function formatSandboxRecreateHint(params) {
	if (params.scope === "session") return formatCliCommand(`PowPow sandbox recreate --session ${params.sessionKey}`);
	if (params.scope === "agent") return formatCliCommand(`PowPow sandbox recreate --agent ${resolveSandboxAgentId(params.sessionKey) ?? "main"}`);
	return formatCliCommand("PowPow sandbox recreate --all");
}
...
const NOVNC_PASSWORD_ENV_KEY = "POWPOW_BROWSER_NOVNC_PASSWORD";
const NOVNC_TOKEN_TTL_MS = 300 * 1e3;
...
function buildNoVncDirectUrl(port, password) {
	const query = new URLSearchParams({
		autoconnect: "1",
		resize: "remote"
	});
	if (password?.trim()) query.set("password", password);
...
const CDP_SOURCE_RANGE_ENV_KEY = "POWPOW_BROWSER_CDP_SOURCE_RANGE";
...
function buildSandboxBrowserResolvedConfig(params) {
	return {
		...
		color: DEFAULT_POWPOW_BROWSER_COLOR,
		executablePath: void 0,
		headless: params.headless,
		noSandbox: false,
		attachOnly: true,
		defaultProfile: DEFAULT_POWPOW_BROWSER_PROFILE_NAME,
		extraArgs: [],
		profiles: { [DEFAULT_POWPOW_BROWSER_PROFILE_NAME]: {
...
async function ensureSandboxBrowser(params) {
	...
	if (hasContainer) {
		...
		currentHash = await readDockerContainerLabel(containerName, "PowPow.configHash");
		hashMismatch = !currentHash || currentHash !== expectedHash;
		if (!currentHash) {
		...
		if (hashMismatch) {
			...
			if (running && (typeof lastUsedAtMs !== "number" || now - lastUsedAtMs < HOT_BROWSER_WINDOW_MS)) {
				const hint = (() => {
					if (params.cfg.scope === "session") return `PowPow sandbox recreate --browser --session ${params.scopeKey}`;
					if (params.cfg.scope === "agent") return `PowPow sandbox recreate --browser --agent ${resolveSandboxAgentId(params.scopeKey) ?? "main"}`;
					return "PowPow sandbox recreate --browser --all";
				})();
	...
	if (!hasContainer) {
		...
		const args = buildSandboxCreateArgs({
			...
			labels: {
				"PowPow.sandboxBrowser": "1",
				"PowPow.browserConfigEpoch": SANDBOX_BROWSER_SECURITY_HASH_EPOCH
			},
			configHash: expectedHash,
			includeBinds: false,
		...
		args.push("-e", `POWPOW_BROWSER_HEADLESS=${params.cfg.browser.headless ? "1" : "0"}`);
		args.push("-e", `POWPOW_BROWSER_ENABLE_NOVNC=${params.cfg.browser.enableNoVnc ? "1" : "0"}`);
		args.push("-e", `POWPOW_BROWSER_CDP_PORT=${params.cfg.browser.cdpPort}`);
		if (cdpSourceRange) args.push("-e", `${CDP_SOURCE_RANGE_ENV_KEY}=${cdpSourceRange}`);
		args.push("-e", `POWPOW_BROWSER_VNC_PORT=${params.cfg.browser.vncPort}`);
		args.push("-e", `POWPOW_BROWSER_NOVNC_PORT=${params.cfg.browser.noVncPort}`);
	...
	const existingProfile = existing ? resolveProfile(existing.bridge.state.resolved, DEFAULT_POWPOW_BROWSER_PROFILE_NAME) : null;
	...
	const noVncUrl = mappedNoVnc && noVncEnabled ? (() => {
		const token = issueNoVncObserverToken({ url: buildNoVncDirectUrl(mappedNoVnc, noVncPassword) });
...
function formatSandboxToolPolicyBlockedMessage(params) {
	...
	lines.push(`- See: ${formatCliCommand(`PowPow sandbox explain --session ${runtime.sessionKey}`)}`);
	return lines.join("\n");
}
...
function isContextOverflowError(errorMessage) {
	...
	if (lower.includes("tpm") || lower.includes("tokens per minute")) return false;
...
*
* PowPow persists provider-specific reasoning metadata in `thinkingSignature`; if that metadata
...
//#endregion
export { normalizeToolName as $, isTimeoutErrorMessage as A, resolveBrowserConfig as B, isCompactionFailureError as C, isLikelyContextOverflowError as D, isFailoverErrorMessage as E, ensureSandboxWorkspaceForSession as F, resolveSandboxConfigForAgent as G, getBridgeAuthForPort as H, resolveSandboxContext as I, collectExplicitAllowlist as J, applyOwnerOnlyToolPolicy as K, resolveSandboxRuntimeStatus as L, parseImageDimensionError as M, parseImageSizeError as N, isRateLimitAssistantError as O, sanitizeUserFacingText as P, expandToolGroups as Q, createBrowserRouteContext as R, isCloudCodeAssistFormatError as S, isFailoverAssistantError as T, ensureBrowserControlAuth as U, resolveProfile as V, resolveBrowserControlAuth as W, mergeAlsoAllowPolicy as X, expandPolicyWithPluginGroups as Y, stripPluginOnlyAllowlist as Z, formatRawAssistantErrorForUi as _, isMessagingToolDuplicateNormalized as a, resolveBootstrapMaxChars as at, isAuthPermanentErrorMessage as b, extractToolCallsFromAssistant as c, resolveGatewayCredentialsFromConfig as ct, downgradeOpenAIReasoningBlocks as d, resolveToolProfilePolicy as et, isGoogleModelApi as f, formatBillingErrorMessage as g, formatAssistantErrorText as h, isMessagingToolDuplicate as i, ensureSessionHeader as it, isTransientHttpError as j, isRawApiErrorPayload as k, extractToolResultId as l, trimToUndefined as lt, classifyFailoverReason as m, validateGeminiTurns as n, matchesAnyGlobPattern as nt, normalizeTextForComparison as o, resolveBootstrapTotalMaxChars as ot, BILLING_ERROR_USER_MESSAGE as p, buildPluginToolGroups as q, pickFallbackThinkingLevel as r, buildBootstrapContextFiles as rt, sanitizeSessionMessagesImages as s, sanitizeGoogleTurnOrdering as st, validateAnthropicTurns as t, compileGlobPatterns as tt, sanitizeToolCallIdsForCloudCodeAssist as u, getApiErrorPayloadFingerprint as v, isContextOverflowError as w, isBillingAssistantError as x, isAuthAssistantError as y, registerBrowserRoutes as z };