import { x as parseAgentSessionKey } from "./session-key-C7qWg3SH.js";
import { s as resolveStateDir } from "./paths-Biw6RtsH.js";
import { t as createSubsystemLogger } from "./subsystem-BBPkQl8F.js";
import { s as resolveAgentWorkspaceDir } from "./agent-scope-D2mtbkzb.js";
import "./redact-WaOJt0Qq.js";
import "./paths-BWCcyYYG.js";
import { _ as isFileMissingError, i as listSessionFilesForAgent, n as requireNodeSqlite, r as buildSessionEntry, t as extractKeywords, v as statRegularFile } from "./query-expansion-D4QN9-CF.js";
	...
	*/
	...
	async ensureMcporterDaemonStarted(mcporter) {
		...
		if (!mcporter.startDaemon) {
			const g = globalThis;
			if (!g.__PowPowMcporterColdStartWarned) {
				g.__PowPowMcporterColdStartWarned = true;
		...
		const g = globalThis;
		if (!g.__PowPowMcporterDaemonStart) g.__PowPowMcporterDaemonStart = (async () => {
			try {
				await this.runMcporter(["daemon", "start"], { timeoutMs: 1e4 });
			} catch (err) {
				log.warn(`mcporter daemon start failed: ${String(err)}`);
				delete g.__PowPowMcporterDaemonStart;
			}
		})();
		await g.__PowPowMcporterDaemonStart;
	}
