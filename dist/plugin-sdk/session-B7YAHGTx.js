import { f as readCredsJsonRaw, g as resolveWebCredsPath, h as resolveWebCredsBackupPath, m as resolveDefaultWebAuthDir, u as maybeRestoreCredsFromBackup } from "./accounts-DJ8kKC0c.js";
import { H as resolveUserPath, M as ensureDir, Z as danger, ct as getChildLogger, dt as toPinoLikeLogger, rt as success } from "./subsystem-BBPkQl8F.js";
import { Mt as VERSION } from "./config-Beao1DYH.js";
...
async function createWaSocket(printQr, verbose, opts = {}) {
	...
	sock.ev.on("connection.update", (update) => {
		try {
			...
			if (connection === "close") {
				if (getStatusCode(lastDisconnect?.error) === DisconnectReason.loggedOut) console.error(danger(`WhatsApp session logged out. Run: ${formatCliCommand("PowPow channels login")}`));
			}
