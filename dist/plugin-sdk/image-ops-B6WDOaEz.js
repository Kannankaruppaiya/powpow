import { D as runExec } from "./agent-scope-D2mtbkzb.js";
...
function prefersSips() {
	return process.env.POWPOW_IMAGE_BACKEND === "sips" || process.env.POWPOW_IMAGE_BACKEND !== "sharp" && isBun() && process.platform === "darwin";
}
...
async function withTempDir(fn) {
	const dir = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-img-"));
	try {
		return await fn(dir);
	} finally {
		await fs.rm(dir, {
