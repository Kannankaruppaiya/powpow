import { H as resolveUserPath, O as CONFIG_DIR, mt as resolvePreferredPowPowTmpDir, t as createSubsystemLogger } from "./subsystem-BBPkQl8F.js";
import { i as parseBooleanValue } from "./command-format-KJDDAM2E.js";
import { it as isDangerousHostEnvVarName } from "./model-selection-CYqXFRmu.js";
import { N as normalizeSkillFilter, T as resolvePowPowPackageRootSync } from "./agent-scope-D2mtbkzb.js";
import { a as LEGACY_MANIFEST_KEYS, c as normalizePluginsConfig, l as resolveEffectiveEnableState, o as MANIFEST_KEY, t as loadPluginManifestRegistry, u as resolveMemorySlotDecision } from "./manifest-registry-CPTnK_Gz.js";
...
function resolvePowPowManifestBlock(params) {
...
function resolvePowPowManifestRequires(metadataObj) {
...
function resolvePowPowManifestInstall(metadataObj, parseInstallSpec) {
	return (Array.isArray(metadataObj.install) ? metadataObj.install : []).map((entry) => parseInstallSpec(entry)).filter((entry) => Boolean(entry));
}
function resolvePowPowManifestOs(metadataObj) {
	return normalizeStringList(metadataObj.os);
}
function parsePowPowManifestInstallBase(input, allowedKinds) {
...
function parseInstallSpec(input) {
	const parsed = parsePowPowManifestInstallBase(input, [
		"brew",
		"node",
		"go",
		"uv",
...
function resolvePowPowMetadata(frontmatter) {
	const metadataObj = resolvePowPowManifestBlock({ frontmatter });
	if (!metadataObj) return;
	const requires = resolvePowPowManifestRequires(metadataObj);
	const install = resolvePowPowManifestInstall(metadataObj, parseInstallSpec);
	const osRaw = resolvePowPowManifestOs(metadataObj);
	return {
...
const BUNDLED_SOURCES = new Set(["powpow-bundled"]);
...
const BLOCKED_ENV_VAR_PATTERNS = [
	...
	/^POWPOW_GATEWAY_(TOKEN|PASSWORD)$/i,
...
async function assertNoHardlinkedFinalPath(params) {
	...
	if (stat.nlink > 1) throw new Error(`Hardlinked path is not allowed under ${params.boundaryLabel} (${shortPath$2(params.root)}): ${shortPath$2(params.filePath)}`);
}
function shortPath$2(value) {
...
async function assertNoPathAliasEscape(params) {
	...
	if (!isPathInside(root, target)) throw new Error(`Path escapes ${params.boundaryLabel} (${shortPath$1(root)}): ${shortPath$1(params.absolutePath)}`);
	...
	if (relative) {
		...
		for (let idx = 0; idx < parts.length; idx += 1) {
			...
			try {
				if (!(await fs$1.lstat(current)).isSymbolicLink()) continue;
				if (params.policy?.allowFinalSymlinkForUnlink && isLast) return;
				const symlinkTarget = await tryRealpath(current);
				if (!isPathInside(rootReal, symlinkTarget)) throw new Error(`Symlink escapes ${params.boundaryLabel} (${shortPath$1(rootReal)}): ${shortPath$1(current)}`);
				current = symlinkTarget;
			} catch (error) {
				if (isNotFoundPathError(error)) break;
				throw error;
			}
		}
	}
...
async function resolveAllowedTmpMediaPath(params) {
	...
	const PowPowTmpDir = path.resolve(resolvePreferredPowPowTmpDir());
	if (!isPathInside(PowPowTmpDir, resolved)) return;
	await assertNoTmpAliasEscape({
		filePath: resolved,
		tmpRoot: PowPowTmpDir
	});
	return resolved;
}
...
function resolveBundledSkillsDir(opts = {}) {
	const override = process.env.POWPOW_BUNDLED_SKILLS_DIR?.trim();
	...
	try {
		...
		const packageRoot = resolvePowPowPackageRootSync({
...
function loadSkillEntries(workspaceDir, opts) {
	...
	return Array.from(merged.values()).map((skill) => {
		...
		return {
			skill,
			frontmatter,
			metadata: resolvePowPowMetadata(frontmatter),
...
function resolveWorkspaceSkillPromptState(workspaceDir, opts) {
	...
	return {
		...
		prompt: [
			remoteNote,
			truncated ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`PowPow skills check\` to audit.` : "",
...
//#endregion
export { safeEqualSecret as _, syncSkillsToWorkspace as a, assertSandboxPath as c, resolveSandboxedMediaSource as d, PATH_ALIAS_POLICIES as f, sanitizeEnvVars as g, applySkillEnvOverridesFromSnapshot as h, resolveSkillsPromptForRun as i, resolveSandboxInputPath as l, applySkillEnvOverrides as m, buildWorkspaceSkillSnapshot as n, resolvePluginSkillDirs as o, assertNoPathAliasEscape as p, loadWorkspaceSkillEntries as r, assertMediaNotDataUrl as s, buildWorkspaceSkillCommandSpecs as t, resolveSandboxPath as u };