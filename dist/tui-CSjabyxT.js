import { l as stripAnsi$1, u as visibleWidth } from "./subsystem-DXlyH4H4.js";
import { d as resolveDefaultAgentId } from "./agent-scope-Ac0Vygcn.js";
import { c as normalizeMainKey, r as buildAgentMainSessionKey, s as normalizeAgentId, w as parseAgentSessionKey } from "./session-key-DXmKFT2p.js";
import { i as loadConfig } from "./config-B91e8u6L.js";
import { r as VERSION } from "./env-vars-D53NnL-N.js";
import { f as GATEWAY_CLIENT_CAPS, h as GATEWAY_CLIENT_NAMES, m as GATEWAY_CLIENT_MODES } from "./message-channel-DYK7E7vt.js";
import { i as stripLeadingInboundMetadata } from "./chat-envelope-9mWFsNOQ.js";
import { At as PROTOCOL_VERSION, t as GatewayClient } from "./client-FngVpCQH.js";
import { i as ensureExplicitGatewayAuth, o as resolveExplicitGatewayAuth, t as buildGatewayConnectionDetails } from "./call-Dy4jPY6_.js";
import { _ as formatRawAssistantErrorForUi } from "./pi-embedded-helpers-DWdt6mA4.js";
import { c as normalizeUsageDisplay, r as listThinkingLevelLabels, t as formatThinkingLevels, u as resolveResponseUsageMode } from "./thinking-CJPPUYWd.js";
import { a as listChatCommandsForConfig, i as listChatCommands } from "./commands-registry-5aQe-nwe.js";
import { n as resolveToolDisplay, t as formatToolDetail } from "./tool-display-DYQmKeWr.js";
import { n as formatTimeAgo, t as formatRelativeTimestamp } from "./format-relative-Czf5fUjn.js";
import { n as formatTokenCount } from "./usage-format-EdDmn0JB.js";
...
function resolveGatewayConnection(opts) {
	...
	return {
		...
		token: explicitAuth.token || (!urlOverride ? isRemoteMode ? typeof remote?.token === "string" && remote.token.trim().length > 0 ? remote.token.trim() : void 0 : process.env.POWPOW_GATEWAY_TOKEN?.trim() || (typeof authToken === "string" && authToken.trim().length > 0 ? authToken.trim() : void 0) : void 0),
		password: explicitAuth.password || (!urlOverride ? process.env.POWPOW_GATEWAY_PASSWORD?.trim() || (typeof remote?.password === "string" && remote.password.trim().length > 0 ? remote.password.trim() : void 0) : void 0)
	};
}
...
function resolveGatewayDisconnectState(reason) {
	...
	if (/pairing required/i.test(reasonLabel)) return {
		...
		activityStatus: "pairing required: run PowPow devices list",
		pairingHint: "Pairing required. Run `PowPow devices list`, approve your request ID, then reconnect."
	};
	return {
		connectionStatus: `gateway disconnected: ${reasonLabel}`,
		activityStatus: "idle"
...
async function runTui(opts) {
	...
	const updateHeader = () => {
		...
		header.setText(theme.header(`PowPow tui - ${client.connection.url} - agent ${agentLabel} - session ${sessionLabel}`));
	};
