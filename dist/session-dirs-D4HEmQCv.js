import { m as pathExists } from "./utils-BWJH7VRh.js";
...
import { d as channelToNpmTag, l as DEFAULT_PACKAGE_CHANNEL, m as isStableTag, p as isBetaTag, r as compareSemverStrings, s as detectPackageManager$1, u as DEV_BRANCH } from "./channel-account-context-23ILPOVs.js";
...
const PRIMARY_PACKAGE_NAME = "PowPow";
...
async function runGatewayUpdate(opts = {}) {
	...
	if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
		...
		if (channel === "dev") {
			...
			const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-update-preflight-"));
		...
		const doctorEntry = path.join(gitRoot, "PowPow.mjs");
		if (!await fs.stat(doctorEntry).then(() => true).catch(() => false)) {
			steps.push({
				name: "PowPow doctor entry",
		...
		const doctorStep = await runStep(step("PowPow doctor", [
			process.execPath,
			doctorEntry,
			"doctor",
			"--non-interactive",
			"--fix"
		], gitRoot, { POWPOW_UPDATE_IN_PROGRESS: "1" }));
...
//#endregion
export { detectGlobalInstallManagerForRoot as a, readPackageName as c, detectGlobalInstallManagerByPresence as i, readPackageVersion as l, runGatewayUpdate as n, globalInstallArgs as o, cleanupGlobalRenameDirs as r, resolveGlobalPackageRoot as s, resolveAgentSessionDirs as t };