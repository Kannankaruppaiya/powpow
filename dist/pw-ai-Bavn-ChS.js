...
import { $ as resolvePreferredPowPowTmpDir } from "./utils-BWJH7VRh.js";
...
import { _ as appendCdpPath, b as getHeadersWithAuth, d as normalizeCdpWsUrl, g as withBrowserNavigationPolicy, h as assertBrowserNavigationResultAllowed, m as assertBrowserNavigationAllowed, t as getChromeWebSocketUrl, u as formatAriaSnapshot, v as fetchJson, x as withCdpSocket } from "./chrome-CwN6j-nF.js";
import "./redact-CAjBvpi8.js";
import { n as formatErrorMessage } from "./errors-BpyuHard.js";
import "./fs-safe-CEkqp78T.js";
import { a as resolveStrictExistingPathsWithinRoot, r as DEFAULT_UPLOAD_DIR } from "./paths-B2BIhK5k.js";
import "./ssrf-AiRrlY78.js";
import "./ports-61DuVTUr.js";
import { n as markPwAiLoaded } from "./tool-loop-detection-DjAZW14L.js";
...
function buildTempDownloadPath(fileName) {
	...
	return path.join(resolvePreferredPowPowTmpDir(), "downloads", `${id}-${safeName}`);
}
...
async function screenshotWithLabelsViaPlaywright(opts) {
	...
	try {
		if (boxes.length > 0) await page.evaluate((labels) => {
			document.querySelectorAll("[data-powpow-labels]").forEach((el) => el.remove());
			const root = document.createElement("div");
			root.setAttribute("data-powpow-labels", "1");
			...
			for (const label of labels) {
				...
				box.setAttribute("data-powpow-labels", "1");
				...
				tag.setAttribute("data-powpow-labels", "1");
...
async function responseBodyViaPlaywright(opts) {
	...
	const resp = await new Promise((resolve, reject) => {
		...
		timer = setTimeout(() => {
			...
			cleanup();
			reject(/* @__PURE__ */ new Error(`Response not found for url pattern "${pattern}". Run '${formatCliCommand("PowPow browser requests")}' to inspect recent network activity.`));
		}, timeout);
	});
	const url = resp.url?.() || "";
