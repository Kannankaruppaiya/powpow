...
function resolveRawHomeDir(env, homedir) {
	const explicitHome = normalize(env.POWPOW_HOME);
	if (explicitHome) {
...
/**
* Nix mode detection: When POWPOW_NIX_MODE=1, the gateway is running under Nix.
* In this mode:
...
const NEW_STATE_DIRNAME = ".PowPow";
const CONFIG_FILENAME = "PowPow.json";
...
/** Build a homedir thunk that respects POWPOW_HOME for the given env. */
...
* Can be overridden via POWPOW_STATE_DIR.
* Default: ~/.PowPow
*/
function resolveStateDir(env = process.env, homedir = envHomedir(env)) {
	const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
	const override = env.POWPOW_STATE_DIR?.trim() || env.POWPOW_STATE_DIR?.trim();
...
* Can be overridden via POWPOW_CONFIG_PATH.
* Default: ~/.PowPow/PowPow.json (or $POWPOW_STATE_DIR/PowPow.json)
*/
function resolveCanonicalConfigPath(env = process.env, stateDir = resolveStateDir(env, envHomedir(env))) {
	const override = env.POWPOW_CONFIG_PATH?.trim() || env.powpow_CONFIG_PATH?.trim();
...
function resolveConfigPath(env = process.env, stateDir = resolveStateDir(env, envHomedir(env)), homedir = envHomedir(env)) {
	const override = env.POWPOW_CONFIG_PATH?.trim();
	if (override) return resolveUserPath$1(override, env, homedir);
	const stateOverride = env.POWPOW_STATE_DIR?.trim();
...
function resolveDefaultConfigCandidates(env = process.env, homedir = envHomedir(env)) {
	const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
	const explicit = env.POWPOW_CONFIG_PATH?.trim() || env.powpow_CONFIG_PATH?.trim();
	if (explicit) return [resolveUserPath$1(explicit, env, effectiveHomedir)];
	const candidates = [];
	const PowPowStateDir = env.POWPOW_STATE_DIR?.trim() || env.POWPOW_STATE_DIR?.trim();
	if (PowPowStateDir) {
		const resolved = resolveUserPath$1(PowPowStateDir, env, effectiveHomedir);
...
//#endregion
//#region src/infra/tmp-powpow-dir.ts
const POSIX_POWPOW_TMP_DIR = "/tmp/PowPow";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;
function isNodeErrorWithCode(err, code) {
	return typeof err === "object" && err !== null && "code" in err && err.code === code;
}
function resolvePreferredPowPowTmpDir(options = {}) {
	...
	const fallback = () => {
		const base = tmpdir();
		const suffix = uid === void 0 ? "PowPow" : `powpow-${uid}`;
	...
	const ensureTrustedFallbackDir = () => {
		...
		if (state === "invalid") throw new Error(`Unsafe fallback PowPow temp dir: ${fallbackPath}`);
		try {
			...
			throw new Error(`Unable to create fallback PowPow temp dir: ${fallbackPath}`);
		}
		if (resolveDirState(fallbackPath) !== "available") throw new Error(`Unsafe fallback PowPow temp dir: ${fallbackPath}`);
		return fallbackPath;
	};
	const existingPreferredState = resolveDirState(POSIX_POWPOW_TMP_DIR);
	if (existingPreferredState === "available") return POSIX_POWPOW_TMP_DIR;
	if (existingPreferredState === "invalid") return ensureTrustedFallbackDir();
	try {
		accessSync("/tmp", TMP_DIR_ACCESS_MODE);
		mkdirSync(POSIX_POWPOW_TMP_DIR, {
			recursive: true,
			mode: 448
		});
		if (resolveDirState(POSIX_POWPOW_TMP_DIR) !== "available") return ensureTrustedFallbackDir();
		return POSIX_POWPOW_TMP_DIR;
	} catch {
...
function resolveEnvLogLevelOverride() {
	const raw = process.env.POWPOW_LOG_LEVEL;
	...
	if (loggingState.invalidEnvLogLevelValue !== trimmed) {
		...
		process.stderr.write(`[powpow] Ignoring invalid POWPOW_LOG_LEVEL="${trimmed}" (allowed: ${ALLOWED_LOG_LEVELS.join("|")}).\n`);
	}
}
...
const DEFAULT_LOG_DIR = resolvePreferredPowPowTmpDir();
const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "PowPow.log");
const LOG_PREFIX = "PowPow";
...
function resolveSettings() {
	...
	const defaultLevel = process.env.VITEST === "true" && process.env.POWPOW_TEST_FILE_LOG !== "1" ? "silent" : "info";
		...
		try {
			...
			if (nextBytes > settings.maxFileBytes) {
				if (!warnedAboutSizeCap) {
					...
					process.stderr.write(`[powpow] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`);
				}
				return;
			}
...
function resolveConfigDir(env = process.env, homedir = os.homedir) {
	...
	const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".PowPow");
	try {
		if (fs.existsSync(newDir)) return newDir;
	} catch {}
	return newDir;
...
function resolveHomeDisplayPrefix() {
	...
	if (!home) return;
	if (process.env.POWPOW_HOME?.trim()) return {
		home,
		prefix: "$POWPOW_HOME"
	};
	return {
...
const DOCS_ROOT = "https://docs.PowPow.ai";
...
const DEFAULT_CLI_NAME = "PowPow";
const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE$1 = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(PowPow)\b/;
...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+PowPow\b|^PowPow\b/;
...
function formatCliCommand(command, env = process.env) {
	...
	const profile = normalizeProfileName(env.POWPOW_PROFILE);
...
function shouldEmitRuntimeLog(env = process.env) {
	...
	if (env.POWPOW_TEST_RUNTIME_LOG === "1") return true;
...
const CORE_PACKAGE_NAME = "PowPow";
...
const VERSION = typeof __POWPOW_VERSION__ === "string" && __POWPOW_VERSION__ || process.env.POWPOW_BUNDLED_VERSION || resolveVersionFromModuleUrl(import.meta.url) || "0.0.0";
...
function buildServiceEnvironment(params) {
	...
	const profile = env.POWPOW_PROFILE;
	...
	const stateDir = env.POWPOW_STATE_DIR;
	const configPath = env.POWPOW_CONFIG_PATH;
	...
	return {
		...
		...proxyEnv,
		POWPOW_PROFILE: profile,
		POWPOW_STATE_DIR: stateDir,
		POWPOW_CONFIG_PATH: configPath,
		POWPOW_GATEWAY_PORT: String(port),
		POWPOW_GATEWAY_TOKEN: token,
		POWPOW_LAUNCHD_LABEL: resolvedLaunchdLabel,
		POWPOW_SYSTEMD_UNIT: systemdUnit,
		POWPOW_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
		POWPOW_SERVICE_KIND: GATEWAY_SERVICE_KIND,
		POWPOW_SERVICE_VERSION: VERSION
	};
}
...
async function buildGatewayInstallPlan(params) {
	...
	const serviceEnvironment = buildServiceEnvironment({
		...
		port: params.port,
		token: params.token,
		launchdLabel: process.platform === "darwin" ? resolveGatewayLaunchAgentLabel(params.env.POWPOW_PROFILE) : void 0
	});
...
const REGISTRY_STATE = Symbol.for("PowPow.pluginRegistryState");
const state = (() => {
...
function normalizeConsoleLevel(level) {
	...
	if (!level && process.env.VITEST === "true" && process.env.POWPOW_TEST_CONSOLE !== "1") return "silent";
...
function resolveDefaultAgentWorkspaceDir(env = process.env, homedir = os.homedir) {
	...
	const profile = env.POWPOW_PROFILE?.trim();
	if (profile && profile.toLowerCase() !== "default") return path.join(home, ".PowPow", `workspace-${profile}`);
	return path.join(home, ".PowPow", "workspace");
}
...
function loadShellEnvFallback(opts) {
	...
	if (!probe.ok) {
		logger.warn(`[powpow] shell env fallback failed: ${probe.error}`);
		lastAppliedKeys = [];
		return {
			ok: false,
			error: probe.error,
...
function shouldEnableShellEnvFallback(env) {
	return isTruthyEnvValue(env.POWPOW_LOAD_SHELL_ENV);
}
function shouldDeferShellEnvFallback(env) {
	return isTruthyEnvValue(env.POWPOW_DEFER_SHELL_ENV_FALLBACK);
}
function resolveShellEnvFallbackTimeoutMs(env) {
	const raw = env.POWPOW_SHELL_ENV_TIMEOUT_MS?.trim();
...
const HELD_LOCKS$1 = resolveProcessScopedMap(Symbol.for("PowPow.fileLockHeldLocks"));
...
function resolveBundledPluginsDir() {
	const override = process.env.POWPOW_BUNDLED_PLUGINS_DIR?.trim();
...
const PROJECT_NAME = "PowPow";
const MANIFEST_KEY = PROJECT_NAME;

//#endregion
//#region src/plugins/manifest.ts
const PLUGIN_MANIFEST_FILENAME = "PowPow.plugin.json";
...
function discoverPowPowPlugins(params) {
	const candidates = [];
	const diagnostics = [];
	const seen = /* @__PURE__ */ new Set();
	...
	if (workspaceDir) {
		...
		const workspaceExtDirs = [path.join(workspaceRoot, ".PowPow", "extensions")];
...
function resolveManifestCacheMs(env) {
	const raw = env.POWPOW_PLUGIN_MANIFEST_CACHE_MS?.trim();
...
function shouldUseManifestCache(env) {
	if (env.POWPOW_DISABLE_PLUGIN_MANIFEST_CACHE?.trim()) return false;
...
function validateConfigObjectRaw(raw) {
	...
	const validated = PowPowSchema.safeParse(raw);
...
function parsePowPowVersion(raw) {
	if (!raw) return null;
...
function comparePowPowVersions(a, b) {
	const parsedA = parsePowPowVersion(a);
	const parsedB = parsePowPowVersion(b);
...
const SHELL_ENV_EXPECTED_KEYS = [
	...
	"SLACK_BOT_TOKEN",
	"SLACK_APP_TOKEN",
	"POWPOW_GATEWAY_TOKEN",
	"POWPOW_GATEWAY_PASSWORD"
];
...
function formatConfigValidationFailure(pathLabel, issueMessage) {
	...
	return [
		...
		"",
		"Fix with:",
		`  PowPow config set ${allowPath} '["*"]'`,
		"",
		"Or switch policy:",
		`  PowPow config set ${policyPath} "pairing"`
	].join("\n");
}
...
function warnIfConfigFromFuture(cfg, logger) {
	...
	if (!touched) return;
	const cmp = comparePowPowVersions(VERSION, touched);
	if (cmp === null) return;
	if (cmp < 0) logger.warn(`Config was last written by a newer PowPow (${touched}); current version is ${VERSION}.`);
}
...
function createConfigIO(overrides = {}) {
	...
	async function writeConfigFile(cfg, options = {}) {
		...
		const logConfigOverwrite = () => {
			...
			const shouldLogInVitest = deps.env.POWPOW_TEST_CONFIG_OVERWRITE_LOG === "1";
		...
		const logConfigWriteAnomalies = () => {
			...
			const isVitest = deps.env.VITEST === "true";
			const shouldLogInVitest = deps.env.POWPOW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
			if (isVitest && !shouldLogInVitest) return;
		...
		const auditRecordBase = {
			...
			watchMode: deps.env.POWPOW_WATCH_MODE === "1",
			watchSession: typeof deps.env.POWPOW_WATCH_SESSION === "string" && deps.env.POWPOW_WATCH_SESSION.trim().length > 0 ? deps.env.POWPOW_WATCH_SESSION.trim() : null,
			watchCommand: typeof deps.env.POWPOW_WATCH_COMMAND === "string" && deps.env.POWPOW_WATCH_COMMAND.trim().length > 0 ? deps.env.POWPOW_WATCH_COMMAND.trim() : null,
			existsBefore: snapshot.exists,
...
function resolveConfigCacheMs(env) {
	const raw = env.POWPOW_CONFIG_CACHE_MS?.trim();
	if (raw === "" || raw === "0") return 0;
...
function shouldUseConfigCache(env) {
	if (env.POWPOW_DISABLE_CONFIG_CACHE?.trim()) return false;
...
const GATEWAY_CLIENT_IDS = {
	...
	CONTROL_UI: "powpow-control-ui",
	WEBCHAT: "webchat",
	CLI: "cli",
	GATEWAY_CLIENT: "gateway-client",
	MACOS_APP: "powpow-macos",
	IOS_APP: "powpow-ios",
	ANDROID_APP: "powpow-android",
	NODE_HOST: "node-host",
	TEST: "test",
...
const CLEANUP_STATE_KEY = Symbol.for("PowPow.sessionWriteLockCleanupState");
const HELD_LOCKS_KEY = Symbol.for("PowPow.sessionWriteLockHeldLocks");
const WATCHDOG_STATE_KEY = Symbol.for("PowPow.sessionWriteLockWatchdogState");
...
/**
* Strips powpow-injected inbound metadata blocks from a user-role message
...
function readGatewayTokenEnv(env, includeLegacyEnv) {
	const primary = trimToUndefined$1(env.POWPOW_GATEWAY_TOKEN);
	if (primary) return primary;
	if (!includeLegacyEnv) return;
	return trimToUndefined$1(env.powpow_GATEWAY_TOKEN);
}
function readGatewayPasswordEnv(env, includeLegacyEnv) {
	const primary = trimToUndefined$1(env.POWPOW_GATEWAY_PASSWORD);
	if (primary) return primary;
	if (!includeLegacyEnv) return;
	return trimToUndefined$1(env.powpow_GATEWAY_PASSWORD);
}
...
function resolveGatewayStateDir(env) {
	const override = env.POWPOW_STATE_DIR?.trim();
	...
	const suffix = resolveGatewayProfileSuffix(env.POWPOW_PROFILE);
	return path.join(home, `.PowPow${suffix}`);
}
...
function resolveLaunchAgentLabel(args) {
	const envLabel = args?.env?.POWPOW_LAUNCHD_LABEL?.trim();
	if (envLabel) return envLabel;
	return resolveGatewayLaunchAgentLabel(args?.env?.POWPOW_PROFILE);
}
...
function resolveGatewayLogPaths(env) {
	...
	const prefix = env.POWPOW_LOG_PREFIX?.trim() || "gateway";
	return {
		logDir,
...
async function installLaunchAgent({ env, stdout, programArguments, workingDirectory, environment, description }) {
	...
	const domain = resolveGuiDomain();
	const label = resolveLaunchAgentLabel({ env });
	for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.POWPOW_PROFILE)) {
	...
	if (boot.code !== 0) {
		...
		if (isUnsupportedGuiDomain(detail)) throw new Error([
			...
			"Fix: sign in to the macOS desktop as the target user and rerun `PowPow gateway install --force`.",
			"Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.PowPow.ai/gateway"
		].join("\n"));
...
async function restartLaunchAgent({ stdout, env }) {
	...
	if (boot.code !== 0) {
		...
		if (isUnsupportedGuiDomain(detail)) throw new Error([
			...
			"This usually means you are running from SSH/headless context or as the wrong user (including sudo).",
			"Fix: sign in to the macOS desktop as the target user and rerun `PowPow gateway restart`.",
			"Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.PowPow.ai/gateway"
		].join("\n"));
		throw new Error(`launchctl bootstrap failed: ${detail}`);
	}
...
function resolveTaskName(env) {
	const override = env.POWPOW_WINDOWS_TASK_NAME?.trim();
	if (override) return override;
	return resolveGatewayWindowsTaskName(env.POWPOW_PROFILE);
}
function resolveTaskScriptPath(env) {
	const override = env.POWPOW_TASK_SCRIPT?.trim();
	if (override) return override;
	const scriptName = env.POWPOW_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
	const stateDir = resolveGatewayStateDir(env);
...
function buildSystemdUnit({ description, programArguments, workingDirectory, environment }) {
	...
	const descriptionValue = description?.trim() || "PowPow Gateway";
...
function resolveSystemdServiceName(env) {
	const override = env.POWPOW_SYSTEMD_UNIT?.trim();
	if (override) return override.endsWith(".service") ? override.slice(0, -8) : override;
	return resolveGatewaySystemdServiceName(env.POWPOW_PROFILE);
}
...
async function installSystemdService({ env, stdout, programArguments, workingDirectory, environment, description }) {
	...
	const unitName = `${resolveGatewaySystemdServiceName(env.POWPOW_PROFILE)}.service`;
...
async function uninstallSystemdService({ env, stdout }) {
	...
	await execSystemctl([
		...
		"disable",
		"--now",
		`${resolveGatewaySystemdServiceName(env.POWPOW_PROFILE)}.service`
	]);
	const unitPath = resolveSystemdUnitPath(env);
	try {
...
function renderGatewayServiceStartHints(env = process.env) {
	const base = [formatCliCommand("PowPow gateway install", env), formatCliCommand("PowPow gateway", env)];
	const profile = env.POWPOW_PROFILE;
...
async function runDaemonInstall(opts) {
	...
	if (loaded) {
		if (!opts.force) {
			...
			if (!json) {
				...
				defaultRuntime.log(`Reinstall with: ${formatCliCommand("PowPow gateway install --force")}`);
			}
			return;
		}
	}
	...
	let token = opts.token || cfg.gateway?.auth?.token || process.env.POWPOW_GATEWAY_TOKEN || process.env.powpow_GATEWAY_TOKEN;
	if (!token && needsToken) {
		token = randomToken();
...
function auditGatewayToken(command, issues, expectedGatewayToken) {
	...
	const serviceToken = command?.environment?.POWPOW_GATEWAY_TOKEN?.trim();
	if (serviceToken === expectedToken) return;
	issues.push({
		code: SERVICE_AUDIT_CODES.gatewayTokenMismatch,
		message: "Gateway service POWPOW_GATEWAY_TOKEN does not match gateway.auth.token in PowPow.json",
...
function checkTokenDrift(params) {
	...
	if (configToken && serviceToken !== configToken) return {
		...
		detail: "Run `PowPow gateway install --force` to sync the token.",
		level: "recommended"
	};
	return null;
}
...
function renderSystemdUnavailableHints(options = {}) {
	...
	return ["systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.", `If you're in a container, run the gateway in the foreground instead of \`${formatCliCommand("PowPow gateway")}\`.`];
}
...
async function runServiceRestart(params) {
	...
	if (params.checkTokenDrift) try {
		const serviceToken = (await params.service.readCommand(process.env))?.environment?.POWPOW_GATEWAY_TOKEN;
...
function classifyPortListener(listener, port) {
	...
	if (raw.includes("PowPow")) return "gateway";
	if (raw.includes("ssh")) {
...
function buildPortHints(listeners, port) {
	...
	const hints = [];
	if (kinds.has("gateway")) hints.push(`Gateway already running locally. Stop it (${formatCliCommand("PowPow gateway stop")}) or use a different port.`);
...
async function runDaemonRestart(opts = {}) {
	...
	return await runServiceRestart({
		...
		postRestartCheck: async ({ warnings, fail, stdout }) => {
			...
			fail(`Gateway restart timed out after ${restartWaitSeconds}s waiting for health checks.`, [formatCliCommand("PowPow gateway status --deep"), formatCliCommand("PowPow doctor")]);
		}
	});
}
...
function hasGatewayServiceMarker(content) {
	const lower = content.toLowerCase();
	const markerKeys = ["POWPOW_service_marker"];
	const kindKeys = ["POWPOW_service_kind"];
...
function isPowPowGatewayLaunchdService(label, contents) {
	if (hasGatewayServiceMarker(contents)) return true;
	if (!contents.toLowerCase().includes("gateway")) return false;
	return label.startsWith("ai.PowPow.");
}
function isPowPowGatewaySystemdService(name, contents) {
	if (hasGatewayServiceMarker(contents)) return true;
	if (!name.startsWith("powpow-gateway")) return false;
	return contents.toLowerCase().includes("gateway");
}
function isPowPowGatewayTaskName(name) {
	const normalized = name.trim().toLowerCase();
	if (!normalized) return false;
	return normalized === resolveGatewayWindowsTaskName().toLowerCase() || normalized.startsWith("PowPow gateway");
}
...
function isLegacyLabel(label) {
	...
	return lower.includes("powpow") || lower.includes("moltbot");
}
	...
	for (const { name: labelFromName, fullPath, contents } of candidates) {
		...
		if (!marker) {
			...
			results.push({
				...
				scope: params.scope,
				marker: isLegacyLabel(label) ? "powpow" : "moltbot",
				legacy: true
			});
			continue;
		}
		if (isIgnoredLaunchdLabel(label)) continue;
		if (marker === "PowPow" && isPowPowGatewayLaunchdService(label, contents)) continue;
		results.push({
			platform: "darwin",
			label,
			detail: `plist: ${fullPath}`,
			scope: params.scope,
			marker,
			legacy: marker !== "PowPow" || isLegacyLabel(label)
		});
	}
	return results;
}
async function scanSystemdDir(params) {
	...
	for (const { entry, name, fullPath, contents } of candidates) {
		const marker = detectMarker(contents);
		if (!marker) continue;
		if (marker === "PowPow" && isPowPowGatewaySystemdService(name, contents)) continue;
		results.push({
			platform: "linux",
			label: entry,
...
async function findExtraGatewayServices(env, opts = {}) {
	...
	if (process.platform === "win32") {
		...
		for (const task of tasks) {
			...
			if (!name) continue;
			if (isPowPowGatewayTaskName(name)) continue;
...
async function gatherDaemonStatus(opts) {
	...
	const rpc = opts.probe ? await probeGatewayStatus({
		url: probeUrl,
		token: opts.rpc.token || mergedDaemonEnv.POWPOW_GATEWAY_TOKEN || daemonCfg.gateway?.auth?.token,
		password: opts.rpc.password || mergedDaemonEnv.POWPOW_GATEWAY_PASSWORD || daemonCfg.gateway?.auth?.password,
		timeoutMs,
		json: opts.rpc.json,
...
function printDaemonStatus(status, opts) {
	...
	if (service.configAudit?.issues.length) {
		...
		defaultRuntime.error(warnText(`Recommendation: run "${formatCliCommand("PowPow doctor")}" (or "${formatCliCommand("PowPow doctor --repair")}").`));
	}
	if (status.config) {
		...
		if (status.config.mismatch) {
			...
			defaultRuntime.error(errorText(`Fix: rerun \`${formatCliCommand("PowPow gateway install --force")}\` from the same --profile / POWPOW_STATE_DIR you expect.`));
		}
		spacer();
	}
	if (status.gateway) {
	...
	if (service.runtime?.cachedLabel) {
		const labelValue = resolveGatewayLaunchAgentLabel((service.command?.environment ?? process.env).POWPOW_PROFILE);
		...
		defaultRuntime.error(errorText(`Then reinstall: ${formatCliCommand("PowPow gateway install")}`));
		spacer();
	}
	...
	if (service.loaded && service.runtime?.status === "running" && status.port && status.port.status !== "busy") {
		...
		if (process.platform === "linux") {
			const unit = resolveGatewaySystemdServiceName((service.command?.environment ?? process.env).POWPOW_PROFILE);
	...
	defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("PowPow status")}`);
	defaultRuntime.log(`${label("Troubleshooting:")} https://docs.PowPow.ai/troubleshooting`);
}
