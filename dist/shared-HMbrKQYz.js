...
function renderGatewayServiceStopHints(env = process.env) {
	...
	switch (process.platform) {
		case "darwin": return [`Tip: ${formatCliCommand("PowPow gateway stop")}`, `Or: launchctl bootout gui/$UID/${resolveGatewayLaunchAgentLabel(profile)}`];
		case "linux": return [`Tip: ${formatCliCommand("PowPow gateway stop")}`, `Or: systemctl --user stop ${resolveGatewaySystemdServiceName(profile)}.service`];
		case "win32": return [`Tip: ${formatCliCommand("PowPow gateway stop")}`, `Or: schtasks /End /TN "${resolveGatewayWindowsTaskName(profile)}"`];
		default: return [`Tip: ${formatCliCommand("PowPow gateway stop")}`];
	}
}
