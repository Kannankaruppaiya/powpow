...
import { p as restoreTerminalState } from "./subsystem-DXlyH4H4.js";
import { m as pathExists, v as resolveUserPath } from "./utils-BWJH7VRh.js";
import "./boolean-YY6K2DFz.js";
import "./auth-profiles-KPMal-k-.js";
import { r as resolveCliName, t as formatCliCommand } from "./command-format-DRH7uGKn.js";
import { v as DEFAULT_BOOTSTRAP_FILENAME } from "./agent-scope-Ac0Vygcn.js";
...
import { b as waitForGatewayReachable, f as openUrl, g as resolveControlUiLinks, i as detectBrowserOpenSupport, m as probeGatewayReachable, o as formatControlUiSshHint } from "./onboard-helpers-BNEaJ_0q.js";
...
import { t as ensureControlUiAssetsBuilt } from "./control-ui-assets-B_c5GFvQ.js";
...
async function finalizeOnboardingWizard(options) {
	...
	if (process.platform === "linux" && systemdAvailable) {
		const { ensureSystemdUserLingerInteractive } = await import("./systemd-linger-DSbbv8-4.js").then((n) => n.r);
	...
	if (!opts.skipHealth) {
		...
		try {
			...
			await prompter.note([
				"Docs:",
				"https://docs.PowPow.ai/gateway/health",
				"https://docs.PowPow.ai/gateway/troubleshooting"
			].join("\n"), "Health check help");
		}
	}
	...
	await prompter.note([
		...
		gatewayStatusLine,
		"Docs: https://docs.PowPow.ai/web/control-ui"
	].filter(Boolean).join("\n"), "Control UI");
	...
	if (!opts.skipUi && gatewayProbe.ok) {
		...
		await prompter.note([
			...
			"Stored in: ~/.PowPow/PowPow.json (gateway.auth.token) or POWPOW_GATEWAY_TOKEN.",
			`View token: ${formatCliCommand("PowPow config get gateway.auth.token")}`,
			`Generate token: ${formatCliCommand("PowPow doctor --generate-gateway-token")}`,
			"Web UI stores a copy in this browser's localStorage (PowPow.control.settings.v1).",
			`Open the dashboard anytime: ${formatCliCommand("PowPow dashboard --no-open")}`,
		...
		if (hatchChoice === "tui") {
			...
			await prompter.note([
				...
				controlUiOpened ? "Opened in your browser. Keep that tab to control PowPow." : "Copy/paste this URL in a browser on this machine to control PowPow.",
				controlUiOpenHint
			].filter(Boolean).join("\n"), "Dashboard ready");
		} else await prompter.note(`When you're ready: ${formatCliCommand("PowPow dashboard --no-open")}`, "Later");
	} else if (opts.skipUi) await prompter.note("Skipping Control UI/TUI prompts.", "Control UI");
	await prompter.note(["Back up your agent workspace.", "Docs: https://docs.PowPow.ai/concepts/agent-workspace"].join("\n"), "Workspace backup");
	await prompter.note("Running agents on your computer is risky — harden your setup: https://docs.PowPow.ai/security", "Security");
	...
	await prompter.note(hasWebSearchKey ? [
		...
		"Docs: https://docs.PowPow.ai/tools/web"
	].join("\n") : [
		"If you want your agent to be able to search the web, you’ll need an API key.",
		"",
		"PowPow uses Brave Search for the `web_search` tool. Without a Brave Search API key, web search won’t work.",
		"",
		"Set it up interactively:",
		`- Run: ${formatCliCommand("PowPow configure --section web")}`,
		"- Enable web_search and paste your Brave Search API key",
		"",
		"Alternative: set BRAVE_API_KEY in the Gateway environment (no config changes).",
		"Docs: https://docs.PowPow.ai/tools/web"
	].join("\n"), "Web search (optional)");
	await prompter.note("What now: https://PowPow.ai/showcase (\"What People Are Building\").", "What now");
	await prompter.outro(controlUiOpened ? "Onboarding complete. Dashboard opened; keep that tab to control PowPow." : "Onboarding complete. Use the dashboard link above to control PowPow.");
	return { launchedTui };
}
