...
import { t as CONFIG_PATH, u as resolveGatewayPort } from "./paths-BJtes5yu.js";
import { f as defaultRuntime } from "./subsystem-DXlyH4H4.js";
import { v as resolveUserPath, x as shortenHomePath } from "./utils-BWJH7VRh.js";
import { x as ensureAuthProfileStore } from "./auth-profiles-KPMal-k-.js";
...
import { n as listChannelPlugins, t as getChannelPlugin } from "./plugins-CPaMeorQ.js";
import { a as findTailscaleBinary } from "./tailscale-Mny9GO9e.js";
import { n as withProgress } from "./progress-DF7PrFZS.js";
import { a as ensureWorkspaceAndSessions, b as waitForGatewayReachable, g as resolveControlUiLinks, h as randomToken, m as probeGatewayReachable, n as applyWizardMetadata, p as printWizardHeader, s as guardCancel, t as DEFAULT_WORKSPACE, u as normalizeGatewayTokenInput, v as summarizeExistingConfig, y as validateGatewayPasswordInput } from "./onboard-helpers-BNEaJ_0q.js";
...
import { r as setupChannels, t as noteChannelStatus } from "./onboard-channels-02EDsJ--.js";
...
import { n as promptAuthChoiceGrouped } from "./auth-choice-prompt-BkMDAVXQ.js";
import { i as applyAuthChoice, n as resolvePreferredProviderForAuthChoice } from "./auth-choice-DR41Z0CW.js";
import { a as promptDefaultModel, n as applyModelFallbacksFromSelection, o as promptModelAllowlist, r as applyPrimaryModel, t as applyModelAllowlist } from "./model-picker-BEfZoi__.js";
import { t as ensureSystemdUserLingerInteractive } from "./systemd-linger-DSbbv8-4.js";
import { a as promptCustomApiConfig } from "./onboard-custom-Da9woe9U.js";
import { i as TAILSCALE_MISSING_BIN_NOTE_LINES, n as TAILSCALE_DOCS_LINES, r as TAILSCALE_EXPOSURE_OPTIONS, t as validateIPv4AddressInput } from "./ipv4-BAchxS-o.js";
import { t as formatHealthCheckFailure } from "./health-format-BP7yY5At.js";
import { n as promptRemoteGatewayConfig } from "./onboard-remote-DTIOnGgc.js";
import { n as setupSkills } from "./onboard-skills-DxXXGb1O.js";
...
async function removeChannelConfigWizard(cfg, runtime) {
	...
	while (true) {
		...
		if (configured.length === 0) {
			note$1(["No channel config found in PowPow.json.", `Tip: \`${formatCliCommand("PowPow channels status")}\` shows what is configured and enabled.`].join("\n"), "Remove channel");
			return next;
		}
...
async function promptGatewayConfig(cfg, runtime) {
	...
	if (tailscaleMode !== "off") {
		if (!await findTailscaleBinary()) note$1(TAILSCALE_MISSING_BIN_NOTE_LINES.join("\n"), "Tailscale Warning");
	}
	...
	if (authMode === "trusted-proxy") {
		note$1([
			"Trusted proxy mode: PowPow trusts user identity from a reverse proxy.",
			...
			"Docs: https://docs.PowPow.ai/gateway/trusted-proxy-auth"
		].join("\n"), "Trusted Proxy Auth");
...
async function runGatewayHealthCheck(params) {
	...
	await waitForGatewayReachable({
		...
		password: params.cfg.gateway?.auth?.password ?? process.env.POWPOW_GATEWAY_PASSWORD,
		deadlineMs: 15e3
	});
	try {
		await healthCommand({
...
async function promptChannelMode(runtime) {
	return guardCancel(await select$1({
		...
		options: [{
			...
			hint: "Delete channel tokens/settings from PowPow.json"
		}],
...
async function runConfigureWizard(opts, runtime = defaultRuntime) {
	try {
		...
		intro$1(opts.command === "update" ? "PowPow update wizard" : "PowPow configure");
		...
		if (snapshot.exists) {
			...
			if (!snapshot.valid && snapshot.issues.length > 0) note$1([
				...
				"",
				"Docs: https://docs.PowPow.ai/gateway/configuration"
			].join("\n"), "Config issues");
			if (!snapshot.valid) {
				outro$1(`Config invalid. Run \`${formatCliCommand("PowPow doctor")}\` to repair it, then re-run configure.`);
				runtime.exit(1);
				return;
			}
		}
		...
		const localProbe = await probeGatewayReachable({
			url: localUrl,
			token: baseConfig.gateway?.auth?.token ?? process.env.POWPOW_GATEWAY_TOKEN,
			password: baseConfig.gateway?.auth?.password ?? process.env.POWPOW_GATEWAY_PASSWORD
		});
		...
		let gatewayToken = nextConfig.gateway?.auth?.token ?? baseConfig.gateway?.auth?.token ?? process.env.POWPOW_GATEWAY_TOKEN;
		...
		const newPassword = nextConfig.gateway?.auth?.password ?? process.env.POWPOW_GATEWAY_PASSWORD;
		const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.POWPOW_GATEWAY_PASSWORD;
		const token = nextConfig.gateway?.auth?.token ?? process.env.POWPOW_GATEWAY_TOKEN;
