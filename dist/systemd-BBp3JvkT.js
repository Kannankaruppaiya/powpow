...
import { d as resolveGatewayServiceDescription, f as resolveGatewaySystemdServiceName, i as LEGACY_GATEWAY_SYSTEMD_SERVICE_NAMES, u as resolveGatewayProfileSuffix } from "./constants-Clw8zvXe.js";
...
async function isSystemctlAvailable() {
	const res = await execSystemctl(["--user", "status"]);
	if (res.code === 0) return true;
	return !(res.stderr || res.stdout).toLowerCase().includes("not found");
}
