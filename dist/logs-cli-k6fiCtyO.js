...
import { c as formatLocalIsoWithOffset, m as clearActiveProgressLine } from "./subsystem-DXlyH4H4.js";
...
function createLogWriters() {
	const writer = createSafeStreamWriter({
		...
		onBrokenPipe: (err, stream) => {
			...
			const message = `PowPow logs: output ${stream === process.stdout ? "stdout" : "stderr"} closed (${code}). Stopping tail.`;
			try {
...
function emitGatewayError(err, opts, mode, rich, emitJsonLine, errorLine) {
	...
	const hint = `Hint: run \`${formatCliCommand("PowPow doctor")}\`.`;
