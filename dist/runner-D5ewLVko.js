import { F as shouldLogVerbose, M as logVerbose, et as resolvePreferredPowPowTmpDir } from "./utils-C9Yd70Dr.js";
import { M as resolveAgentModelFallbackValues, N as resolveAgentModelPrimaryValue } from "./agent-scope-Coqt3COl.js";
import { n as runExec } from "./exec-7lv95ExR.js";
import { X as resolveApiKeyForProvider, Y as requireApiKey, k as normalizeGoogleModelId, u as normalizeProviderId } from "./model-selection-DAT49TVG.js";
import { _ as mergeInboundPathRoots, g as isInboundPathAllowed, h as DEFAULT_IMESSAGE_ATTACHMENT_ROOTS, v as resolveIMessageAttachmentRoots } from "./config-ClR1jZcN.js";
import { t as normalizeChatType } from "./chat-type-DLInEbxd.js";
import { t as describeImageWithModel } from "./image-BKPBLVoT.js";
import { r as formatUncaughtError, t as extractErrorCode } from "./errors-Adli4uDW.js";
import { c as detectMime, f as isAudioFileName, m as kindFromMime, u as getFileExtension } from "./image-ops-DDebYeLI.js";
import { n as fetchWithTimeout } from "./fetch-timeout-DiBmbZ6Z.js";
import { t as fetchWithSsrFGuard } from "./fetch-guard-C82rfIQY.js";
import { n as executeWithApiKeyRotation, r as parseGeminiAuth, t as collectProviderApiKeysForExecution } from "./api-key-rotation-ic-5DL61.js";
import { i as fetchRemoteMedia, n as getDefaultMediaLocalRoots, r as MediaFetchError } from "./local-roots-DMThNrwL.js";
import { n as loadModelCatalog, r as modelSupportsVision, t as findModelInCatalog } from "./model-catalog-BzI947Bm.js";
...
function isUnhandledRejectionHandled(reason) {
	for (const handler of handlers) try {
		...
		console.error("[powpow] Unhandled rejection handler failed:", err instanceof Error ? err.stack ?? err.message : err);
	}
	return false;
}
function installUnhandledRejectionHandler() {
	process$1.on("unhandledRejection", (reason, _promise) => {
		...
		if (isAbortError(reason)) {
			console.warn("[powpow] Suppressed AbortError:", formatUncaughtError(reason));
			return;
		}
		if (isFatalError(reason)) {
			console.error("[powpow] FATAL unhandled rejection:", formatUncaughtError(reason));
			process$1.exit(1);
			return;
		}
		if (isConfigError(reason)) {
			console.error("[powpow] CONFIGURATION ERROR - requires fix:", formatUncaughtError(reason));
			process$1.exit(1);
			return;
		}
		if (isTransientNetworkError(reason)) {
			console.warn("[powpow] Non-fatal unhandled rejection (continuing):", formatUncaughtError(reason));
			return;
		}
		console.error("[powpow] Unhandled promise rejection:", formatUncaughtError(reason));
		process$1.exit(1);
	});
}
...
function resolveTempRoot(tmpDir) {
	return tmpDir ?? resolvePreferredPowPowTmpDir();
}
...
async function runCliEntry(params) {
	...
	const outputDir = await fs$1.mkdtemp(path.join(resolvePreferredPowPowTmpDir(), "powpow-media-cli-"));
