import { c as ensureDir, t as CONFIG_DIR } from "./utils-C9Yd70Dr.js";
...
function resolveWideAreaDiscoveryDomain(params) {
	...
	return normalizeWideAreaDomain(params?.configDomain ?? env.POWPOW_WIDE_AREA_DOMAIN ?? null);
}
...
function extractContentHash(zoneText) {
	return zoneText.match(/^\s*;\s*powpow-content-hash:\s*(\S+)\s*$/m)?.[1] ?? null;
}
...
function renderZone(opts) {
	const hostname = os.hostname().split(".")[0] ?? "PowPow";
	const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "PowPow");
	const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "powpow-gw");
	...
	records.push(`_powpow-gw._tcp IN PTR ${instanceLabel}._powpow-gw._tcp`);
	records.push(`${instanceLabel}._powpow-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
	records.push(`${instanceLabel}._powpow-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
	const contentBody = `${records.join("\n")}\n`;
	return `; powpow-content-hash: ${computeContentHash(`${records.map((line) => line === soaLine ? `@ IN SOA ns1 hostmaster SERIAL 7200 3600 1209600 60` : line).join("\n")}\n`)}\n${contentBody}`;
}
