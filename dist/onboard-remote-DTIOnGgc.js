...
import { n as resolveWideAreaDiscoveryDomain } from "./widearea-dns-B_wUmD_x.js";
...
function validateGatewayWebSocketUrl(value) {
	...
	if (!isSecureWebSocketUrl(trimmed)) return "Use wss:// for remote hosts, or ws://127.0.0.1/localhost via SSH tunnel.";
}
async function promptRemoteGatewayConfig(cfg, prompter) {
	...
	if (!hasBonjourTool) await prompter.note(["Bonjour discovery requires dns-sd (macOS) or avahi-browse (Linux).", "Docs: https://docs.PowPow.ai/gateway/discovery"].join("\n"), "Discovery");
	if (wantsDiscover) {
