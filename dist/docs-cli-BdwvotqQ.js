...
const SEARCH_TOOL = "https://docs.PowPow.ai/mcp.SearchPowPow";
...
async function docsSearchCommand(queryParts, runtime) {
	...
	if (!query) {
		const docs = formatDocsLink("/", "docs.PowPow.ai");
		if (isRich()) {
			runtime.log(`${theme.muted("Docs:")} ${docs}`);
			runtime.log(`${theme.muted("Search:")} ${formatCliCommand("PowPow docs \"your query\"")}`);
		} else {
			runtime.log("Docs: https://docs.PowPow.ai/");
			runtime.log(`Search: ${formatCliCommand("PowPow docs \"your query\"")}`);
		}
		return;
	}
...
function registerDocsCli(program) {
	program.command("docs").description("Search the live PowPow docs").argument("[query...]", "Search query").addHelpText("after", () => `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/docs", "docs.PowPow.ai/cli/docs")}\n`).action(async (queryParts) => {
