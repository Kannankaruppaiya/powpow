...
import { S as listChatChannels, b as formatChannelSelectionLine, t as createSubsystemLogger, y as formatChannelPrimerLine } from "./subsystem-DXlyH4H4.js";
import { m as pathExists, p as normalizeE164, t as CONFIG_DIR } from "./utils-BWJH7VRh.js";
...
import { C as resolveDiscordAccount, S as resolveDefaultDiscordAccountId, b as listDiscordAccountIds, d as resolveTelegramAccount, h as resolveSlackAccount, l as listTelegramAccountIds, m as resolveDefaultSlackAccountId, n as listChannelPlugins, p as listSlackAccountIds, t as getChannelPlugin, u as resolveDefaultTelegramAccountId } from "./plugins-CPaMeorQ.js";
import { a as resolveWhatsAppAuthDir, n as listWhatsAppAccountIds, r as resolveDefaultWhatsAppAccountId } from "./accounts-CafntguV.js";
import { n as resolveDefaultIMessageAccountId, r as resolveIMessageAccount, t as listIMessageAccountIds } from "./accounts--1-XHGvw.js";
import { i as resolveSignalAccount, n as listSignalAccountIds, r as resolveDefaultSignalAccountId } from "./accounts-DUEZIYFu.js";
import { dt as normalizeDiscordSlug } from "./send-BglfZujd.js";
import { An as resolveDiscordUserAllowlist, T as resolveDiscordChannelAllowlist, _ as loadPowPowPlugins, g as createPluginLoaderLogger, kn as resolveSlackUserAllowlist, y as resolveSlackChannelAllowlist } from "./subagent-registry-DXZuSN83.js";
import { a as listChannelPluginCatalogEntries, n as isChannelConfigured } from "./plugin-auto-enable-CRRq2Feu.js";
import { s as normalizeIMessageHandle } from "./send-BzCREhP7.js";
import { t as formatDocsLink } from "./links-C00eySj4.js";
import { r as detectBinary } from "./onboard-helpers-BNEaJ_0q.js";
import { t as loginWeb } from "./login-sH7j9cli.js";
import { t as resolveBrewExecutable } from "./brew-Ber9DJ6h.js";
...
import { a as extractArchive } from "./install-safe-path-DoxY3Jl3.js";
import { n as recordPluginInstall, r as installPluginFromNpmSpec, t as buildNpmResolutionInstallFields } from "./installs-DNXwXVSR.js";
import { t as fetchTelegramChatId } from "./api-COdweLzv.js";
...
async function noteChannelPrimer(prompter, channels) {
	...
	await prompter.note([
		...
		`Approve with: ${formatCliCommand("PowPow pairing approve <channel> <code>")}`,
		"Public DMs require dmPolicy=\"open\" + allowFrom=[\"*\"].",
		"Multi-user DMs: run: " + formatCliCommand("PowPow config set session.dmScope \"per-channel-peer\"") + " (or \"per-account-channel-peer\" for multi-account channels) to isolate sessions.",
...
async function maybeConfigureDmPolicies(params) {
	...
	const selectPolicy = async (policy) => {
		await prompter.note([
			...
			`Approve: ${formatCliCommand(`PowPow pairing approve ${policy.channel} <code>`)}`,
	...
	const ensureBundledPluginEnabled = async (channel) => {
		...
		if (!getChannelPlugin(channel)) {
			if (getChannelOnboardingAdapter(channel)) {
				await prompter.note(`${channel} plugin not available (continuing with onboarding). If the channel still doesn't work after setup, run \`${formatCliCommand("PowPow plugins list")}\` and \`${formatCliCommand("PowPow plugins enable " + channel)}\`, then restart the gateway.`, "Channel setup");
	...
	if (options?.quickstartDefaults) {
		...
		const choice = await prompter.select({
			...
			options: [...buildSelectionOptions(entries), {
				value: "__skip__",
				label: "Skip for now",
				hint: `You can add channels later via \`${formatCliCommand("PowPow channels add")}\``
			}],
