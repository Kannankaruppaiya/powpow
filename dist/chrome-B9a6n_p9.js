import { T as CONFIG_DIR, t as createSubsystemLogger, ut as resolvePreferredPowPowTmpDir } from "./subsystem-J2JHz_PH.js";
import { n as loadConfig } from "./config-ctWck2-F.js";
import { a as resolvePinnedHostnameWithPolicy, s as isLoopbackIpAddress } from "./ssrf-PljZpPCf.js";
import { n as isPathInside, t as isNotFoundPathError } from "./path-guards-BWONywFu.js";
import { i as isErrno } from "./errors-D9rAZNPr.js";
import { n as openFileWithinRoot, t as SafeOpenError } from "./fs-safe-C47TU8nv.js";
...
const DEFAULT_POWPOW_BROWSER_ENABLED = true;
const DEFAULT_BROWSER_EVALUATE_ENABLED = true;
const DEFAULT_POWPOW_BROWSER_COLOR = "#FF4500";
const DEFAULT_POWPOW_BROWSER_PROFILE_NAME = "PowPow";
...
const DEFAULT_BROWSER_TMP_DIR = resolvePreferredPowPowTmpDir();
...
const RELAY_TOKEN_CONTEXT = "powpow-extension-relay-v1";
const DEFAULT_RELAY_PROBE_TIMEOUT_MS = 500;
const POWPOW_RELAY_BROWSER = "PowPow/extension-relay";
function resolveGatewayAuthToken() {
	const envToken = process.env.POWPOW_GATEWAY_TOKEN?.trim() || process.env.powpow_GATEWAY_TOKEN?.trim();
...
function resolveRelayAcceptedTokensForPort(port) {
	...
	if (!gatewayToken) throw new Error("extension relay requires gateway auth token (set gateway.auth.token or POWPOW_GATEWAY_TOKEN)");
...
async function probeAuthenticatedPowPowRelay(params) {
	...
	try {
		...
		return (typeof body?.Browser === "string" ? body.Browser.trim() : "") === POWPOW_RELAY_BROWSER;
	} catch {
		return false;
	} finally {
		clearTimeout(timer);
...
const RELAY_AUTH_HEADER = "x-powpow-relay-token";
...
async function ensureChromeExtensionRelayServer(opts) {
	...
	const routeCdpCommand = async (cmd) => {
		switch (cmd.method) {
			case "Browser.getVersion": return {
				protocolVersion: "1.3",
				product: "Chrome/powpow-Extension-Relay",
				revision: "0",
				userAgent: "powpow-Extension-Relay",
				jsVersion: "V8"
			};
	...
	const server = createServer((req, res) => {
		...
		if ((path === "/json/version" || path === "/json/version/") && (req.method === "GET" || req.method === "PUT")) {
			const payload = {
				Browser: "PowPow/extension-relay",
	...
	try {
		...
		if (isAddrInUseError(err) && await probeAuthenticatedPowPowRelay({
...
function decoratedMarkerPath(userDataDir) {
	return path.join(userDataDir, ".powpow-profile-decorated");
}
...
*/
function decoratePowPowProfile(userDataDir, opts) {
	const desiredName = opts?.name ?? DEFAULT_POWPOW_BROWSER_PROFILE_NAME;
	const desiredColor = (opts?.color ?? DEFAULT_POWPOW_BROWSER_COLOR).toUpperCase();
...
function resolvePowPowUserDataDir(profileName = DEFAULT_POWPOW_BROWSER_PROFILE_NAME) {
...
async function launchPowPowChrome(resolved, profile) {
	...
	const needsDecorate = !isProfileDecorated(userDataDir, profile.name, (profile.color ?? DEFAULT_POWPOW_BROWSER_COLOR).toUpperCase());
	const spawnOnce = () => {
		const args = [
	...
	if (needsDecorate) try {
		decoratePowPowProfile(userDataDir, {
			name: profile.name,
			color: profile.color
		});
		log.info(`🦞 PowPow browser profile decorated (${profile.color})`);
	} catch (err) {
		log.warn(`PowPow browser profile decoration failed: ${String(err)}`);
	}
	try {
		ensureProfileCleanExit(userDataDir);
	} catch (err) {
		log.warn(`PowPow browser clean-exit prefs failed: ${String(err)}`);
	}
	...
	log.info(`🦞 PowPow browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`);
	return {
		pid,
		exe,
		userDataDir,
...
async function stopPowPowChrome(running, timeoutMs = 2500) {
...
//#endregion
export { resolveWritablePathWithinRoot as A, DEFAULT_POWPOW_BROWSER_PROFILE_NAME as B, stopChromeExtensionRelayServer as C, DEFAULT_UPLOAD_DIR as D, DEFAULT_TRACE_DIR as E, DEFAULT_AI_SNAPSHOT_MAX_CHARS as F, DEFAULT_BROWSER_DEFAULT_PROFILE_NAME as I, DEFAULT_BROWSER_EVALUATE_ENABLED as L, isSecureWebSocketUrl as M, DEFAULT_AI_SNAPSHOT_EFFICIENT_DEPTH as N, resolveExistingPathsWithinRoot as O, DEFAULT_AI_SNAPSHOT_EFFICIENT_MAX_CHARS as P, DEFAULT_POWPOW_BROWSER_COLOR as R, ensureChromeExtensionRelayServer as S, DEFAULT_DOWNLOAD_DIR as T, appendCdpPath as _, resolvePowPowUserDataDir as a, getHeadersWithAuth as b, captureScreenshot as c, normalizeCdpWsUrl as d, snapshotAria as f, withBrowserNavigationPolicy as g, assertBrowserNavigationResultAllowed as h, launchPowPowChrome as i, isLoopbackHost as j, resolveStrictExistingPathsWithinRoot as k, createTargetViaCdp as l, assertBrowserNavigationAllowed as m, isChromeCdpReady as n, stopPowPowChrome as o, InvalidBrowserNavigationUrlError as p, isChromeReachable as r, resolveBrowserExecutableForPlatform as s, getChromeWebSocketUrl as t, formatAriaSnapshot as u, fetchJson as v, rawDataToString as w, withCdpSocket as x, fetchOk as y, DEFAULT_POWPOW_BROWSER_ENABLED as z };