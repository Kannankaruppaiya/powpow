...
function appendPowHubHint(output, json) {
	if (json) return output;
	return `${output}\n\nTip: use \`npx PowHub\` to search, install, and sync skills.`;
}
...
function formatSkillsList(report, opts) {
	...
	if (skills.length === 0) return appendPowHubHint(opts.eligible ? `No eligible skills found. Run \`${formatCliCommand("PowPow skills list")}\` to see all skills.` : "No skills found.", opts.json);
	...
	return appendPowHubHint(lines.join("\n"), opts.json);
}
function formatSkillInfo(report, skillName, opts) {
	...
	if (!skill) {
		...
		return appendPowHubHint(`Skill "${skillName}" not found. Run \`${formatCliCommand("PowPow skills list")}\` to see available skills.`, opts.json);
	}
...
async function loadSkillsStatusReport() {
	...
	const { buildWorkspaceSkillStatus } = await import("./skills-status-TqUhMg2L.js").then((n) => n.n);
