...
import { n as resolvePowPowPackageRootSync, t as resolvePowPowPackageRoot } from "./powpow-root-BraSy6pq.js";
...
async function resolveControlUiDistIndexPath(argv1OrOpts) {
	...
	for (let i = 0; i < 8; i++) {
		...
		if (fs.existsSync(pkgJsonPath)) try {
			...
			if (JSON.parse(raw).name === "PowPow") return fs.existsSync(indexPath) ? indexPath : null;
			return null;
		} catch {
			return null;
		}
