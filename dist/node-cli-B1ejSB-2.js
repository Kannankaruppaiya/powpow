import { d as resolveIsNixMode, g as resolveStateDir } from "./paths-BJtes5yu.js";
import { f as defaultRuntime } from "./subsystem-DXlyH4H4.js";
import { L as colorize, z as theme } from "./utils-BWJH7VRh.js";
import "./boolean-YY6K2DFz.js";
import "./auth-profiles-KPMal-k-.js";
import { t as formatCliCommand } from "./command-format-DRH7uGKn.js";
import { i as resolveAgentConfig } from "./agent-scope-Ac0Vygcn.js";
import "./exec-Du-v-jcI.js";
import "./github-copilot-token-CgHPGmmS.js";
import { a as sanitizeSystemRunEnvOverrides, i as sanitizeHostExecEnv } from "./host-env-security-DkeGvzp3.js";
import "./manifest-registry-C9O9KUb3.js";
import { X as analyzeArgvCommand, i as loadConfig } from "./config-B91e8u6L.js";
...
import { Zt as loadOrCreateDeviceIdentity, t as GatewayClient } from "./client-FngVpCQH.js";
import "./pairing-token-DdQlEK2o.js";
import "./net-CliLiAjH.js";
import "./ip-cpmUu6M4.js";
import "./tailnet-DVxZdYPM.js";
import { a as createBrowserControlContext, i as createBrowserRouteDispatcher, o as startBrowserControlServiceFromConfig, r as resolveSystemRunCommand, s as getMachineDisplayName, t as withTimeout } from "./with-timeout-BP0yuB9G.js";
...
import { i as resolveBrowserConfig } from "./server-context-B-VoITNb.js";
...
import { n as evaluateShellAllowlist, r as resolveAllowAlwaysPatterns, t as evaluateExecAllowlist } from "./exec-approvals-allowlist-Bydyw6jF.js";
import { n as resolveExecSafeBinRuntimePolicy } from "./exec-safe-bin-runtime-policy-C5YP9uai.js";
import { a as mergeExecApprovalsSocketDefaults, c as readExecApprovalsSnapshot, d as resolveExecApprovals, l as recordAllowlistUse, m as requestJsonlSocket, n as addAllowlistEntry, p as saveExecApprovals, r as ensureExecApprovals, s as normalizeExecApprovals, u as requiresExecApproval } from "./exec-approvals-Bs-EcVjz.js";
import { _ as resolveNodeWindowsTaskName, c as formatNodeServiceDescription, g as resolveNodeSystemdServiceName, h as resolveNodeLaunchAgentLabel } from "./constants-Clw8zvXe.js";
...
import { a as buildNodeServiceEnvironment, i as emitNodeRuntimeWarning, p as resolveNodeProgramArguments, r as resolveGatewayDevMode, u as resolvePreferredNodePath } from "./daemon-install-helpers-COWxlOD2.js";
...
import { s as resolveGatewayLogPaths } from "./service-DkDZ7shP.js";
import { a as createCliStatusTextStyles, f as resolveRuntimeStatusColor, g as installDaemonServiceAndEmit, h as createDaemonActionContext, i as runServiceUninstall, m as buildDaemonServiceSnapshot, r as runServiceStop, t as runServiceRestart } from "./lifecycle-core-Kl2d7KL2.js";
import { s as formatRuntimeStatus } from "./systemd-hints-Cfyn9qjl.js";
import { t as parsePort } from "./parse-port-BwqGYves.js";
...
function isPathLikeExecutableToken(value) {
	if (!value) return false;
	if (value.startsWith(".") || value.startsWith("/") || value.startsWith("\\")) return true;
	if (value.includes("/") || value.includes("\\")) return true;
	if (process.platform === "win32" && /^[a-zA-Z]:[\\/]/.test(value)) return true;
	return false;
}
function hardenApprovedExecutionPaths(params) {
	...
	if (params.shellCommand !== null || params.argv.length === 0) return {
		ok: true,
		argv: params.argv,
		cwd: hardenedCwd
	};
	...
	if (!isPathLikeExecutableToken(rawExecutable)) return {
		ok: true,
		argv,
		cwd: hardenedCwd
	};
...
const execHostEnforced = process.env.POWPOW_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed = process.env.POWPOW_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function resolveExecutablePathFromEnv(bin, pathEnv) {
	...
	const extensions = process.platform === "win32" ? hasExtension ? [""] : (process.env.PATHEXT ?? process.env.PathExt ?? ".EXE;.CMD;.BAT;.COM").split(";").map((ext) => ext.toLowerCase()) : [""];
...
function ensureNodePathEnv() {
	ensurePowPowCliOnPath({ pathEnv: process.env.PATH ?? "" });
...
async function runNodeHost(opts) {
	...
	const token = process.env.POWPOW_GATEWAY_TOKEN?.trim() || (isRemoteMode ? cfg.gateway?.remote?.token : cfg.gateway?.auth?.token);
	const password = process.env.POWPOW_GATEWAY_PASSWORD?.trim() || (isRemoteMode ? cfg.gateway?.remote?.password : cfg.gateway?.auth?.password);
...
async function buildNodeInstallPlan(params) {
	...
	return {
		...
		workingDirectory,
		environment,
		description: formatNodeServiceDescription({ version: environment.POWPOW_SERVICE_VERSION })
	};
}
...
function renderNodeServiceStartHints() {
	const base = [formatCliCommand("PowPow node install"), formatCliCommand("PowPow node start")];
...
async function runNodeDaemonInstall(opts) {
	...
	if (loaded && !opts.force) {
		...
		if (!json) {
			...
			defaultRuntime.log(`Reinstall with: ${formatCliCommand("PowPow node install --force")}`);
		}
		return;
	}
...
async function runNodeDaemonStatus(opts = {}) {
	...
	const hintEnv = {
		...baseEnv,
		POWPOW_LOG_PREFIX: baseEnv.POWPOW_LOG_PREFIX ?? "node"
	};
...
function registerNodeCli(program) {
	const node = program.command("node").description("Run and manage the headless node host service").addHelpText("after", () => `\n${theme.heading("Examples:")}\n${formatHelpExamples([
		["PowPow node run --host 127.0.0.1 --port 18789", "Run the node host in the foreground."],
		["PowPow node status", "Check node host service status."],
		["PowPow node install", "Install the node host service."],
		["PowPow node restart", "Restart the installed node host service."]
	])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.PowPow.ai/cli/node")}\n`);
