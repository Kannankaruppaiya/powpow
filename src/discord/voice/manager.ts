...
function resolveVoiceTtsConfig(params: { cfg: PowPowConfig; override?: TtsConfig }): {
  cfg: PowPowConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredPowPowTmpDir(), "discord-voice-"));
