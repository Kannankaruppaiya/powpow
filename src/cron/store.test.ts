...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses POWPOW_HOME for tilde expansion", () => {
    vi.stubEnv("POWPOW_HOME", "/srv/powpow-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/powpow-home", "cron", "jobs.json"));
  });
});
