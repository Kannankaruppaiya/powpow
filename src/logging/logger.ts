...
import type { PowPowConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredPowPowTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "PowPow.log"); // legacy single-file path
...
function resolveSettings(): ResolvedSettings {
  let cfg: PowPowConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => PowPowConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.POWPOW_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (nextBytes > settings.maxFileBytes) {
        if (!warnedAboutSizeCap) {
          ...
          process.stderr.write(
            `[powpow] log file size cap reached; suppressing writes file=${settings.file} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
        return;
      }
