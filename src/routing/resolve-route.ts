...
function listAgents(cfg: PowPowConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: PowPowConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: PowPowConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<PowPowConfig, EvaluatedBindingsCache>();
