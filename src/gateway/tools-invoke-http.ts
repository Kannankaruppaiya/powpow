...
import { createPowPowTools } from "../agents/powpow-tools.js";
import {
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-powpow-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-powpow-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-powpow-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-powpow-thread-id")?.trim() || undefined;
  ...
  const allTools = createPowPowTools({
