...
function cfg(input: Partial<PowPowConfig>): PowPowConfig {
  return input as PowPowConfig;
}

type ResolveFromConfigInput = Parameters<typeof resolveGatewayCredentialsFromConfig>[0];
type GatewayConfig = NonNullable<PowPowConfig["gateway"]>;
...
const DEFAULT_GATEWAY_ENV = {
  POWPOW_GATEWAY_TOKEN: "env-token",
  POWPOW_GATEWAY_PASSWORD: "env-password",
...
describe("resolveGatewayCredentialsFromConfig", () => {
  ...
  it("can disable legacy powpow env fallback", () => {
    const resolved = resolveGatewayCredentialsFromConfig({
      cfg: cfg({
        gateway: {
          mode: "local",
      ...
      env: {
        powpow_GATEWAY_TOKEN: "legacy-token",
        powpow_GATEWAY_PASSWORD: "legacy-password",
      } as NodeJS.ProcessEnv,
