...
export function resolveHooksConfig(cfg: PowPowConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: PowPowConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-powpow-token"] === "string"
      ? req.headers["x-powpow-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
