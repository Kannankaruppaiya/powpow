...
const { createPowPowTools } = await import("../agents/powpow-tools.js");
...
type SessionSendTool = ReturnType<typeof createPowPowTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createPowPowTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["POWPOW_GATEWAY_PORT", "POWPOW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.POWPOW_GATEWAY_PORT = String(gatewayPort);
  process.env.POWPOW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      const configPath = process.env.POWPOW_CONFIG_PATH;
      if (!configPath) {
        throw new Error("POWPOW_CONFIG_PATH missing in gateway test environment");
      }
