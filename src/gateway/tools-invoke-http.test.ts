...
let lastCreatePowPowToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/powpow-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreatePowPowToolsContext?.agentTo,
          agentThreadId: lastCreatePowPowToolsContext?.agentThreadId,
        },
      }),
    },
    {
  ...
  return {
    createPowPowTools: (ctx: Record<string, unknown>) => {
      lastCreatePowPowToolsContext = ctx;
      return tools;
    },
  };
});
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreatePowPowToolsContext = undefined;
});
...
describe("POST /tools/invoke", () => {
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-powpow-message-to": "channel:24514",
        "x-powpow-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
