...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: PowPowConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: PowPowConfig) {
  ...
  async function expectNoTokenGeneration(cfg: PowPowConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          POWPOW_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
