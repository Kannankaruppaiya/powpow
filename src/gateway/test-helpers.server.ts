...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "POWPOW_STATE_DIR",
  "POWPOW_CONFIG_PATH",
  "POWPOW_SKIP_BROWSER_CONTROL_SERVER",
  "POWPOW_SKIP_GMAIL_WATCHER",
  "POWPOW_SKIP_CANVAS_HOST",
  "POWPOW_BUNDLED_PLUGINS_DIR",
  "POWPOW_SKIP_CHANNELS",
  "POWPOW_SKIP_PROVIDERS",
  "POWPOW_SKIP_CRON",
  "POWPOW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.POWPOW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.POWPOW_SKIP_GMAIL_WATCHER = "1";
  process.env.POWPOW_SKIP_CANVAS_HOST = "1";
  process.env.POWPOW_SKIP_CHANNELS = "1";
  process.env.POWPOW_SKIP_PROVIDERS = "1";
  process.env.POWPOW_SKIP_CRON = "1";
  process.env.POWPOW_TEST_MINIMAL_GATEWAY = "1";
  process.env.POWPOW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "powpow-test-no-bundled-extensions")
    : "powpow-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "powpow-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".powpow-test");
...
const CONNECT_CHALLENGE_NONCE_KEY = "__PowPowTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__PowPowTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["POWPOW_GATEWAY_TOKEN"]);
  const prev = process.env.POWPOW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.POWPOW_GATEWAY_TOKEN = fallbackToken;
  }
