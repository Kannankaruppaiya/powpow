...
import { resolvePowPowAgentDir } from "../agents/agent-paths.js";
...
import { ensurePowPowModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, PowPowConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.POWPOW_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.POWPOW_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.POWPOW_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.POWPOW_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<PowPowConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.POWPOW_CONFIG_PATH,
    token: process.env.POWPOW_GATEWAY_TOKEN,
    skipChannels: process.env.POWPOW_SKIP_CHANNELS,
    skipGmail: process.env.POWPOW_SKIP_GMAIL_WATCHER,
    skipCron: process.env.POWPOW_SKIP_CRON,
    skipCanvas: process.env.POWPOW_SKIP_CANVAS_HOST,
    agentDir: process.env.POWPOW_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.POWPOW_STATE_DIR,
  };
  let tempAgentDir: string | undefined;
  let tempStateDir: string | undefined;

  process.env.POWPOW_SKIP_CHANNELS = "1";
  process.env.POWPOW_SKIP_GMAIL_WATCHER = "1";
  process.env.POWPOW_SKIP_CRON = "1";
  process.env.POWPOW_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.POWPOW_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolvePowPowAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-live-state-"));
  process.env.POWPOW_STATE_DIR = tempStateDir;
  ...
  process.env.POWPOW_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.powpow-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolvePowPowAgentDir();
  const sanitizedCfg: PowPowConfig = {
    ...params.cfg,
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-live-"));
  const tempConfigPath = path.join(tempDir, "PowPow.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.POWPOW_CONFIG_PATH = tempConfigPath;

  await ensurePowPowModelsJson(nextCfg);
  ...
  try {
          ...
          for (
            ...
            const toolProbe = await client.request<AgentFinalPayload>(
              "agent",
              {
                ...
                message: strictReply
                  ? "PowPow live tool probe (local, safe): " +
    ...
    process.env.POWPOW_CONFIG_PATH = previous.configPath;
    process.env.POWPOW_GATEWAY_TOKEN = previous.token;
    process.env.POWPOW_SKIP_CHANNELS = previous.skipChannels;
    process.env.POWPOW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.POWPOW_SKIP_CRON = previous.skipCron;
    process.env.POWPOW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.POWPOW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.POWPOW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.POWPOW_LIVE_GATEWAY_MODELS?.trim();
      const useModern = !rawModels || rawModels === "modern" || rawModels === "all";
      const useExplicit = Boolean(rawModels) && !useModern;
      const filter = useExplicit ? parseFilter(rawModels) : null;
      const maxModels = toInt(process.env.POWPOW_LIVE_GATEWAY_MAX_MODELS, 0);
      const wanted = filter
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via POWPOW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.powpow-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
