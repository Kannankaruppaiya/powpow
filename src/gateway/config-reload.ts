...
import type { PowPowConfig, ConfigFileSnapshot, GatewayReloadMode } from "../config/config.js";
...
export function resolveGatewayReloadSettings(cfg: PowPowConfig): GatewayReloadSettings {
...
export function startGatewayConfigReloader(opts: {
  initialConfig: PowPowConfig;
  readSnapshot: () => Promise<ConfigFileSnapshot>;
  onHotReload: (plan: GatewayReloadPlan, nextConfig: PowPowConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: PowPowConfig) => void;
  log: {
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: PowPowConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
  ...
  const applySnapshot = async (nextConfig: PowPowConfig) => {
