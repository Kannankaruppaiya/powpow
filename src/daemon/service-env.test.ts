...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.POWPOW_GATEWAY_PORT).toBe("18789");
    expect(env.POWPOW_GATEWAY_TOKEN).toBe("secret");
    expect(env.POWPOW_SERVICE_MARKER).toBe("PowPow");
    expect(env.POWPOW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.POWPOW_SERVICE_VERSION).toBe("string");
    expect(env.POWPOW_SYSTEMD_UNIT).toBe("powpow-gateway.service");
    if (process.platform === "darwin") {
      expect(env.POWPOW_LAUNCHD_LABEL).toBe("ai.PowPow.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", POWPOW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.POWPOW_SYSTEMD_UNIT).toBe("powpow-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.POWPOW_LAUNCHD_LABEL).toBe("ai.PowPow.work");
    }
  });
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".PowPow"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", POWPOW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".powpow-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", POWPOW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".PowPow"));
  });

  it("uses POWPOW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", POWPOW_STATE_DIR: "/var/lib/PowPow" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/PowPow"));
  });

  it("expands ~ in POWPOW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", POWPOW_STATE_DIR: "~/powpow-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/powpow-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { POWPOW_STATE_DIR: "C:\\State\\PowPow" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\PowPow");
  });
});
