...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `powpow-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      POWPOW_LAUNCHD_LABEL: `ai.PowPow.launchd-int-${testId}`,
      POWPOW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
