...
declare const __POWPOW_VERSION__: string | undefined;
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["POWPOW_VERSION"],
      env["POWPOW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current PowPow version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION =
  (typeof __POWPOW_VERSION__ === "string" && __POWPOW_VERSION__) ||
  process.env.POWPOW_BUNDLED_VERSION ||
