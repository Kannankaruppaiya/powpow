...
describe("resolveGatewayConnection", () => {
  ...
  it("uses POWPOW_GATEWAY_TOKEN for local mode", () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });

    withEnv({ POWPOW_GATEWAY_TOKEN: "env-token" }, () => {
      const result = resolveGatewayConnection({});
  ...
  it("prefers POWPOW_GATEWAY_PASSWORD over remote password fallback", () => {
    ...
    withEnv({ POWPOW_GATEWAY_PASSWORD: "env-pass" }, () => {
      const result = resolveGatewayConnection({});
