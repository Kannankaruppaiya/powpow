...
describe("shared/frontmatter", () => {
  ...
  test("resolvePowPowManifestBlock parses JSON5 metadata and picks PowPow block", () => {
    const frontmatter = {
      metadata: "{ PowPow: { foo: 1, bar: 'baz' } }",
    };
    expect(resolvePowPowManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolvePowPowManifestBlock returns undefined for invalid input", () => {
    expect(resolvePowPowManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolvePowPowManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolvePowPowManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
