...
function resolveGatewayPort(cfg: PowPowConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: PowPowConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.POWPOW_GATEWAY_PASSWORD?.trim() ||
    env.powpow_GATEWAY_PASSWORD?.trim() ||
