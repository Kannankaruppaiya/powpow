...
function createSinglePluginWithEmptySlotsConfig(): PowPowConfig {
  return {
    plugins: {
...
function createSingleNpmInstallConfig(installPath: string): PowPowConfig {
  return {
    plugins: {
      entries: createSinglePluginEntries(),
      installs: {
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    const extensionsDir = path.join(os.tmpdir(), "powpow-uninstall-safe");
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        source: "npm",
        spec: "my-plugin@1.0.0",
        installPath: "/tmp/not-powpow-extensions/my-plugin",
      },
      extensionsDir,
    });
