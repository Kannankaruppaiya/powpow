...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: PowPowPluginDefinition;
  register?: PowPowPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as PowPowPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as PowPowPluginDefinition;
...
export function loadPowPowPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverPowPowPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "PowPow/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "PowPow/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: PowPowPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as PowPowPluginModule;
    } catch (err) {
      recordPluginError({
        logger,
        registry,
