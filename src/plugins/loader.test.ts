...
import { loadPowPowPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `powpow-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.POWPOW_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "PowPow.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.POWPOW_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadPowPowPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
function expectTelegramLoaded(registry: ReturnType<typeof loadPowPowPlugins>) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.POWPOW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.POWPOW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadPowPowPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadPowPowPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.POWPOW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ POWPOW_STATE_DIR: stateDir, POWPOW_STATE_DIR: undefined }, () => {
