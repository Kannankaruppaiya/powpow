...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "powpow-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    POWPOW_GATEWAY_TOKEN: "test-token",
    POWPOW_CONFIG_DIR: join(sandbox.rootDir, "config"),
    POWPOW_WORKSPACE_DIR: join(sandbox.rootDir, "PowPow"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      POWPOW_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      POWPOW_EXTRA_MOUNTS: undefined,
      POWPOW_HOME_VOLUME: "powpow-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("POWPOW_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("POWPOW_EXTRA_MOUNTS=");
    expect(envFile).toContain("POWPOW_HOME_VOLUME=powpow-home");
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("powpow-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("powpow-home:");
    const log = await readFile(activeSandbox.logPath, "utf8");
    expect(log).toContain("--build-arg POWPOW_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
  ...
  it("precreates config identity dir for CLI device auth writes", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      POWPOW_CONFIG_DIR: configDir,
      POWPOW_WORKSPACE_DIR: workspaceDir,
    });
  ...
  it("reuses existing config token when POWPOW_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("POWPOW_GATEWAY_TOKEN=config-token-123");
  });

  it("rejects injected multiline POWPOW_EXTRA_MOUNTS values", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      POWPOW_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("POWPOW_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid POWPOW_EXTRA_MOUNTS mount format", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      POWPOW_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid POWPOW_HOME_VOLUME names", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      POWPOW_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("POWPOW_HOME_VOLUME must match");
  });
