...
beforeAll(() => {
  envSnapshot = captureEnv(["POWPOW_PROFILE"]);
  process.env.POWPOW_PROFILE = "isolated";
});
...
vi.mock("../infra/powpow-root.js", () => ({
  resolvePowPowPackageRoot: vi.fn().mockResolvedValue("/tmp/PowPow"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/PowPow/pnpm-lock.yaml",
      markerPath: "/tmp/PowPow/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.PowPow.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.PowPow.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("PowPow status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("PowPow status --all") ||
          l.includes("PowPow --profile isolated status --all") ||
          l.includes("PowPow status --all") ||
          l.includes("PowPow --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.POWPOW_GATEWAY_TOKEN = "abcd1234";
    try {
