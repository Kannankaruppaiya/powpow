...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "POWPOW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("POWPOW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv POWPOW_GATEWAY_TOKEN");
    expect(message).not.toContain("POWPOW_GATEWAY_PASSWORD");
  });
