...
const ensurePowPowModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/powpow-agent/auth.json" });
const resolvePowPowAgentDir = vi.fn().mockReturnValue("/tmp/powpow-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/powpow-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensurePowPowModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/powpow-agent");
  });
