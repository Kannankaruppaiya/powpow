...
function resolveMode(cfg: PowPowConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  let cfg: PowPowConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    ...
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("PowPow setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("PowPow doctor --fix")}" to apply changes.`);
  }
