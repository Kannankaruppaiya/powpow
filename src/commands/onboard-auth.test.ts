...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under POWPOW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("powpow-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-oauth-sync-"));
    ...
    process.env.POWPOW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-oauth-nosync-"));
    process.env.POWPOW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside POWPOW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-oauth-external-"));
    process.env.POWPOW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* POWPOW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to POWPOW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("powpow-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
