...
import type { PowPowConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: PowPowConfig) {
  ...
  if (cfg.approvals?.exec?.enabled === false) {
    warnings.push(
      ...
      "  Host exec gating still comes from ~/.PowPow/exec-approvals.json.",
      `  Check local policy with: ${formatCliCommand("PowPow approvals get --gateway")}`,
    );
  }
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("PowPow configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("PowPow config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("PowPow doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("PowPow config set gateway.bind loopback")}`,
    ...
    if (dmScope === "main" && isMultiUserDm) {
      warnings.push(
        `- ${params.label} DMs: multiple senders share the main session; run: ` +
          formatCliCommand('PowPow config set session.dmScope "per-channel-peer"') +
