...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("PowPow models auth setup-token")}\` or \`${formatCliCommand("PowPow configure")}\`.`
          : `Run \`${formatCliCommand("PowPow configure")}\` or set an API key env var.`;
