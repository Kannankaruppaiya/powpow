...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolvePowPowAgentDir: vi.fn().mockReturnValue("/tmp/powpow-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/powpow-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/powpow-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolvePowPowAgentDir: mocks.resolvePowPowAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/powpow-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolvePowPowAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/powpow-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/powpow-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/powpow-agent-custom");
