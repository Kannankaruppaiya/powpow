...
export async function loadValidConfigOrThrow(): Promise<PowPowConfig> {
...
export async function updateConfig(
  mutator: (cfg: PowPowConfig) => PowPowConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: PowPowConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: PowPowConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("PowPow agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
