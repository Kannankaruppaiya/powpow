...
type PromptDefaultModelResult = { model?: string; config?: PowPowConfig };
...
function resolveConfiguredModelRaw(cfg: PowPowConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: PowPowConfig): string[] {
...
export function applyPrimaryModel(cfg: PowPowConfig, model: string): PowPowConfig {
...
export function applyModelAllowlist(cfg: PowPowConfig, models: string[]): PowPowConfig {
  const defaults = cfg.agents?.defaults;
