...
export async function statusCommand(
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`PowPow devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
  ...
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("PowPow status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("PowPow gateway probe")}`);
  }
}
