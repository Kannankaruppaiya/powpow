...
async function detectPowPowGitCheckout(root: string): Promise<"git" | "not-git" | "unknown"> {
...
export async function maybeOfferUpdateBeforeDoctor(params: {
  ...
  const updateInProgress = isTruthyEnvValue(process.env.POWPOW_UPDATE_IN_PROGRESS);
  ...
  if (git === "not-git") {
    note(
      [
        ...
        `Run \`${formatCliCommand("PowPow update")}\` to update via your package manager (npm/pnpm), then rerun doctor.`,
      ].join("\n"),
      "Update",
    );
  }
