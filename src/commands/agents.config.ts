...
function resolveAgentName(cfg: PowPowConfig, agentId: string) {
...
function resolveAgentModel(cfg: PowPowConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: PowPowConfig): AgentSummary[] {
