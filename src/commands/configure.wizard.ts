...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.POWPOW_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.POWPOW_GATEWAY_PASSWORD;
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "PowPow update wizard" : "PowPow configure");
    const prompter = createClackPrompter();

    const snapshot = await readConfigFileSnapshot();
    const baseConfig: PowPowConfig = snapshot.valid ? snapshot.config : {};
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("PowPow doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
