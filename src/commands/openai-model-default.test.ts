...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: PowPowConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: PowPowConfig) => { changed: boolean; next: PowPowConfig };
  defaultModel: string;
  overrideConfig: PowPowConfig;
  alreadyDefaultConfig: PowPowConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: PowPowConfig) => config,
      applyDefaultConfig: (config: PowPowConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: PowPowConfig = { agents: { defaults: {} } };
