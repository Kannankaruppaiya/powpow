...
const TEMP_MEDIA_PREFIX = "powpow-media-";
...
function createGroqAudioConfig(): PowPowConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): PowPowConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): PowPowConfig {
  return {
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: PowPowConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  ...
  beforeAll(async () => {
    const baseDir = resolvePreferredPowPowTmpDir();
  ...
  it("skips audio auto-detect when no supported binaries or provider keys are available", async () => {
    ...
    await withMediaAutoDetectEnv(
      {
        PATH: emptyBinDir,
        POWPOW_AGENT_DIR: isolatedAgentDir,
