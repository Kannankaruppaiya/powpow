...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("powpow-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "powpow-sandbox-browser-global" },
      agentBrowser: { network: "powpow-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("powpow-sandbox-browser-agent");
  });
