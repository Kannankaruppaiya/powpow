...
function isWhatsAppConfigured(cfg: PowPowConfig): boolean {
...
function isGenericChannelConfigured(cfg: PowPowConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: PowPowConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: PowPowConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: PowPowConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: PowPowConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: PowPowConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: PowPowConfig, pluginId: string): PowPowConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
