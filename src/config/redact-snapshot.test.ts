...
import type { ConfigFileSnapshot } from "./types.PowPow.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.PowPow/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/PowPow/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/PowPow/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/PowPow/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/PowPow/nickserv-password.txt");
  ...
  it("contract-covers dynamic catchall/record paths for redact+restore", () => {
    const hints = mapSensitivePaths(PowPowSchema, "", {});
    const snapshot = makeSnapshot({
      env: {
