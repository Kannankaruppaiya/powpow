...
import type { PowPowConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: PowPowConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: PowPowConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as PowPowConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as PowPowConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as PowPowConfig,
  };
}
