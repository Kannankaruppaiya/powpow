...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ POWPOW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-PowPow",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-PowPow#general");
  });
  ...
  it("derives session transcripts dir from POWPOW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { POWPOW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.PowPow");
  ...
  it("resolves cross-agent paths when POWPOW_STATE_DIR differs from stored paths", () => {
  ...
  it("falls back to derived transcript path when sessionFile is outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.PowPow"), () => {
      const sessionFile = resolveSessionFilePath(
        "sess-1",
