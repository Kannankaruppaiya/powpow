...
import type { PowPowConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { comparePowPowVersions } from "./version.js";
...
function unsetPathForWrite(
  root: PowPowConfig,
  pathSegments: string[],
): { changed: boolean; next: PowPowConfig } {
...
function coerceConfig(value: unknown): PowPowConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as PowPowConfig;
}
...
function stampConfigVersion(cfg: PowPowConfig): PowPowConfig {
...
function warnIfConfigFromFuture(cfg: PowPowConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer PowPow (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as PowPowConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): PowPowConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as PowPowConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: PowPowConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as PowPowConfig)
        : cfgToWrite;
    ...
    const auditRecordBase = {
      ...
      watchSession:
        typeof deps.env.POWPOW_WATCH_SESSION === "string" &&
        deps.env.POWPOW_WATCH_SESSION.trim().length > 0
          ? deps.env.POWPOW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.POWPOW_WATCH_COMMAND === "string" &&
        deps.env.POWPOW_WATCH_COMMAND.trim().length > 0
          ? deps.env.POWPOW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `POWPOW_CONFIG_PATH` (and friends) are expected to work even
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.POWPOW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function loadConfig(): PowPowConfig {
