...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["POWPOW_BUNDLED_HOOKS_DIR"]);
    process.env.POWPOW_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
