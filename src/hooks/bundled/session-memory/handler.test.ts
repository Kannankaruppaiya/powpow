...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => PowPowConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("powpow-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): PowPowConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
