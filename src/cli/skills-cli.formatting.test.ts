...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "powpow-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "powpow-bundled-skills-test-"));
    process.env.POWPOW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
