...
import type { PowPowConfig, ConfigFileSnapshot } from "../config/types.PowPow.js";
...
vi.mock("../infra/powpow-root.js", () => ({
  resolvePowPowPackageRoot: vi.fn(),
}));
...
const { resolvePowPowPackageRoot } = await import("../infra/powpow-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/powpow-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as PowPowConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/powpow-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolvePowPowPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("powpow-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolvePowPowPackageRoot).mockClear();
    ...
    vi.mocked(resolvePowPowPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("PowPow");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/powpow-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "powpow-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("PowPow update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as PowPowConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolvePowPowPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("powpow-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ POWPOW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.POWPOW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("powpow-update-wizard");
    await withEnvAsync({ POWPOW_GIT_DIR: tempDir }, async () => {
      setTty(true);
