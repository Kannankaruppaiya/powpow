...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.PowPow${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  if (!env.POWPOW_STATE_DIR?.trim()) {
    env.POWPOW_STATE_DIR = stateDir;
  }

  if (!env.POWPOW_CONFIG_PATH?.trim()) {
    env.POWPOW_CONFIG_PATH = path.join(stateDir, "PowPow.json");
  }

  if (profile === "dev" && !env.POWPOW_GATEWAY_PORT?.trim()) {
    env.POWPOW_GATEWAY_PORT = "19001";
  }
}
