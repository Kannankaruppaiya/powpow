...
export function registerDnsCli(program: Command) {
  ...
  dns
    ...
    .option("--domain <domain>", "Wide-area discovery domain (e.g. PowPow.internal)")
    .option(
      "--apply",
    ...
    .action(async (opts) => {
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in ~/.PowPow/PowPow.json on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
