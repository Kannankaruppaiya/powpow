...
function buildHooksReport(config: PowPowConfig): HookStatusReport {
...
function enableInternalHookEntries(config: PowPowConfig, hookNames: string[]): PowPowConfig {
...
export function formatHooksList(report: HookStatusReport, opts: HooksListOptions): string {
  ...
  if (hooks.length === 0) {
    const message = opts.eligible
      ? `No eligible hooks found. Run \`${formatCliCommand("PowPow hooks list")}\` to see all hooks.`
      : "No hooks found.";
    return message;
  }
