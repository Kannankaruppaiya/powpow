...
beforeEach(() => {
  ...
  originalHideBanner = process.env.POWPOW_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.POWPOW_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.POWPOW_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("PowPow");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "PowPow", "status", "--debug"],
    });
  ...
  it("loads plugin registry for configure command", async () => {
    await runCommand({
      parseArgv: ["configure"],
      processArgv: ["node", "PowPow", "configure"],
    });
  ...
  it("loads plugin registry for onboard command", async () => {
    await runCommand({
      parseArgv: ["onboard"],
      processArgv: ["node", "PowPow", "onboard"],
    });
  ...
  it("loads plugin registry for agents command", async () => {
    await runCommand({
      parseArgv: ["agents"],
      processArgv: ["node", "PowPow", "agents"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "PowPow", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "PowPow", "completion"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "PowPow", "--version"],
    });
  ...
  it("hides banner when POWPOW_HIDE_BANNER is truthy", async () => {
    process.env.POWPOW_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "PowPow", "status"],
    });
