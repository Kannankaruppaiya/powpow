...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@PowPow/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@PowPow/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@PowPow/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@PowPow/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @PowPow/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@PowPow/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@PowPow/plugin-alpha@1.2.3",
        installPath: "/tmp/PowPow/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@PowPow/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @PowPow/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@PowPow/plugin-alpha@latest",
      true,
      "/tmp/PowPow/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@PowPow/plugin-alpha@latest",
      installPath: "/tmp/PowPow/extensions/alpha",
      version: "1.2.3",
