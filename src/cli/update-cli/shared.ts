...
import { resolvePowPowPackageRoot } from "../../infra/powpow-root.js";
...
export const DEFAULT_PACKAGE_NAME = "PowPow";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("PowPow@")) {
    return trimmed.slice("PowPow@".length);
  }
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolvePowPowPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", POWPOW_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `POWPOW_GIT_DIR points at a non-git directory: ${params.dir}. Set POWPOW_GIT_DIR to an empty folder or an PowPow checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`POWPOW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
