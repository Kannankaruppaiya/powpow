...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    process.env.POWPOW_STATE_DIR = "/tmp/powpow-cli-state";
    process.env.POWPOW_CONFIG_PATH = "/tmp/powpow-cli-state/PowPow.json";
    delete process.env.POWPOW_GATEWAY_PORT;
    delete process.env.POWPOW_PROFILE;
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        POWPOW_PROFILE: "dev",
        POWPOW_STATE_DIR: "/tmp/powpow-daemon-state",
        POWPOW_CONFIG_PATH: "/tmp/powpow-daemon-state/PowPow.json",
        POWPOW_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.PowPow.gateway.plist",
    });
