...
function appendPowHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("PowPow skills list")}\` to see all skills.`
      : "No skills found.";
    return appendPowHubHint(message, opts.json);
  }
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendPowHubHint(
      `Skill "${skillName}" not found. Run \`${formatCliCommand("PowPow skills list")}\` to see available skills.`,
      opts.json,
    );
  }
