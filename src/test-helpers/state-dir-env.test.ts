...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    PowPow: process.env.POWPOW_STATE_DIR,
    legacy: process.env.POWPOW_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.POWPOW_STATE_DIR).toBe(snapshot.PowPow);
  expect(process.env.POWPOW_STATE_DIR).toBe(snapshot.legacy);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips POWPOW_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/powpow-state-dir-test");
    expect(process.env.POWPOW_STATE_DIR).toBe("/tmp/powpow-state-dir-test");
    expect(process.env.POWPOW_STATE_DIR).toBeUndefined();
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("powpow-state-dir-env-", async ({ tempRoot, stateDir }) => {
      capturedTempRoot = tempRoot;
      capturedStateDir = stateDir;
      expect(process.env.POWPOW_STATE_DIR).toBe(stateDir);
      expect(process.env.POWPOW_STATE_DIR).toBeUndefined();
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("powpow-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
  ...
  it("withStateDirEnv restores both env vars when legacy var was previously set", async () => {
    ...
    process.env.POWPOW_STATE_DIR = "/tmp/original-PowPow";
    process.env.POWPOW_STATE_DIR = "/tmp/original-legacy";
    const prev = snapshotCurrentStateDirVars();
