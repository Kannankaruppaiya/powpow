...
function summarizeGroupPolicy(cfg: PowPowConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: PowPowConfig): boolean {
...
function collectModels(cfg: PowPowConfig): ModelRef[] {
  const out: ModelRef[] = [];
  addModel(
    out,
...
function listKnownNodeCommands(cfg: PowPowConfig): Set<string> {
  const baseCfg: PowPowConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: PowPowConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: PowPowConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: PowPowConfig): boolean {
...
function isBrowserEnabled(cfg: PowPowConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: PowPowConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
function listPotentialMultiUserSignals(cfg: PowPowConfig): string[] {
  const out = new Set<string>();
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return [];
...
function collectRiskyToolExposureContexts(cfg: PowPowConfig): {
...
export function collectAttackSurfaceSummaryFindings(cfg: PowPowConfig): SecurityAuditFinding[] {
...
export function collectSecretsInConfigFindings(cfg: PowPowConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer POWPOW_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const PowPowGatewayToken =
    typeof env.POWPOW_GATEWAY_TOKEN === "string" && env.POWPOW_GATEWAY_TOKEN.trim()
      ? env.POWPOW_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-powpow-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: PowPowConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: PowPowConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
  ...
  if (browserExposurePaths.length > 0) {
    findings.push({
      ...
      remediation:
        "Set sandbox.browser.network to a dedicated bridge network (recommended default: powpow-sandbox-browser), " +
...
export function collectNodeDenyCommandPatternFindings(cfg: PowPowConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: PowPowConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: PowPowConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: PowPowConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectLikelyMultiUserSetupFindings(cfg: PowPowConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
