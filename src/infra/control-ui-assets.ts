...
import { resolvePowPowPackageRoot, resolvePowPowPackageRootSync } from "./powpow-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolvePowPowPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "PowPow" + dist/control-ui/index.html
