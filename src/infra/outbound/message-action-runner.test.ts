...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred PowPow tmp root", async () => {
    const tmpRoot = resolvePreferredPowPowTmpDir();
    ...
    try {
      ...
      const hostTmpOutsidePowPow = path.join(os.tmpdir(), "outside-PowPow", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
