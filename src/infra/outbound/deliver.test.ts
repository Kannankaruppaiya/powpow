...
import { resolvePreferredPowPowTmpDir } from "../tmp-powpow-dir.js";
...
const telegramChunkConfig: PowPowConfig = {
  channels: { telegram: { botToken: "tok-1", textChunkLimit: 2 } },
};

const whatsappChunkConfig: PowPowConfig = {
...
describe("deliverOutboundPayloads", () => {
  ...
  it("includes PowPow tmp root in telegram mediaLocalRoots", async () => {
    const sendTelegram = vi.fn().mockResolvedValue({ messageId: "m1", chatId: "c1" });
    ...
    expect(sendTelegram).toHaveBeenCalledWith(
      ...
      "hi",
      expect.objectContaining({
        mediaLocalRoots: expect.arrayContaining([resolvePreferredPowPowTmpDir()]),
      }),
    );
  });

  it("includes PowPow tmp root in signal mediaLocalRoots", async () => {
  ...
  it("includes PowPow tmp root in whatsapp mediaLocalRoots", async () => {
  ...
  it("includes PowPow tmp root in imessage mediaLocalRoots", async () => {
  ...
  it("uses signal media maxBytes from config", async () => {
    const sendSignal = vi.fn().mockResolvedValue({ messageId: "s1", timestamp: 123 });
    const cfg: PowPowConfig = { channels: { signal: { mediaMaxMb: 2 } } };
