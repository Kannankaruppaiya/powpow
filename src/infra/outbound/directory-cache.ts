...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: PowPowConfig | null = null;
  ...
  get(key: string, cfg: PowPowConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: PowPowConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: PowPowConfig): void {
