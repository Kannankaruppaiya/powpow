...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: PowPowConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: PowPowConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: PowPowConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: PowPowConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: PowPowConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: PowPowConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: PowPowConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
