...
function candidateBinDirs(opts: EnsurePowPowPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `PowPow` lives next to the executable (process.execPath).
  try {
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.POWPOW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "PowPow"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `PowPow` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensurePowPowCliOnPath(opts: EnsurePowPowPathOpts = {}) {
  if (isTruthyEnvValue(process.env.POWPOW_PATH_BOOTSTRAPPED)) {
    return;
  }
