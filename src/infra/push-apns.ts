...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const inlineKeyRaw =
    normalizeNonEmptyString(env.POWPOW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.POWPOW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set POWPOW_APNS_PRIVATE_KEY_P8 or POWPOW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading POWPOW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
