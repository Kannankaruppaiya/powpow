...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["POWPOW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.POWPOW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolvePowPowPackageRoot } = await import("./powpow-root.js"));
  ...
  function mockPackageUpdateStatus(tag = "latest", version = "2.0.0") {
    vi.mocked(resolvePowPowPackageRoot).mockResolvedValue("/opt/PowPow");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/PowPow/dist/entry.js"];
    try {
      await runGatewayUpdateCheck({
        cfg: {
          update: {
