...
let ensurePowPowCliOnPath: typeof import("./path-env.js").ensurePowPowCliOnPath;

describe("ensurePowPowCliOnPath", () => {
  const envKeys = [
    "PATH",
    "POWPOW_PATH_BOOTSTRAPPED",
    "POWPOW_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensurePowPowCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling PowPow exists", () => {
    const tmp = abs("/tmp/powpow-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "PowPow");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.POWPOW_PATH_BOOTSTRAPPED;
  ...
  it("is idempotent", () => {
    ...
    process.env.POWPOW_PATH_BOOTSTRAPPED = "1";
    ensurePowPowCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/powpow-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "PowPow");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/powpow-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "PowPow");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "PowPow");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/powpow-path/case-linuxbrew");
