...
describe("resolveEffectiveHomeDir", () => {
  it("prefers POWPOW_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands POWPOW_HOME when set to ~", () => {
    const env = {
      POWPOW_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for POWPOW_HOME", () => {
    const result = resolveRequiredHomeDir(
      { POWPOW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when POWPOW_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ POWPOW_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { POWPOW_HOME: "/srv/powpow-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/powpow-home")}/x`);
  });
