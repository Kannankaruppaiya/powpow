...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__POWPOW_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "powpow-root");
...
describe("resolvePowPowPackageRoot", () => {
  let resolvePowPowPackageRoot: typeof import("./powpow-root.js").resolvePowPowPackageRoot;
  let resolvePowPowPackageRootSync: typeof import("./powpow-root.js").resolvePowPowPackageRootSync;

  beforeAll(async () => {
    ({ resolvePowPowPackageRoot, resolvePowPowPackageRootSync } =
      await import("./powpow-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "PowPow");
    const pkgRoot = path.join(project, "node_modules", "PowPow");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "PowPow" }));

    expect(resolvePowPowPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "PowPow");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "PowPow.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "PowPow" }));

    expect(resolvePowPowPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolvePowPowPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-PowPow package roots", async () => {
    const pkgRoot = fx("not-PowPow");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-PowPow" }));

    expect(resolvePowPowPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolvePowPowPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolvePowPowPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
