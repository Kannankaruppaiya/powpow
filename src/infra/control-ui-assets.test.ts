...
vi.mock("./powpow-root.js", () => ({
  resolvePowPowPackageRoot: vi.fn(async () => null),
  resolvePowPowPackageRootSync: vi.fn(() => null),
}));
...
let PowPowRoot: typeof import("./powpow-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    PowPowRoot = await import("./powpow-root.js");
  });
  ...
  it("uses resolvePowPowPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/PowPow");
    (
      PowPowRoot.resolvePowPowPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/PowPow"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "PowPow" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "PowPow.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-PowPow");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/powpow-bundle");
    (
      PowPowRoot.resolvePowPowPackageRootSync as unknown as ReturnType<typeof vi.fn>
