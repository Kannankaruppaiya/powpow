...
function cfg(accounts?: Record<string, unknown> | null): PowPowConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as PowPowConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as PowPowConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as PowPowConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as PowPowConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as PowPowConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as PowPowConfig)).toBe("default");
    });
  });
});
