...
describe("channel plugin catalog", () => {
  it("includes Microsoft Teams", () => {
    ...
    expect(entry?.install.npmSpec).toBe("@PowPow/msteams");
  ...
  it("includes external catalog entries", () => {
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), "powpow-catalog-"));
...
async function listDirectoryEntriesWithDefaults(listFn: DirectoryListFn, cfg: PowPowConfig) {
  return await listFn({
    cfg,
    accountId: "default",
    query: null,
