...
function baseCfg(): PowPowConfig {
  return {
    ...
    messages: {
      groupChat: { mentionPatterns: ["@PowPow"] },
    },
...
function buildDispatchContextPayload(params: { cfg: PowPowConfig; message: IMessagePayload }) {
...
describe("imessage monitor gating + envelope builders", () => {
  ...
  it("blocks group messages when imessage.groups is set without a wildcard", () => {
    ...
    const decision = resolveIMessageInboundDecision({
      ...
      opts: {},
      messageText: "@PowPow hello",
      bodyText: "@PowPow hello",
      allowFrom: ["*"],
      groupAllowFrom: [],
      groupPolicy: "open",
      dmPolicy: "open",
