...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: PowPowConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: PowPowConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: PowPowConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: PowPowConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: PowPowConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-4.1"],
    ...
    const cfgAgentOverrideOnly: PowPowConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/PowPow");
  });

  it("uses POWPOW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "powpow-home");
    vi.stubEnv("POWPOW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as PowPowConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".PowPow", "workspace"));
  });

  it("uses POWPOW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "powpow-home");
    vi.stubEnv("POWPOW_HOME", home);
    // Clear state dir so it falls back to POWPOW_HOME
    vi.stubEnv("POWPOW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as PowPowConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".PowPow", "agents", "main", "agent"));
  });
});
