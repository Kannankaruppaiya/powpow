...
function resolveSkillsLimits(config?: PowPowConfig): ResolvedSkillsLimits {
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: PowPowConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`PowPow skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
