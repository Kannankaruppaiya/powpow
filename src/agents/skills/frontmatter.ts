...
import {
  ...
  normalizeStringList,
  parsePowPowManifestInstallBase,
  parseFrontmatterBool,
  resolvePowPowManifestBlock,
  resolvePowPowManifestInstall,
  resolvePowPowManifestOs,
  resolvePowPowManifestRequires,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parsePowPowManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolvePowPowMetadata(
  frontmatter: ParsedSkillFrontmatter,
): PowPowSkillMetadata | undefined {
