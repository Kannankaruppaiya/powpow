...
import { createPowPowCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-pi-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: PowPowConfig) {
    return createPowPowCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<PowPowConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<PowPowConfig["agents"]>["list"]>[number]["tools"];
  }): PowPowConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createPowPowCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createPowPowCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createPowPowCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createPowPowCodingTools({
      config: cfg,
    ...
    const bobTools = createPowPowCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createPowPowCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createPowPowCodingTools({
      config: cfg,
