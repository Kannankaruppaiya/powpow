...
import { resolvePreferredPowPowTmpDir } from "../infra/tmp-powpow-dir.js";
...
async function withOutsideHardlinkInPowPowTmp<T>(
  params: {
    PowPowTmpDir: string;
  ...
  const hardlinkPath = path.join(params.PowPowTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.PowPowTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.PowPowTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.PowPowTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const PowPowTmpDir = resolvePreferredPowPowTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred PowPow tmp root",
      media: path.join(PowPowTmpDir, "image.png"),
      expected: path.join(PowPowTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred PowPow tmp root",
      media: pathToFileURL(path.join(PowPowTmpDir, "photo.png")).href,
      expected: path.join(PowPowTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred PowPow tmp root",
      media: path.join(PowPowTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(PowPowTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(PowPowTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside PowPow tmp root",
      media: path.join(os.tmpdir(), "outside-PowPow", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked PowPow tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(PowPowTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(PowPowTmpDir, { recursive: true });
      const symlinkPath = path.join(PowPowTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked PowPow tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInPowPowTmp(
      {
        PowPowTmpDir,
  ...
  it("rejects symlinked PowPow tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInPowPowTmp(
