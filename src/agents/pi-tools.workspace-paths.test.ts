...
describe("workspace path resolution", () => {
  it("resolves relative read/write/edit paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("powpow-ws-", async (workspaceDir) => {
      await withTempDir("powpow-cwd-", async (otherDir) => {
        const cwdSpy = vi.spyOn(process, "cwd").mockReturnValue(otherDir);
        try {
          const tools = createPowPowCodingTools({ workspaceDir });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("powpow-ws-", async (workspaceDir) => {
      await withTempDir("powpow-override-", async (overrideDir) => {
        const tools = createPowPowCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
  ...
  it("rejects @-prefixed absolute paths outside workspace when workspaceOnly is enabled", async () => {
    await withTempDir("powpow-ws-", async (workspaceDir) => {
      const cfg: PowPowConfig = { tools: { fs: { workspaceOnly: true } } };
      const tools = createPowPowCodingTools({ workspaceDir, config: cfg });
      const { readTool } = expectReadWriteEditTools(tools);

      const outsideAbsolute = path.resolve(path.parse(workspaceDir).root, "outside-PowPow.txt");
      await expect(
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("powpow-sandbox-", async (sandboxDir) => {
      await withTempDir("powpow-workspace-", async (workspaceDir) => {
        ...
        const tools = createPowPowCodingTools({ workspaceDir, sandbox });
        const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
