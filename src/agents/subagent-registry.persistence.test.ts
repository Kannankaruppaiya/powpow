...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["POWPOW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-subagent-"));
  ...
  it("uses isolated temp state when POWPOW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "powpow-test-state"));
  });
});
