...
describe("createPiToolsSandboxContext", () => {
  it("provides stable defaults for pi-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("powpow-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("powpow-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("powpow-sbx-");
