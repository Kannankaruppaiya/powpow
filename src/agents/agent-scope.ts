...
type AgentEntry = NonNullable<NonNullable<PowPowConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: PowPowConfig): AgentEntry[] {
...
export function listAgentIds(cfg: PowPowConfig): string[] {
...
export function resolveDefaultAgentId(cfg: PowPowConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: PowPowConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: PowPowConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: PowPowConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: PowPowConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
