...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses POWPOW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      POWPOW_HOME: "/srv/powpow-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/powpow-home"), ".PowPow", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".PowPow", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("powpow-workspace-");
