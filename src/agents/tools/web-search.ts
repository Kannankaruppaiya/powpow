...
type WebSearchConfig = NonNullable<PowPowConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: PowPowConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("PowPow configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.PowPow.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://PowPow.ai",
      "X-Title": "PowPow Web Search",
    },
