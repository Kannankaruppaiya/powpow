...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    PowPow: process.env.POWPOW_GATEWAY_TOKEN,
    powpow: process.env.powpow_GATEWAY_TOKEN,
  };
  ...
  afterAll(() => {
    if (envSnapshot.PowPow === undefined) {
      delete process.env.POWPOW_GATEWAY_TOKEN;
    } else {
      process.env.POWPOW_GATEWAY_TOKEN = envSnapshot.PowPow;
    }
    if (envSnapshot.powpow === undefined) {
      delete process.env.powpow_GATEWAY_TOKEN;
    } else {
  ...
  it("uses POWPOW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.POWPOW_GATEWAY_TOKEN = "local-env-token";
    process.env.powpow_GATEWAY_TOKEN = "legacy-env-token";
    configState.value = {
      gateway: {
