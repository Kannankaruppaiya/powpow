...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): PowPowConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as PowPowConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): PowPowConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as PowPowConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  async function expectAccountStickerSend(cfg: PowPowConfig, accountId = "media") {
    await handleTelegramAction(
