...
describe("canvas host state dir defaults", () => {
  it("uses POWPOW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("powpow-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("PowPow Canvas");
      } finally {
        await handler.close();
      }
    });
