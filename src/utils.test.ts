...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("powpow-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("powpow-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("powpow-lid-a-", (first) => {
      withTempDirSync("powpow-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.PowPow when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "powpow-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers POWPOW_HOME over HOME", () => {
    vi.stubEnv("POWPOW_HOME", "/srv/powpow-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/powpow-home"));
...
describe("shortenHomePath", () => {
  it("uses $POWPOW_HOME prefix when POWPOW_HOME is set", () => {
    vi.stubEnv("POWPOW_HOME", "/srv/powpow-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/powpow-home")}/.PowPow/PowPow.json`)).toBe(
      "$POWPOW_HOME/.PowPow/PowPow.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $POWPOW_HOME replacement when POWPOW_HOME is set", () => {
    vi.stubEnv("POWPOW_HOME", "/srv/powpow-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/powpow-home")}/.PowPow/PowPow.json`),
    ).toBe("config: $POWPOW_HOME/.PowPow/PowPow.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/PowPow")).toBe(path.resolve(os.homedir(), "PowPow"));
  });
  ...
  it("prefers POWPOW_HOME for tilde expansion", () => {
    vi.stubEnv("POWPOW_HOME", "/srv/powpow-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/PowPow")).toBe(path.resolve("/srv/powpow-home", "PowPow"));
