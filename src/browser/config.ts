import type { BrowserConfig, BrowserProfileConfig, PowPowConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_POWPOW_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "PowPow" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_POWPOW_BROWSER_COLOR;
  }
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_POWPOW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_POWPOW_BROWSER_PROFILE_NAME] = {
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for PowPow/PowPow CDP).
