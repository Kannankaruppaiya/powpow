...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  if (prevGatewayPort === undefined) {
    delete process.env.POWPOW_GATEWAY_PORT;
    return;
  }
  process.env.POWPOW_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/PowPow" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "PowPow",
        profiles: {
          PowPow: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchPowPowChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolvePowPowUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopPowPowChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.POWPOW_GATEWAY_PORT;
  process.env.POWPOW_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.POWPOW_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.POWPOW_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.POWPOW_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.POWPOW_GATEWAY_PASSWORD;
  } else {
    process.env.POWPOW_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
