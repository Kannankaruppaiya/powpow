  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          PowPow: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.POWPOW_GATEWAY_PORT;
    process.env.POWPOW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.POWPOW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.POWPOW_GATEWAY_PASSWORD;
