...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredPowPowTmpDir: vi.fn(() => "/tmp/PowPow"),
}));
vi.mock("../infra/tmp-powpow-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredPowPowTmpDir.mockReturnValue("/tmp/PowPow");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredPowPowTmpDir.mockReturnValue("/tmp/powpow-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "powpow-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredPowPowTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "powpow-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "powpow-preferred", "downloads")}${path.sep}`),
    );
  });
