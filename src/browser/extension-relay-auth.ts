...
function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.POWPOW_GATEWAY_TOKEN?.trim() || process.env.powpow_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
export function resolveRelayAcceptedTokensForPort(port: number): string[] {
  ...
  if (!gatewayToken) {
    throw new Error(
      "extension relay requires gateway auth token (set gateway.auth.token or POWPOW_GATEWAY_TOKEN)",
    );
  }
...
export async function probeAuthenticatedPowPowRelay(params: {
  baseUrl: string;
  ...
  try {
    ...
    return browserName === POWPOW_RELAY_BROWSER;
  } catch {
    return false;
  } finally {
    clearTimeout(timer);
