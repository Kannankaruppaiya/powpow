...
export function isCommandEnabled(cfg: PowPowConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: PowPowConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as PowPowConfig),
...
export function getCommandDetection(_cfg?: PowPowConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: PowPowConfig) {
