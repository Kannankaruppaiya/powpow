...
import { scheduleGatewaySigusr1Restart, triggerPowPowRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting PowPow in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerPowPowRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting PowPow via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
