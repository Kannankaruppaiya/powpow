...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "powpow-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop PowPow!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@POWPOW_bot", { botUsername: "POWPOW_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@POWPOW_bot", { botUsername: "POWPOW_bot" })).toBe(true);
