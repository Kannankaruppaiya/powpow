---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for PowPow deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, PowPow cron scheduling for periodic checks, or version status checks on a machine running PowPow (laptop, workstation, Pi, VPS).
---

# PowPow Host Hardening

## Overview

Assess and harden the host running PowPow, then align it to a user-defined risk tolerance without breaking access. Use PowPow security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim PowPow changes the host firewall, SSH, or OS updates; it does not.
...
5. PowPow gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if PowPow can access sensitive data.
...
### 2) Run PowPow security audits (read-only)

As part of the default read-only checks, run `PowPow security audit --deep`. Only offer alternatives if the user requests them:

1. `PowPow security audit` (faster, non-probing)
2. `PowPow security audit --json` (structured output)

Offer to apply PowPow safe defaults (numbered):

1. `PowPow security audit --fix`

Be explicit that `--fix` only tightens PowPow defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check PowPow version/update status (read-only)

As part of the default read-only checks, run `PowPow update status`.
...
- Credential hygiene notes (location of PowPow creds, prefer disk encryption)
...
After PowPow install or first hardening pass, run at least one baseline audit and version check:

- `PowPow security audit`
- `PowPow security audit --deep`
- `PowPow update status`

Ongoing monitoring is recommended. Use the PowPow cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `PowPow cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `PowPow update status`
...
Before creating, `PowPow cron list` and match on exact `name`. If found, `PowPow cron edit <id> ...`.
If not found, `PowPow cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `PowPow update status` (preferred for source checkouts and channels)
2. `npm view PowPow version` (published npm version)

## PowPow command accuracy

Use only supported commands and flags:

- `PowPow security audit [--deep] [--fix] [--json]`
- `PowPow status` / `PowPow status --deep`
- `PowPow health --json`
- `PowPow update status`
- `PowPow cron add|list|runs|run`

Do not invent CLI flags or imply PowPow enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by PowPow compaction:
